	...
	private enum EnumItemFilterDefs implements ItemFilterTest {
	    has_only_supp_image_type("Item Image Bitstreams are Supported", null, CAT_MIME_SUPP) {
	        public boolean testItem(Context context, Item item) {
	        	int imageCount = ItemFilterUtil.countOriginalBitstreamMimeStartsWith(item, "image/");
	        	if (imageCount == 0) {
	        		return false;
	        	}
	        	int suppImageCount = ItemFilterUtil.countOriginalBitstreamMime(item, ItemFilterUtil.getSupportedImageMimeTypes());
	    ...
	    has_only_supp_doc_type("Item Document Bitstreams are Supported", null, CAT_MIME_SUPP) {
	        public boolean testItem(Context context, Item item) {
	        	int docCount = ItemFilterUtil.countOriginalBitstreamMime(item, ItemFilterUtil.getDocumentMimeTypes());
	        	if (docCount == 0) {
	        		return false;
	        	}
	        	int suppDocCount = ItemFilterUtil.countOriginalBitstreamMime(item, ItemFilterUtil.getSupportedDocumentMimeTypes());
	    ...
	    has_small_pdf("Has unusually small PDF", null, ItemFilterDefs.CAT_MIME) {
	        public boolean testItem(Context context, Item item) {
	        	return ItemFilterUtil.countBitstreamSmallerThanMinSize(BundleName.ORIGINAL, item, "application/pdf", "rest-report-pdf-min-size") > 0;
	        }        
	    },
	    has_large_pdf("Has unusually large PDF", null, ItemFilterDefs.CAT_MIME) {
	        public boolean testItem(Context context, Item item) {
	        	return ItemFilterUtil.countBitstreamLargerThanMaxSize(BundleName.ORIGINAL,  item, "application/pdf", "rest-report-pdf-max-size") > 0;
	        }        
	    },
	    ...
	    has_small_thumbnail("Has unusually small thumbnail", null, CAT_MISC) {
	        public boolean testItem(Context context, Item item) {
	        	return ItemFilterUtil.countBitstreamSmallerThanMinSize(BundleName.THUMBNAIL, item, "image/jpeg", "rest-report-thumbnail-min-size") > 0;
	        }        
	    },
	    has_doc_without_text("Has document bitstream without TEXT item", null, ItemFilterDefs.CAT_MIME) {
	        public boolean testItem(Context context, Item item) {
	        	int countDoc = ItemFilterUtil.countOriginalBitstreamMime(item, ItemFilterUtil.getDocumentMimeTypes());
	        	if (countDoc == 0) {
	        		return false;
	        	}
