...
public class FilteredItemsResource extends Resource {
    ...
    public org.dspace.rest.common.ItemFilter getItemQuery(@QueryParam("expand") String expand, 
        ...
        try {
            ...
            String exists = " exists (select 1 from handle where item.item_id=handle.resource_id and handle.resource_type_id=2) ";
            StringBuilder sql = new StringBuilder("select item.* from item where" + exists);
            StringBuilder csql = new StringBuilder("select count(*) as count from item where"+exists);
            ...
            try {                
                for(int i=0; i<query_field.size(); i++) {
                    ...
                    if (field.equals("*")) {
                    	...
                    	addAnyFieldOp(context, csql, cparams, op, val);
                    } else if (field.endsWith(".*")) {
                        addUnqualifiedOp(context, sql, params, field, op, val);
                        addUnqualifiedOp(context, csql, cparams, field, op, val);
                    } else {
                        addOp(context, sql, params, field, op, val);                    	
                        addOp(context, csql, cparams, field, op, val);                    	
                    }
                ...
                if (queryList.length() > 0) {
                	...
                	csql.append(queryList.toString());
                }
                
                log.debug(csql.toString());
                
                TableRowIterator rows = DatabaseManager.query(context, csql.toString(), cparams.toArray(new Object[0]));
                if (rows.hasNext()) {
                    result.setUnfilteredItemCount(rows.next().getIntColumn("count"));
                }
                ...
                rows = DatabaseManager.queryTable(context, "item", sql.toString(), params.toArray(new Object[0]));
