...
public class SpiderDetector {
     ...
     * @return Set<String>
     */
    public static void addSpiderIpAddress(String ip) {
        loadSpiderIpAddresses();
        try {
            table.add(ip);
        } catch (Exception e) {
            log.error("Error Loading Spider " + ip + ":" + e.getMessage(), e);
        }
    }
    ...
    public static boolean isSpider(HttpServletRequest request) {
        boolean result = isSpiderIP(request);
        if(result)
            return true;
        else
        {
            boolean emptyResult =  isSpiderEmptyAgent(request);
            if(emptyResult)
                return true;
            else
            {
                boolean regexResult =  isSpiderAgentRegex(request);
                if(regexResult)
                    return true;
                else
                    return false;
    ...
    public static boolean isSpiderIP(HttpServletRequest request) {
        /*
    ...
    /*
    * 2) if the user-agent header is empty
    */
    public static boolean isSpiderEmptyAgent(HttpServletRequest request){
        boolean checkSpidersEmptyAgent = ConfigurationManager.getBooleanProperty("spider.agentempty.enabled", true);
    ...
    */
    public static boolean isSpiderAgentRegex(HttpServletRequest request){
