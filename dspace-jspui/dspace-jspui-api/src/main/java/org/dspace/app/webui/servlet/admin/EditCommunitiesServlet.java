...
public class EditCommunitiesServlet extends DSpaceServlet {
  ...
  protected void doDSPost(Context context, HttpServletRequest request,
    ...
    /*
     * Most of the forms supply one or more of these values. Since we just get
     * null if we try and find something with ID -1, we'll just try and find
     * both here to save hassle later on
     */
    ...
    /*
     * First we check for a "cancel" button - if it's been pressed, we simply
     * return to the main control page
     */
    ...
    case START_CREATE_COMMUNITY:
      // no authorize attribute will be given to the jsp so a "clean" creation
      // form
      // will be always supplied, advanced setting on policies and admin group
      // creation
    ...
    case START_CREATE_COLLECTION:
      ...
      response.sendRedirect(response.encodeRedirectURL(request.getContextPath()
          + "/tools/collection-wizard?community_id=" + community.getID()));
    ...
    case CONFIRM_DELETE_COMMUNITY:
      ...
      if (parent == null) {
        response.sendRedirect(response.encodeRedirectURL(request
            .getContextPath() + "/community-list"));
      } else
      // redirect to parent community page
      {
        response.sendRedirect(response.encodeRedirectURL(request
            .getContextPath() + "/handle/" + parent.getHandle()));
      }
  ...
  static void storeAuthorizeAttributeCollectionEdit(Context context,
    ...
    if (AuthorizeManager.authorizeActionBoolean(context,
        collection.getParentObject(), Constants.REMOVE)) {
      request.setAttribute("delete_button", Boolean.TRUE);
    } else {
      request.setAttribute("delete_button", Boolean.FALSE);
    }
  ...
  private void processConfirmEditCommunity(Context context,
      ...
      Community community) throws ServletException, IOException, SQLException,
      AuthorizeException {
  ...
  private void processConfirmEditCollection(Context context,
      HttpServletRequest request, HttpServletResponse response,
      Community community, Collection collection) throws ServletException,
    ...
    // First, if this is not a harvested collection (anymore), set the harvest
    // type to 0; wipe harvest settings
