...
public class OAuthAuthenticationServlet extends DSpaceServlet {
	     ...
	     */
	    protected void doDSGet(Context context, HttpServletRequest request, HttpServletResponse response) throws javax.servlet.ServletException ,IOException ,SQLException ,org.dspace.authorize.AuthorizeException {
	    ...
	    // TODO Auto-generated method stub
            ...
            // Locate the eperson
            int status = AuthenticationManager.authenticate(context, null, null,
                            null, request);
			...
			if (status == AuthenticationMethod.SUCCESS) {
				...
				log.info(LogManager.getHeader(context, "failed_login",
						"type=orcid, no_such_user"));
				jsp = "/login/not-in-records.jsp";
			} else {
				log.info(LogManager.getHeader(context, "failed_login",
						"type=orcid, oauth authentication error"));
