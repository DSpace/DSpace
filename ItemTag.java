...
public class ItemTag extends TagSupport
{
    ...
    private String view_bitstream_agreement;
    ...
    public int doStartTag() throws JspException
    {
        try
        {
            ...
            view_bitstream_agreement= ConfigurationManager.getProperty( style + ".bitstream_view_agreement");
            log.debug("style = " + style);
            log.debug(style + ".bitstream_view_agreement =" +
                    ((view_bitstream_agreement== null) ? "null" : view_bitstream_agreement));
        ...
        try
        {
        	...
        	else
        	{
            	...
            	if (html)
            	{
                    ...
                    String bsLink = request.getContextPath() + "/html/" + handle + "/" +
                        UIUtil.encodeBitstreamName(primaryBitstream .getName(), Constants.DEFAULT_ENCODING); 
                    out.print(bitstreamLink(bsLink, primaryBitstream.getName()));
            		...
            		out.print(bitstreamLink(  bsLink, 
            		...
            		for (int i = 0; i < bundles.length; i++)
            		{
            			...
            			for (int k = 0; k < bitstreams.length; k++)
            			{
            				...
            				if (!bitstreams[k].getFormat().isInternal())
            				{
            					...
            					out.print("<tr><td headers=\"t1\" class=\"standard\">");
            					out.print(bitstreamLink(bsLink, bitstreams[k].getName()));
    ...
    private String bitstreamLink(String url, String displayText) {
        if (view_bitstream_agreement != null) {
            assert(style != null);
            return String.format("<a target=_blank href=\"javascript:agree_to_view('%s', '%s', '%s')\">%s</a>",
                    style, url, view_bitstream_agreement, displayText);
        } else
           return String.format("<a target=_blank href=\"%s\">%s</a>", url, displayText);
    }
