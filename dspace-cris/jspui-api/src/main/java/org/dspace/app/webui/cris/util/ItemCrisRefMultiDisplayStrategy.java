...
public class ItemCrisRefMultiDisplayStrategy extends ASimpleDisplayStrategy implements IAtomicDisplayStrategy
{
    ...
    public static final Log log = LogFactory
            .getLog(ItemCrisRefMultiDisplayStrategy.class);
	...
	public String getMetadataDisplay(HttpServletRequest hrq, int limit,
        ...
        for (int j = 0; j < loopLimit; j++)
        {
            ...
            {
            	ChoiceAuthority ca = cam.getChoiceAuthority(metadataArray[j].schema, metadataArray[j].element, metadataArray[j].qualifier);
    			minConfidence = MetadataAuthorityManager.getManager().getMinConfidence(metadataArray[j].schema, metadataArray[j].element, metadataArray[j].qualifier);
                ...
                if (StringUtils.isNotBlank(metadataArray[j].authority)
                        && metadataArray[j].confidence >= minConfidence && StringUtils.isNotBlank(publicPath))
                {
