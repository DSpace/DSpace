 ...
 * @doc [dspace]/config/modules/evaluePreservation.cfg
...
public class EvaluatePreservation extends AbstractCurationTask {
	...
	public int perform(DSpaceObject dso) throws IOException {
		...
		try {
			...
			float value = this.evaluateItem(item);
	...
	private float evaluateItem(Item item) {
		float value = 0;
		while(this.getRecipe().hasNextStep()){
			RuleStep recipeStep = recipe.nextStep();
			int weigth = recipeStep.getRule().evaluate(item);
