...
public class ACE
{
    ...
    private static final int GRANTEE_ID_ANY = -1;
    ...
    public static ACE fromString(String s)
    {
        ...
        for (int i = 0; i < aceParts.length; i++)
        {
            ...
            else if (key.equals(GRANTEE_ID_KEYWORD))
            {
                ...
                {
                    granteeID = Integer.valueOf(value);
                }
                else
                {
    ...
    private ACE(int policy, int action, int granteeType, int granteeID)
    {
        this.policy = policy;
        this.action = action;
    ...
    public boolean matches(int userID, Set<Integer> groupIDs, int action)
    {
        if (this.action == action)
        {
            if (granteeType == ACE.GRANTEE_TYPE_USER)
            {
                if (granteeID == GRANTEE_ID_ANY || userID == granteeID)
                {
                    return true;
                }
            }
            ...
            {
                if (granteeID == GRANTEE_ID_ANY || groupIDs.contains(granteeID))
                {
                    return true;
                }
