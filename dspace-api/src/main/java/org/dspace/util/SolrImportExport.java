...
public class SolrImportExport
{
		...
		try
		{
			...
			if ("import".equals(action))
			{
				for (String indexName : indexNames)
				{
					try
					{
						importIndex(indexName, makeDirectoryName(line.getOptionValue(DIRECTORY_OPTION)), line.hasOption(CLEAR_OPTION));
					}
	 ...
	 */
	public static void importIndex(final String indexName, String fromDir, boolean clear) throws IOException, SolrServerException
	{
	...
	private static String makeExportFilename(String indexName, long totalRecords, int index)
	{
		return indexName
				       + "_export_"
				       + StringUtils.leftPad("" + (index / ROWS_PER_FILE), (int) Math.ceil(Math.log10(totalRecords / ROWS_PER_FILE)), "0")
				       + ".csv";
	}
