...
public class AuthorProfileUtil {
    ...
    public static AuthorProfile findAuthorProfileByAuthorityId(Context context, String authorityId) throws Exception {
        AuthorProfile authorProfile;
    ...
    public static List<String> toFilterQuery(Context context, AuthorProfile authorProfile) {
        ...
        filterQueries.append("author_ac:(");
        //filterQueries.append("\"");
        filterQueries.append(ClientUtils.escapeQueryChars(fullName.toLowerCase(new Locale("es"))+SolrServiceImpl.FILTER_SEPARATOR+fullName));
        //filterQueries.append("\"");

        for (Metadatum dcValue : authorProfile.getMetadata(AuthorProfile.AUTHOR_PROFILE_SCHEMA, "name", "variant", Item.ANY)) {

            if(!fullName.equals(dcValue.value))
            {
                filterQueries.append(" OR ");
                //filterQueries.append("\"");
                filterQueries.append(ClientUtils.escapeQueryChars(com.ibm.icu.text.Normalizer2.getNFDInstance().normalize(dcValue.value.toLowerCase(new Locale("es")).replaceAll(" "," "))+ SolrServiceImpl.FILTER_SEPARATOR+dcValue.value.replaceAll(" "," ")));
            }
                //filterQueries.append("\"");
        }
        filterQueries.append(")");
