...
public class V6_0_2016_01_26__DS_2188_Remove_DBMS_Browse_Tables implements JdbcMigration, MigrationChecksumProvider {
    ...
    private void dropTable(Connection connection, String tableName) {
        try {
            ...
            log.warn(
                "Database Table '" + tableName + " could not be dropped during migration. This warning may be " +
                    "ignored, if this table was already deleted.",
                sqe);
        }
    }
    ...
    private void dropSequence(Connection connection, String sequenceName) {
        try {
            ...
            // Ignore any errors (don't cause migration to fail), but log as warning
            log.warn(
                "Database Sequence '" + sequenceName + " could not be dropped during migration. This warning may be " +
                    "ignored, if this sequence was already deleted.",
                sqe);
        }
    }
    ...
    private void dropView(Connection connection, String viewName) {
        try {
            ...
            // Ignore any errors (don't cause migration to fail), but log as warning
            log.warn(
                "Database View '" + viewName + " could not be dropped during migration. This warning may be ignored, " +
                    "if this view was already deleted.",
                sqe);
        }
    }
