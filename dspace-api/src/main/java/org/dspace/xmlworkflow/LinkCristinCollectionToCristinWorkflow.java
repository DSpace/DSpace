...
public class LinkCristinCollectionToCristinWorkflow {
    ...
    private static void findCristinCollection(StringBuilder themeSB) throws SQLException {
        ...
        for (HarvestedCollection harvestedCollection : harvestedCollections) {
            ...
            final Collection cristinCollection = Collection.find(context, collectionId);
            if (cristinCollection == null) {
                continue;
            }

            final Deque<Community> communities = new ArrayDeque<>(Arrays.asList(cristinCollection.getCommunities()));
            while (!communities.isEmpty())
            {
                Community community = communities.pop();
                Community parent = community.getParentCommunity();
                if (parent == null) {
                    final String cristinHandle = cristinCollection.getHandle();
                    appendMapping(themeSB, community.getName(), cristinHandle);
                    break;
                } else {
                    communities.push(parent);
                }
            }
        }
    }
