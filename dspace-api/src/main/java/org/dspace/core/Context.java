...
public class Context {
    ...
    /**
     * Construct a new context object with default options. A database
     * connection is opened. No user is authenticated.
     */
    public Context() {
        this.mode = Mode.READ_WRITE;
        init();
    ...
    /**
     * Construct a new context object with the given mode enabled. A database
     * connection is opened. No user is authenticated.
     *
     ...
     *
     * @exception SQLException if there was an error obtaining a database
     * connection
     */
    private void init() {
     ...
     * changed by the current caller a warning will be displayed in log. Use:      <code>
     ...
     * committed.
     * <p>
     * Calling complete() on a Context which is no longer valid
     * (isValid()==false), is a no-op.
     *
     * @exception SQLException if there was an error completing the database
    ...
    /**
     * Commit the current transaction with the database, persisting any pending
     * changes. The database connection is not closed and can be reused
     * afterwards.
     *
     * <b>WARNING: After calling this method all previously fetched entities are
     * "detached" (pending changes are not tracked anymore). You have to reload
     * all entities you still want to work with manually after this method call
     * (see {@link Context#reloadEntity(ReloadableEntity)}).</b>
     *
     * @throws SQLException When committing the transaction in the database
     * fails.
     */
    ...
    /**
     * Retrieves the first element in the events list and removes it from the
     * list of events once retrieved
     *
     * @return The first event of the list or <code>null</code> if the list is
     * empty
     */
     ...
     * needed (see {@link Context#uncacheEntity(ReloadableEntity)} ()
     * uncacheEntity}).
     *
     ...
     *
     * BATCH_EDIT: Enabling batch edit mode means that the database connection
     * is configured so that it is optimized to process a large number of
     * records.
     *
     * READ_ONLY: READ ONLY mode will tell the database we are nog going to do
     * any updates. This means it can disable optimalisations for delaying or
     * grouping updates.
     *
     * READ_WRITE: This is the default mode and enables the normal database
     * behaviour. This behaviour is optimal for querying and updating a small
     * number of records.
     *
     ...
     *
     * Enabling batch processing mode means that the database connection is
     * configured so that it is optimized to process a large number of records.
     *
     * Disabling batch processing mode restores the normal behaviour that is
     * optimal for querying and updating a small number of records.
     *
     * @param batchModeEnabled When true, batch processing mode will be enabled.
    ...
    /**
     * Reload an entity from the database into the cache. This method will
     * return a reference to the "attached" entity. This means changes to the
     * entity will be tracked and persisted to the database.
     *
     * @param entity The entity to reload
     * @param <E> The class of the enity. The entity must implement the
     * {@link ReloadableEntity} interface.
     * @return A (possibly) <b>NEW</b> reference to the entity that should be
    ...
    /**
     * Remove an entity from the cache. This is necessary when batch processing
