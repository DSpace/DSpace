...
public class SolrDedupServiceImpl implements DedupService {
	...
	public enum DeduplicationFlag {
		...
		FAKE("fake", 0), MATCH("match", 1), REJECTWS("reject_ws", 2), REJECTWF("reject_wf",
	...
	protected /* HttpSolrServer */SolrClient getSolr() {
		if (solr == null) {
			String solrService = DSpaceServicesFactory.getInstance()
                    .getConfigurationService().getProperty("deduplication.search.server");
	...
	@Override
	public void indexContent(Context ctx, /*BrowsableDSpaceObject<UUID>*/DSpaceObject iu, boolean force) throws SearchServiceException {
	...
	private void fillSignature(Context ctx, /*BrowsableDSpaceObject*/DSpaceObject iu, Map<String, List<String>> tmpMapFilter,
	...
	private void buildPotentialMatch(Context ctx, /*BrowsableDSpaceObject*/DSpaceObject iu, Map<String, List<String>> tmpMapFilter,
		...
		try {
			...
			if (force) {
				...
				while (iter.hasNext()) {
					...
					for (Object id : ids) {
						/*RootObject*/DSpaceObject o = ContentServiceFactory.getInstance().getDSpaceObjectService(type).find(context,
...
//	}
			...
			try {
				...
				for (UUID id : itemids) {
					try {
						...
						Map<String, List<String>> tmpMapFilter = new HashMap<String, List<String>>();
						List<String> tmpFilter = new ArrayList<String>();
						fillSignature(context, /*(BrowsableDSpaceObject)*/(DSpaceObject) item, tmpMapFilter, tmpFilter);
						if (!tmpFilter.isEmpty()) {
							// retrieve all search plugin to build search document in the same index
							SearchDeduplication searchSignature = dspace.getServiceManager()
									.getServiceByName(/*CrisConstants.getEntityTypeText(Constants.ITEM)*/"item".toUpperCase()
											+ "SearchDeduplication", SearchDeduplication.class);
							if (onlyFake) {
	...
	private void buildFromDedupReject(Context ctx, /*BrowsableDSpaceObject*/DSpaceObject iu, Map<String, List<String>> tmpMapFilter,
