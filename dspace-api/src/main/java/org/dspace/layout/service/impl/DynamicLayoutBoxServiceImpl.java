 ...
 */
public class DynamicLayoutBoxServiceImpl implements DynamicLayoutBoxService {
    ...
    @Override
    public DynamicLayoutBox create(Context context) throws SQLException, AuthorizeException {
        if (!authorizeService.isAdmin(context)) {
            throw new AuthorizeException(
                "You must be an admin to create a Box");
        }
        return dao.create(context, new DynamicLayoutBox());
    }

    @Override
    public DynamicLayoutBox find(Context context, int id) throws SQLException {
        return dao.findByID(context, DynamicLayoutBox.class, id);
    }

    @Override
    public void update(Context context, DynamicLayoutBox boxList) throws SQLException, AuthorizeException {
        update(context, Collections.singletonList(boxList));
    }

    @Override
    public void update(Context context, List<DynamicLayoutBox> boxList) throws SQLException, AuthorizeException {
        if (!authorizeService.isAdmin(context)) {
            throw new AuthorizeException(
                "You must be an admin to update a Box");
        }
        if (CollectionUtils.isNotEmpty(boxList)) {
            for (DynamicLayoutBox box : boxList) {
    ...
    @Override
    public void delete(Context context, DynamicLayoutBox box) throws SQLException, AuthorizeException {
        if (!authorizeService.isAdmin(context)) {
            throw new AuthorizeException(
    ...
    @Override
    public DynamicLayoutBox create(Context context, DynamicLayoutBox box) throws SQLException, AuthorizeException {
        if (!authorizeService.isAdmin(context)) {
            throw new AuthorizeException(
                "You must be an admin to create a Box");
        }
    ...
    @Override
    public DynamicLayoutBox create(Context context, EntityType eType, String boxType, boolean collapsed,
    ...
    @Override
    public boolean hasContent(Context context, DynamicLayoutBox box, Item item) {
    ...
    @Override
    public boolean hasAccess(Context context, DynamicLayoutBox box, Item item) {
        return dynamicLayoutBoxAccessService.hasAccess(context, context.getCurrentUser(), box, item);
    }

    @Override
    public DynamicLayoutBoxConfiguration getConfiguration(DynamicLayoutBox box) {
        return new DynamicLayoutBoxConfiguration(box);
    }

    private boolean hasMetadataBoxContent(Context context, DynamicLayoutBox box, Item item) {

        List<DynamicLayoutField> boxFields = box.getLayoutFields();
        if (CollectionUtils.isEmpty(boxFields)) {
            return false;
        }

        for (DynamicLayoutField field : boxFields) {

            if (field.isMetadataField() && isMetadataFieldPresent(item, field.getMetadataField())) {
                return true;
            }

            if (field.isBitstreamField() && isBitstreamPresent(context, item, (DynamicLayoutFieldBitstream) field)) {
                return true;
            }
    ...
    private boolean isBitstreamPresent(Context context, Item item, DynamicLayoutFieldBitstream field) {
