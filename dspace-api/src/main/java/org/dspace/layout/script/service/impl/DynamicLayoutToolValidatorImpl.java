...
import org.dspace.layout.script.service.DynamicLayoutToolRenderValidator;
...
/**
 * Implementation of {@link DynamicLayoutToolValidator}.
 *
 * @author Luca Giamminonni (luca.giamminonni at 4science.it)
 *
 */
public class DynamicLayoutToolValidatorImpl implements DynamicLayoutToolValidator {
    ...
    @Autowired
    private List<DynamicLayoutToolRenderValidator> renderingValidators;

    private Map<String, DynamicLayoutToolRenderValidator> renderingValidatorsByName;

    @PostConstruct
    private void setup() {
        renderingValidatorsByName = renderingValidators.stream()
            .collect(Collectors.toMap(DynamicLayoutToolRenderValidator::getName, Function.identity()));
    }

    @Override
    public DynamicLayoutToolValidationResult validate(Context context, Workbook workbook) {

        List<String> allEntityTypes = getAllEntityTypes(context);
        List<String> allMetadataFields = getAllMetadataFields(context);

        DynamicLayoutToolValidationResult result = new DynamicLayoutToolValidationResult();
    ...
    private void validateTabSheet(Workbook workbook, DynamicLayoutToolValidationResult result,
    ...
    private void validateDuplicateBoxes(Sheet boxSheet, DynamicLayoutToolValidationResult result,
    ...
    private void validateTab2BoxSheet(Workbook workbook, DynamicLayoutToolValidationResult result) {
    ...
    private void validateBox2MetadataSheet(List<String> allMetadataFields,
        Workbook workbook, DynamicLayoutToolValidationResult result) {
    ...
    private void validateBoxPolicySheet(Context context, List<String> allMetadataFields, Workbook workbook,
        DynamicLayoutToolValidationResult result) {
    ...
    private void validatePolicySheet(Context context, List<String> allMetadataFields, Workbook workbook,
        DynamicLayoutToolValidationResult result, String policySheetName) {
    ...
    private void validateBox2MetadataFieldTypes(Sheet sheet, DynamicLayoutToolValidationResult result, int typeColumn) {
    ...
    private void validateFieldType(Row row, DynamicLayoutToolValidationResult result, int bundleColumn,
    ...
    private void validateMetadataFields(List<String> allMetadataFields, Sheet sheet,
        int metadataColumn, int fieldTypeColumn, DynamicLayoutToolValidationResult result) {
    ...
    private void validateRenderingColumn(Sheet sheet, int fieldTypeColumn, DynamicLayoutToolValidationResult result) {
    ...
    private void validateRenderType(Cell cell, int fieldTypeColumn, DynamicLayoutToolValidationResult result) {

        String renderType = WorkbookUtils.getCellValue(cell);

        String fieldType = getCellValue(cell.getRow(), fieldTypeColumn);
        if (StringUtils.isEmpty(fieldType)) {
            fieldType = DynamicLayoutBoxTypes.METADATA.name();
        }

        String sheetName = cell.getRow().getSheet().getSheetName();

        DynamicLayoutToolRenderValidator renderValidator = findRenderValidatorByName(renderType);
    ...
    private DynamicLayoutToolRenderValidator findRenderValidatorByName(String renderType) {
    ...
    private void validatePresenceInBoxSheet(DynamicLayoutToolValidationResult result, Sheet sheet,
    ...
    private void validateTab2BoxRowsReferences(Sheet tab2boxSheet, DynamicLayoutToolValidationResult result) {
    ...
    private void validatePresenceInTab2BoxSheet(DynamicLayoutToolValidationResult result, Sheet sheet,
        String columnName, int entityTypeColumn, int shortnameColumn) {
    ...
    private void validateTab2BoxRowReferences(Row row, DynamicLayoutToolValidationResult result,
    ...
    private void validateEntityTypes(DynamicLayoutToolValidationResult result, Sheet sheet,
    ...
    private void validateBoxTypes(DynamicLayoutToolValidationResult result, Sheet sheet, int typeColumn) {

        for (Cell typeCell : getColumnWithoutHeader(sheet, typeColumn)) {
            String type = WorkbookUtils.getCellValue(typeCell);
            if (StringUtils.isNotBlank(type) && !EnumUtils.isValidEnum(DynamicLayoutBoxTypes.class, type)) {
    ...
    private void validateColumnsPresence(Sheet sheet, DynamicLayoutToolValidationResult result, String... columns) {
    ...
    private void validateBooleanColumns(Sheet sheet, DynamicLayoutToolValidationResult result, String... columnNames) {
    ...
    private void validateIntegerColumns(Sheet sheet, DynamicLayoutToolValidationResult result, String... columnNames) {
        for (String columnName : columnNames) {
            validateColumn(sheet, columnName, (value) -> isNotInteger(value), result, "integer values");
        }
    }

    private void validateSecurityColumns(Sheet sheet, DynamicLayoutToolValidationResult result, String... columnNames) {
        for (String columnName : columnNames) {
    ...
    private void validateColumn(Sheet sheet, String columnName, Predicate<String> predicate,
        DynamicLayoutToolValidationResult result, String allowedValues) {
