...
class GoogleRunnable implements Runnable {
    ...
    public void run() {
        ...
        nvps.add(new BasicNameValuePair("aip", "1"));
        ...
        try {
            ...
            log.debug("GoogleAnalyticsWorker, User-Agent was null: "+e.getMessage());
        }
        try {
            nvps.add(new BasicNameValuePair("dr", ue.getRequest().getHeader("referer")));
        } catch (NullPointerException e) {
            log.debug("GoogleAnalyticsWorker, Referer was null: "+e.getMessage());
        }
