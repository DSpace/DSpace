...
public class GoogleAsyncEventListener extends AbstractUsageEventListener {
    ...
    public GoogleAsyncEventListener(ConfigurationService configurationServiceRef) {
        ...
        if (StringUtils.isNotEmpty(analyticsKey)) {
            int analyticsBufferlimit = configurationService.getPropertyAsType("google.analytics.buffer.limit", 256);
    ...
    private String getObjectName(UsageEvent ue) {
        try {
            if (ue.getObject().getType() == Constants.BITSTREAM) {
                ...
                return DSpaceObject.find(ue.getContext(), ue.getObject().getType(), ue.getObject().getID()).getParentObject().getName();
            } else {
    ...
    private String getIPAddress(HttpServletRequest request) {
        ...
        if (configurationService.getPropertyAsType("useProxies", false) && request.getHeader("X-Forwarded-For") != null) {
