...
public class EPersonDAOImpl extends AbstractHibernateDSODAO<EPerson> implements EPersonDAO {
    ...
    public EPerson findByEmail(Session session, String email) throws SQLException {
        ...
        return uniqueResult(session, criteriaQuery, true, EPerson.class);
    }
    ...
    public List<EPerson> search(Session session, String query, List<MetadataField> queryFields,
        ...
        Query hibernateQuery = getSearchQuery(session, queryString, query, queryFields, sortFields, null);
    ...
    public int searchResultCount(Session session, String query, List<MetadataField> queryFields) throws SQLException {
        ...
        Query hibernateQuery = getSearchQuery(session, queryString, query, queryFields, Collections.EMPTY_LIST, null);

        return count(hibernateQuery);
    }

    @Override
    public List<EPerson> findAll(Session session, MetadataField metadataSortField, String sortField, int pageSize,
        ...
        Query query = getSearchQuery(session, queryString, null, ListUtils.EMPTY_LIST, sortFields, sortField, pageSize,
                                     offset);
        return list(query);
    ...
    public List<EPerson> findWithPasswordWithoutDigestAlgorithm(Session session) throws SQLException {
        ...
        return list(session, criteriaQuery, false, EPerson.class, -1, -1);
    }
    ...
    protected Query getSearchQuery(Session session, String queryString, String queryParam,
                                   List<MetadataField> queryFields, List<MetadataField> sortFields, String sortField)
        throws SQLException {
        return getSearchQuery(session, queryString, queryParam, queryFields, sortFields, sortField, -1, -1);
    }
    ...
    public List<EPerson> findAllSubscribers(Session session) throws SQLException {
        return list(createQuery(session, "SELECT DISTINCT e from Subscription s join s.ePerson e"));
    }

    @Override
    public int countRows(Session session) throws SQLException {
        return count(createQuery(session, "SELECT count(*) FROM EPerson"));
    }
}
