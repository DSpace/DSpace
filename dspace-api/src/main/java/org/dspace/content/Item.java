...
public class Item extends DSpaceObject {
  ...
  public static ItemIterator findAll(Context context) throws SQLException {
    ...
    TableRowIterator rows = DatabaseManager
        .queryTable(context, "item", myQuery);
  ...
  /**
   * Get Dublin Core metadata for the item. Passing in a <code>null</code> value
   * for <code>qualifier</code> or <code>lang</code> only matches Dublin Core
   * fields where that qualifier or languages is actually <code>null</code>.
   * Passing in <code>Item.ANY</code> retrieves all metadata fields with any
   * value for the qualifier or language, including <code>null</code>
   * <P>
   * Examples:
   * <P>
   * Return values of the unqualified "title" field, in any language. Qualified
   * title fields (e.g. "title.uniform") are NOT returned:
   * <P>
   ...
   * @param lang
   *          the ISO639 language code, optionally followed by an underscore and
   *          the ISO3166 country code. <code>null</code> means only values with
   *          no language are returned, and <code>Item.ANY</code> means values
   ...
   * a <code>null</code> value for <code>qualifier</code> or <code>lang</code>
   * only matches metadata fields where that qualifier or languages is actually
   * <code>null</code>. Passing in <code>Item.ANY</code> retrieves all metadata
   * fields with any value for the qualifier or language, including
   * <code>null</code>
   * <P>
   * Examples:
   * <P>
   ...
   *          <code>null</code> doesn't really make sense as all metadata must
   *          have an element.
   * @param qualifier
   *          the qualifier. <code>null</code> means unqualified, and
   *          <code>Item.ANY</code> means any qualifier (including unqualified.)
   ...
   * @param qualifier
   *          the Dublin Core qualifier, or <code>null</code> for unqualified
   * @param lang
   *          the ISO639 language code, optionally followed by an underscore and
   *          the ISO3166 country code. <code>null</code> means the value has no
   ...
   * The authority confidence depends on whether authority is <em>required</em>
   * or not.
   * 
   * @param schema
   *          the schema for the metadata field. <em>Must</em> match the
   ...
   * @param qualifier
   *          the metadata qualifier name, or <code>null</code> for unqualified
   * @param lang
   *          the ISO639 language code, optionally followed by an underscore and
   *          the ISO3166 country code. <code>null</code> means the value has no
   *          language (for example, a date).
  ...
  public void addMetadata(String schema, String element, String qualifier,
    ...
    String fieldKey = MetadataAuthorityManager.makeFieldKey(schema, element,
        qualifier);
    if (mam.isAuthorityControlled(fieldKey)) {
      ...
      for (int i = 0; i < values.length; ++i) {
        Choices c = ChoiceAuthorityManager.getManager().getBestMatch(fieldKey,
            values[i], getOwningCollectionID(), null);
   ...
   */
  public void addMetadata(String schema, String element, String qualifier,
      String lang, String[] values, String authorities[], int confidences[]) {
    List<DCValue> dublinCore = getMetadata();
    MetadataAuthorityManager mam = MetadataAuthorityManager.getManager();
    boolean authorityControlled = mam.isAuthorityControlled(schema, element,
        qualifier);
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      // - if authority key is present, use given confidence or NOVALUE if not
      // given
      // - otherwise, preserve confidence if meaningful value was given since it
      // may document a failed authority lookup
      // - CF_UNSET signifies no authority nor meaningful confidence.
      // - it's possible to have empty authority & CF_ACCEPTED if e.g. user
      // deletes authority key
      if (authorityControlled) {
        ...
        // XXX FIXME? can't throw a "real" exception here without changing all
        // the callers to expect it, so use a runtime exception
   ...
   * language is actually <code>null</code>.<code>Item.ANY</code> will match any
   * element, qualifier or language, including <code>null</code>. Thus,
   ...
   *          the ISO639 language code, optionally followed by an underscore and
   *          the ISO3166 country code. <code>null</code> means only values with
   *          no language are removed, and <code>Item.ANY</code> means values
   ...
   * and language passed in can be the <code>Item.ANY</code> wildcard.
   * 
   * @param schema
   *          the schema for the metadata field. <em>Must</em> match the
   *          <code>name</code> of an existing metadata schema.
  ...
  public EPerson getSubmitter() throws SQLException {
    if (submitter == null && !itemRow.isColumnNull("submitter_id")) {
      submitter = EPerson
          .find(ourContext, itemRow.getIntColumn("submitter_id"));
    }
    return submitter;
  }
  ...
  public boolean isIn(Collection collection) throws SQLException {
    TableRow tr = DatabaseManager.querySingle(ourContext,
        "SELECT COUNT(*) AS count" + " FROM collection2item"
            + " WHERE collection_id = ? AND item_id = ?", collection.getID(),
        itemRow.getIntColumn("item_id"));
  ...
  public Collection[] getCollections() throws SQLException {
    ...
    TableRowIterator tri = DatabaseManager.queryTable(ourContext, "collection",
        "SELECT collection.* FROM collection, collection2item WHERE "
            ...
            + "collection2item.item_id= ? ", itemRow.getIntColumn("item_id"));
  ...
  public Community[] getCommunities() throws SQLException {
    ...
    TableRowIterator tri = DatabaseManager
        .queryTable(ourContext, "community",
    ...
    if (bundles == null) {
      ...
      TableRowIterator tri = DatabaseManager.queryTable(ourContext, "bundle",
          "SELECT bundle.* FROM bundle, item2bundle WHERE "
              ...
              + "item2bundle.item_id= ? ", itemRow.getIntColumn("item_id"));
  ...
  public void update(boolean bLastModified) throws SQLException,
    ...
    if (dublinCoreChanged) {
      ...
      for (int dcIdx = 0; dcIdx < getMetadata().size(); dcIdx++) {
        ...
        // Store the calculated place number, reset the stored flag, and cache
        // the metadatafield
        ...
        if (dcFields[dcIdx] == null) {
          ...
          log.warn(LogManager.getHeader(ourContext, "bad_dc",
              "Bad DC field. schema=" + dcv.schema + ", element: \""
                  + ((dcv.element == null) ? "null" : dcv.element)
                  + "\" qualifier: \""
                  + ((dcv.qualifier == null) ? "null" : dcv.qualifier)
                  + "\" value: \"" + ((dcv.value == null) ? "null" : dcv.value)
                  + "\""));

          throw new SQLException("bad_dublin_core " + "schema=" + dcv.schema
      ...
      if (tri != null) {
        try {
          while (tri.hasNext()) {
            ...
            // Go through the in-memory metadata, unless we've already decided
            // to keep this row
            for (int dcIdx = 0; dcIdx < getMetadata().size() && removeRow; dcIdx++) {
              // Only process if this metadata has not already been matched to
              // something in the DB
              if (!storedDC[dcIdx]) {
                ...
                if (matched) {
                  ...
                  if (!((dcv.authority == null && auth == null) || (dcv.authority != null
                      && auth != null && dcv.authority.equals(auth))
                      && dcv.confidence == conf)) {
                    matched = false;
                  }
                }
            ...
            // If after processing all the metadata values, we didn't find a
            // match
    ...
    if (dublinCoreChanged || modified) {
      ...
      if (dublinCoreChanged) {
        ourContext.addEvent(new Event(Event.MODIFY_METADATA, Constants.ITEM,
            getID(), getDetails()));
        clearDetails();
        dublinCoreChanged = false;
      }
      ...
      ourContext
          .addEvent(new Event(Event.MODIFY, Constants.ITEM, getID(), null));
      modified = false;
    }
  }
  ...
  private MetadataField getMetadataField(DCValue dcv) throws SQLException,
      AuthorizeException {
  ...
  void delete() throws SQLException, AuthorizeException, IOException {
    ...
    // metadata but when getting to the point of removing the bundles we get an
    // exception
     ...
     * Event consumers, removing an Item *must* be done *here* (inline) because
     * otherwise, tables are left in an inconsistent state and the DB
     * transaction will fail. Any fix would involve too much work on Browse code
     * that is likely to be replaced soon anyway. --lcs, Aug 2006
     ** 
     ** NB Do not check to see if the item is archived - withdrawn / non-archived
  ...
  public void inheritCollectionDefaultPolicies(Collection c)
    ...
    List<ResourcePolicy> policies = AuthorizeManager.getPoliciesActionFilter(
        ourContext, c, Constants.DEFAULT_ITEM_READ);
  ...
  public void move(Collection from, Collection to,
      boolean inheritDefaultPolicies) throws SQLException, AuthorizeException,
      IOException {
    ...
    if (isOwningCollection(from)) {
      ...
      log.info(LogManager.getHeader(ourContext, "move_item", "item_id="
          + getID() + ", from " + "collection_id=" + from.getID() + " to "
  ...
  public static ItemIterator findByMetadataField(Context context,
    ...
    MetadataField mdf = MetadataField.findByElement(context, mds.getSchemaID(),
        element, qualifier);
    if (mdf == null) {
      throw new IllegalArgumentException("No such metadata field: schema="
          + schema + ", element=" + element + ", qualifier=" + qualifier);
    }
  ...
  public DSpaceObject getAdminObject(int action) throws SQLException {
    ...
    case Constants.ADD:
      ...
      // required by a cc or
      // a generic bitstream so simply we ignore it.. UI need to enforce the
      // requirements.
