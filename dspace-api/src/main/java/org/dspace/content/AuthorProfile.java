...
public class AuthorProfile extends DSpaceObject
{
    ...
    {
        EPerson usuario=getContext().getCurrentUser();
        String authorProfileMail=getMetadata("authorProfile.email");
        if(!AuthorizeManager.isAdmin(getContext()) && !(usuario!=null && authorProfileMail!=null && usuario.getEmail().equalsIgnoreCase(authorProfileMail))) {
