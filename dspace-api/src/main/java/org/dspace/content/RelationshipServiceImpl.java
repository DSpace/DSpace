...
public class RelationshipServiceImpl implements RelationshipService {
    ...
    @Override
    public Relationship createTypeLessRelationship(Context context, Item ownerItem, Item targetItem)
        throws AuthorizeException, SQLException {
        assertWriteOnEitherItem(context, ownerItem, targetItem);
        Relationship relationship = new Relationship();
        // Side convention (D21): owner -> left_id, target -> right_id.
        relationship.setLeftItem(ownerItem);
        relationship.setRightItem(targetItem);
        // No RelationshipType: this is an authority-backed, type-less row (type_id NULL).
        relationship.setRelationshipType(null);
        // Default places; there is no type-driven place logic for a type-less row.
        relationship.setLeftPlace(0);
        relationship.setRightPlace(0);
        relationship.setLatestVersionStatus(LatestVersionStatus.BOTH);
        // Persist directly via the DAO: we deliberately skip isRelationshipValidToCreate
        // (entity-type + cardinality checks) and updatePlaceInRelationship (virtual-metadata
        // place shifting), because all of those dereference the null relationship type.
     ...
     * type-less relationship. Used by the type-less create and delete paths, which do not go through the
