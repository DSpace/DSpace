...
/**
 * The primary class for retrieving authority choices from DSpace items.
 * Enables users to search and select from existing verified entities in the repository.
 * <h3>Logic &amp; Functionality:</h3>
 * <ul>
 * <li><strong>Dynamic Scoping:</strong> Uses the configuration property
 * {@code cris.ItemAuthority.{authorityName}.entityType} to inject a filter query
 * into Solr. This ensures that a "Person" authority only returns Items indexed
 * with that specific entity type.</li>
 * <li><strong>Frontend Auto-fill (data- prefixing):</strong> Integrates with
 * {@link ItemAuthorityUtils} to package target metadata into the {@code otherInformation}
 * map. By prefixing keys with {@code data-} (e.g., {@code data-oairecerif_author_affiliation}),
 * it signals the submission UI to automatically map these values into other form fields
 * during selection.</li>
 * <li><strong>Pluggable Filtering:</strong> Leverages {@link CustomAuthorityFilter}
 * to apply additional runtime constraints (e.g., filtering by collection or
 * discovery configuration) beyond the basic entity type.</li>
 * <li><strong>Extra Metadata:</strong> Additional metadata (extras) is generated by
 * implementations of {@link ItemAuthorityExtraMetadataGenerator} and included in the
 * Choice's otherInformation map.
 * </ul>
 * <h3>Configuration Example:</h3>
 * <p>
 * To create an authority for the <code>dc.contributor.author</code> field that searches
 * for Person entities:
 * </p>
 * <pre>
...
public class ItemAuthority implements ChoiceAuthority, LinkableEntityAuthority {
    ...
    final static String CHOICES_EXTERNALSOURCE_PREFIX = "choices.externalsource.";
