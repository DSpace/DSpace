 ...
 */
public class HarvestedCollectionDAOImpl extends AbstractHibernateDAO<HarvestedCollection> implements
    ...
    @Override
    public HarvestedCollection findByStatusAndMinimalTypeOrderByLastHarvestedDesc(Context context, int status, int
        type, int limit) throws SQLException {
...
// order by last_harvested desc limit 1";
    ...
    @Override
    public HarvestedCollection findByStatusAndMinimalTypeOrderByLastHarvestedAsc(Context context, int status, int
        type, int limit) throws SQLException {
//        Old query: "select collection_id from harvested_collection where harvest_type > ? and harvest_status = ?
// order by last_harvested asc limit 1";
    ...
    public List<HarvestedCollection> findByStatus(Context context, int status) throws SQLException {
        ...
        Root<HarvestedCollection> harvestedCollectionRoot = criteriaQuery.from(HarvestedCollection.class);
        criteriaQuery.select(harvestedCollectionRoot);
        criteriaQuery.where(criteriaBuilder.equal(harvestedCollectionRoot.get(HarvestedCollection_.harvestStatus),
            status));
    ...
    public HarvestedCollection findByCollection(Context context, Collection collection) throws SQLException {
        ...
        Root<HarvestedCollection> harvestedCollectionRoot = criteriaQuery.from(HarvestedCollection.class);
        criteriaQuery.select(harvestedCollectionRoot);
        criteriaQuery.where(criteriaBuilder.equal(harvestedCollectionRoot.get(HarvestedCollection_.collection),
            collection));
        return singleResult(context, criteriaQuery);
    ...
    @Override
    public List<HarvestedCollection> findByLastHarvestedAndHarvestTypeAndHarvestStatusesAndHarvestTime(Context context,
        Date startTime, int minimalType, int[] statuses, int expirationStatus,
        ...
        Predicate orPredicate = criteriaBuilder.or(criteriaBuilder.lessThan(harvestedCollectionRoot.get
                (HarvestedCollection_.lastHarvested), startTime),
        ...
        for (int status : statuses) {
            orPredicates.add(criteriaBuilder.equal(harvestedCollectionRoot.get(HarvestedCollection_.harvestStatus),
                status));
        }

        Predicate andPredicate = criteriaBuilder.and(criteriaBuilder.equal(harvestedCollectionRoot.get
                (HarvestedCollection_.harvestStatus), expirationStatus),
            criteriaBuilder.greaterThan(harvestedCollectionRoot.get(HarvestedCollection_.harvestStartTime),
                expirationTime)
        );
    ...
    protected CriteriaQuery getByStatusAndMinimalTypeCriteria(Context context, int status, int type, int limit)
        ...
        Root<HarvestedCollection> harvestedCollectionRoot = criteriaQuery.from(HarvestedCollection.class);
        criteriaQuery.select(harvestedCollectionRoot);
        criteriaQuery.where(criteriaBuilder.and(criteriaBuilder.greaterThan(harvestedCollectionRoot.get
                (HarvestedCollection_.harvestType), type),
