...
public class VersionedDOIIdentifierProvider extends DOIIdentifierProvider implements InitializingBean {
    ...
    void removePreviousVersionDOIsOutOfObject(Context c, Item item, String oldDoi)
        ...
        List<MetadataValue> identifiers = itemService
            .getMetadata(item, doiMetadataFieldName.schema, doiMetadataFieldName.element, doiMetadataFieldName.qualifier, Item.ANY);
        ...
        if (changed) {
            try {
                itemService.clearMetadata(c, item, doiMetadataFieldName.schema, doiMetadataFieldName.element, doiMetadataFieldName.qualifier, Item.ANY);
                // Checks if Array newIdentifiers is empty to avoid adding null values to the metadata field.
                if (!newIdentifiers.isEmpty()) {
                    itemService.addMetadata(c, item, doiMetadataFieldName.schema, doiMetadataFieldName.element, doiMetadataFieldName.qualifier, null, newIdentifiers);
                }
