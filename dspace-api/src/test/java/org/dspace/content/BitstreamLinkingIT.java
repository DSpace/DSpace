...
public class BitstreamLinkingIT extends AbstractIntegrationTestWithDatabase {
    ...
    public void testCopyBitstreams() throws Exception {
        ...
        assertThat("Metadata value (name) of: " + HAS_COPIES, copiesUUIDs.get(0).getValue(),
                equalTo(copy.getName()));
        assertThat("Metadata authority (uuid) of: " + HAS_COPIES, copiesUUIDs.get(0).getAuthority(),
        ...
        assertThat("Metadata value (name) of: " + IS_COPY_OF, originalUUIDs.get(0).getValue(),
                equalTo(original.getName()));
        assertThat("Metadata authority (uuid) of: " + IS_COPY_OF, originalUUIDs.get(0).getAuthority(),
        ...
        assertThat("Metadata value (name) of: " + HAS_COPIES, secondCopies.get(0).getValue(),
                equalTo(secondCopy.getName()));
        assertThat("Metadata authority (uuid) of: " + HAS_COPIES, secondCopies.get(0).getAuthority(),
    ...
    public void testReplacementBitstreams() throws Exception {
        ...
        assertThat("Metadata value (name) of: " + IS_REPLACED_BY, copiesUUIDs.get(0).getValue(),
                equalTo(copy.getName()));
        assertThat("Metadata authority (uuid) of: " + IS_REPLACED_BY, copiesUUIDs.get(0).getAuthority(),
                equalTo(copy.getID().toString()));

        List<MetadataValue> originalUUIDs = bitstreamService.getMetadata(copy, DSPACE,
                BITSTREAM, IS_REPLACEMENT_OF, null);
        assertThat("Metadata amount: " + IS_REPLACEMENT_OF, originalUUIDs.size(), equalTo(1));
        assertThat("Metadata value (name) of: " + IS_REPLACEMENT_OF, originalUUIDs.get(0).getValue(),
                equalTo(original.getName()));
        assertThat("Metadata authority (uuid) of: " + IS_REPLACEMENT_OF, originalUUIDs.get(0).getAuthority(),
                equalTo(original.getID().toString()));
