...
//    TODO i18n
    ...
    datatable.dataTable({
        ...
        "fnInitComplete": function() {
            ...
            var searchFilter = jQuery('.dataTables_filter > input');
            var initialInput = "";
            if (authorityInput.indexOf('value_') != -1) {
                initialInput = jQuery('textarea[name='+ authorityInput+']').val();
            }else{
                initialInput = (jQuery('input[name=' + authorityInput + '_last]').val() + " "+jQuery('input[name=' + authorityInput + '_first]').val()).trim();
            }
            searchFilter.val(initialInput);
            setTimeout(function () {
                searchFilter.trigger(jQuery.Event("keyup", { keyCode: 13 }));
            },50);
            jQuery('.dataTables_wrapper').parent().attr('style','width: auto; min-height: 121px; height: auto;');
        },
        ...
        "fnRowCallback": function( nRow, aData, iDisplayIndex ) {
            ...
            var authorityID = jQuery(this).closest('.dataTables_wrapper').find('.vcard-wrapper .vcard').data('authorityID');
...
//                vcard.find('.vcard-orcid span').empty().append('<a href="http://orcid.org/'+aData['symplectic']+'" target="_new">'+aData['symplectic']+'</a>');
                if(aData['insolr']!="false"){
                    var discoverLink = "/discover?filtertype=author&filter_relational_operator=authority&filter=" + aData['insolr'];
                ...
                vcard.find('.vcard-add input').click(function() {
                    if (authorityInput.indexOf('value_') != -1) {
                    // edit item
                        jQuery('input[name=' + authorityInput + ']').val(vcard.find('.vcard-last-name span').text() + ', ' + vcard.find('.vcard-first-name span').text());
                        var oldAuthority = jQuery('input[name=' + authorityInput + '_authority]');
                        oldAuthority.val(vcard.data('authorityID'));
                        jQuery('textarea[name='+ authorityInput+']').val(vcard.data('name'));
                    } else {
                        // submission
                        jQuery('input[name=' + authorityInput + '_last]').val(vcard.find('.vcard-last-name span').text());
                        jQuery('input[name=' + authorityInput + '_first]').val(vcard.find('.vcard-first-name span').text());
                        jQuery('input[name=' + authorityInput + '_authority]').val(vcard.data('authorityID'));
                        jQuery('input[name=submit_'+ authorityInput +'_add]').click();
        ...
        "fnDrawCallback": function() {
            var wrapper = jQuery(this).closest('.dataTables_wrapper');
            ...
            jQuery('#lookup-more-button').click(function () {
                button = lessButton;
                datatable.fnFilter(jQuery('.dataTables_filter > input').val());
            });
            jQuery('#lookup-less-button').click(function () {
                button = moreButton;
                datatable.fnFilter(jQuery('.dataTables_filter > input').val());
            });
        },
        "fnServerData": function (sSource, aoData, fnCallback) {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    var $xml = jQuery(data);
                    var aaData = [];
                    jQuery.each($xml.find('Choice'), function() {
