...
public class EditTermMetadataValueForm extends AbstractDSpaceTransformer {
    ...
    {
        Term term = Term.find(context, parameters.getParameterAsInteger("termID",-1));
        Concept owner = term.getConcepts()[0];
        int schemeID = (owner == null) ? -1 : owner.getID();
    ...
    public void addBody(Body body) throws SQLException, WingException
    {
        ...
        // DIVISION: main
        Division main = body.addInteractiveDivision("edit-item-status", contextPath+"/admin/term", Division.METHOD_POST,"primary administrative item");

        main.setHead("Edit Metadata For Term");
...
//            options.addItem().addXref(baseURL+"&submit_bitstreams",T_option_bitstreams);
        options.addItem().addHighlight("bold").addContent("Edit Metadata");
...
//            options.addItem().addXref(baseURL + "&submit_curate", T_option_curate);
        ...
        addForm.addItem().addHidden("termID").setValue(termID);
