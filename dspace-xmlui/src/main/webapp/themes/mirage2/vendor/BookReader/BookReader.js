...
function BookReader() {
    ...
    if(window.DSpace) {
        this.imagesBaseURL = window.DSpace.theme_path + '/vendor/BookReader/images/';
...
BookReader.prototype.initToolbar = function(mode, ui) {
    ...
    $("#BookReader").append(
        ...
        +     readIcon
        +     "<button class='BRicon full'></button>"
        +     "<button class='BRnoticon BRclose BRhide'><span class='glyphicon glyphicon-remove'></span></button>"
        +   "</span>"
...
BookReader.prototype.bindNavigationHandlers = function() {
    ...
    jIcons.filter('.onepg').click(function(e) {
        self.switchMode(self.constMode1up);
    });

    jIcons.filter('.twopg').click(function(e) {
        self.switchMode(self.constMode2up);
    });

    jIcons.filter('.thumb').click(function(e) {
    ...
    jIcons.filter('.book_up').click(function(e) {
    ...
    jIcons.filter('.book_down').click(function(e) {
        if ($.inArray(self.mode, [self.constMode1up, self.constModeThumb]) >= 0) {
            self.scrollDown();
        } else {
            self.next();
    ...
    jIcons.filter('.zoom_in').click(function() {
        self.ttsStop();
        self.zoom(1);
        return false;
    });

    jIcons.filter('.zoom_out').click(function() {
        self.ttsStop();
        self.zoom(-1);
        return false;
    });

    jIcons.filter('.full').click(function() {
        if (self.ui == 'embed') {
