...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="addRSSLinks">
        <xsl:for-each select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='feed']">
            <a class="list-group-item">
                ...
                <img src="{concat($context-path, '/static/icons/feed.png')}" alt="feed" class="btn-xs"/>
    ...
    <xsl:template match="dri:options//dri:item[dri:xref]">
        <xsl:choose>
            <!-- Carthage change /login link to /ldap-login -->
            <xsl:when test="./dri:xref[@target='/login'] or ./dri:xref[@target='/xmlui/login']">
                <a href="/ldap-login">
                    <xsl:call-template name="standardAttributes">
                        <xsl:with-param name="class">list-group-item ds-option tweak</xsl:with-param>
                    </xsl:call-template>
                    <xsl:choose>
            ...
            <xsl:otherwise>
                <a href="{dri:xref/@target}">
                    <xsl:call-template name="standardAttributes">
                        <xsl:with-param name="class">list-group-item ds-option notchanged</xsl:with-param>
                    </xsl:call-template>
                    <xsl:choose>
                        <xsl:when test="dri:xref/node()">
                            <xsl:apply-templates select="dri:xref/node()"/>
