...
<xsl:stylesheet
    ...
    <xsl:template name="itemSummaryView-DIM-fields">
      ...
      <xsl:choose>
          ...
          <!-- SAYLOR custom use rights not URI row -->
          <xsl:when test="$clause = 3 and (dim:field[@element='rights' and not(@qualifier)])">
                    <div class="simple-item-view-other">
                        <span class="bold">License Type:</span>
                        <span>
          ...
          <xsl:when test="$clause = 4 and (dim:field[@element='source'])">
                    <div class="simple-item-view-other">
	                <span class="bold">Source:</span>
	                <span>
	                	<xsl:for-each select="dim:field[@element='source']">
		                    <a>
		                        <xsl:attribute name="href">
		                            <xsl:copy-of select="./node()"/>
		                        </xsl:attribute>
		                	<xsl:copy-of select="substring(./node(),1,90)"/>
		                    </a>
		                    <xsl:if test="count(following-sibling::dim:field[@element='source']) != 0">
		                    	<br/>
		                    </xsl:if>
	                    </xsl:for-each>
	                </span>
