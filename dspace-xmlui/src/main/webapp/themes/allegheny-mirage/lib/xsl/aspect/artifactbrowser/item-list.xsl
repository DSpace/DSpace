...
<xsl:stylesheet
    ...
    <!-- Allegheny customizations -->
    <xsl:import href="../../date.month-name.template.xsl"/>
    <xsl:import href="../../date.day-in-month.template.xsl"/>
    ...
    <xsl:template name="itemSummaryList-DIM">
        <xsl:param name="allegheny-date">
            <xsl:choose>
                <xsl:when test="dim:field[@element='comp' and @qualifier='date']">
                    <xsl:value-of select="dim:field[@element='comp' and @qualifier='date']/node()"/>
                </xsl:when>
                ...
                <xsl:when test="dim:field[@element='date' and @qualifier='submitted']">
                    <xsl:value-of select="dim:field[@element='date' and @qualifier='submitted']/node()"/>
                </xsl:when>
                <xsl:otherwise><xsl:value-of select="dim:field[@element='date' and @qualifier='issued']/node()"/></xsl:otherwise>
            </xsl:choose>
        ...
        <xsl:param name="longsight-month">
            <xsl:call-template name="month-name">
                <xsl:with-param name="date-time" select="$allegheny-date"/>
            </xsl:call-template>
        </xsl:param>
            ...
            <div class="artifact-info">
                <span class="author">
                    <xsl:choose>
                        ...
                        <xsl:when test="dim:field[@element='comp'][@qualifier='compauthor']">
                            <xsl:for-each select="dim:field[@element='comp'][@qualifier='compauthor']">
                                <xsl:copy-of select="node()"/>
                                <xsl:if test="count(following-sibling::dim:field[@element='comp'][@qualifier='compauthor']) != 0">
                                    <xsl:text>; </xsl:text>
                                </xsl:if>
                            </xsl:for-each>
                        </xsl:when>
