...
public class CurrentActivityAction extends AbstractAction
{
    ...
    /** The HTTP header that contains the real IP for this request, this is used
     * when DSpace is placed behind a load balancer */
    ...
    private static boolean recordBotEvents = DSpaceServicesFactory.getInstance()
        .getConfigurationService().getBooleanProperty(
            "currentActivityAction.recordBotEvents", false);
    ...
    public static class Event
    {
        ...
        public String getDectectedBrowser()
        {
            ...
            return userAgent.length() == 0
                ? "Unknown"
                : escapeHtml(userAgent.substring(
                    0, Math.min(20, userAgent.length()) ) );
        }
    }
}
