...
public class FlowItemUtils
{
        ...
        throws SQLException, AuthorizeException, UIException, IOException
    {
        ...
        if ("*".equals(scope))
        {
            itemService.clearMetadata(
                context, item, Item.ANY, Item.ANY, Item.ANY, Item.ANY);
        }
        else
        {
            String[] parts = parseName(scope);
            itemService.clearMetadata(
                context, item, parts[0], parts[1], parts[2], Item.ANY);
        }
        ...
        for (Integer index=1; index <= indexes.size(); ++index)
        {
            ...
            if (authority != null && authority.length() > 0 &&
                iconf == Choices.CF_UNSET)
            {
                iconf = Choices.CF_NOVALUE;
            }
            itemService.addMetadata(context, item, parts[0], parts[1], parts[2],
                lang, value, authority, iconf);
        }
            ...
            throws SQLException, AuthorizeException, UIException, IOException
    {
        ...
        itemService.addMetadata(
            context, item, field.getMetadataSchema().getName(),
            field.getElement(), field.getQualifier(), language, value);
        ...
        throws SQLException, AuthorizeException
    {
        ...
        try
        {
            startDate = DateUtils.parseDate(
                request.getParameter("embargo_until_date"),
        ...
        authorizeService.generateAutomaticPolicies(context, startDate, reason,
            b, collection);
    }
     ...
     * @throws org.dspace.authorize.AuthorizeException passed through.
     */
    public static FlowResult processEditBitstream(Context context, UUID itemID,
        UUID bitstreamID, String bitstreamName, String primary,
        String description, int formatID, String userFormat, Request request)
        throws SQLException, AuthorizeException
    {
        FlowResult result = new FlowResult();
        result.setContinue(false);
