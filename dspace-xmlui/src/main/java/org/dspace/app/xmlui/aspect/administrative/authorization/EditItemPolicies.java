...
public class EditItemPolicies extends AbstractDSpaceTransformer
{
        ...
        throws WingException, SQLException
    {
        ...
        ArrayList<ResourcePolicy> itemPolicies =
            (ArrayList<ResourcePolicy>) authorizeService.getPolicies(context, item);

        // DIVISION: main
        Division main = body.addInteractiveDivision(
            "edit-item-policies", contextPath + "/admin/authorize",
    ...
    private void rowBuilder(String baseURL, Table table, java.util.List<ResourcePolicy> policies, UUID objectID, int objectType, int highlightID)
        throws WingException, SQLException
    {
        ...
        else
        {
            for (ResourcePolicy policy : policies)
            {
                ...
                row.addCell().addXref(baseURL + "&submit_edit&policy_id="
                    + policy.getID() + "&object_id=" + objectID + "&object_type="
                    + objectType, String.valueOf(policy.getID()));
                ...
                row.addCell().addXref(baseURL + "&submit_edit&policy_id="
                    + policy.getID() + "&object_id=" + objectID + "&object_type="
                    + objectType, resourcePolicyService.getActionText(policy));
                if (policyGroup != null)
                {
                    ...
                    groupHigh.addXref(baseURL + "&submit_edit_group&group_id="
                        + policyGroup.getID(), T_group_edit);
                    groupHigh.addContent("]");
                }
                else
                {
                ...
                {
                    row.addCell().addContent(DateFormatUtils.format(
                        policy.getStartDate(), "yyyy-MM-dd"));
                }
                else
                {
