 ...
 * Time: 10:34
 */
public class ExternalImportResult extends AbstractDSpaceTransformer {
    ...
    private static final Message T_submit = message("xmlui.administrative.importer.external.external-import-result.submit");
    private static final Message T_submit_next = message("xmlui.administrative.importer.external.external-import-result.submit-next");
    private static final Message T_no_fields_error = message("xmlui.administrative.importer.external.external-import-result.no-fields-error");
    private static final Message T_records_found = message("xmlui.administrative.importer.external.external-import-result.records-found");
    private static final Message T_no_records_found = message("xmlui.administrative.importer.external.external-import-result.no-records-found");
    private static final Message T_records_selected = message("xmlui.administrative.importer.external.external-import-result.records-selected");
    ...
    Logger log = Logger.getLogger(ExternalImportResult.class);
    ...
    public void addBody(Body body) throws SAXException, WingException,
        ...
        if (buttonPressed.equals(NEXT_BUTTON)) {
            ((HttpServletResponse) objectModel.get(HttpEnvironment.HTTP_RESPONSE_OBJECT)).sendRedirect(request.getContextPath() + "/admin/external-import/selected");
        }

        Division div = body.addInteractiveDivision("live-import-result", contextPath + "/admin/external-import/result", Division.METHOD_POST, "");
        div.setHead(T_head);
