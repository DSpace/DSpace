...
public class ItemAdapter extends AbstractAdapter {
	...
	private MetadataExposureService metadataExposureService = UtilServiceFactory.getInstance()
			.getMetadataExposureService();
	private AuthorityValueService authorityValueService = AuthorityServiceFactory.getInstance()
	...
	protected void renderDescriptiveSection()
		...
		if (dmdTypes.size() == 0 || dmdTypes.contains("DIM")) {
			...
			for (MetadataValue dcv : dcvs) {
				...
				String schema = dcv.getMetadataField().getMetadataSchema().getName();
				String element = dcv.getMetadataField().getElement();
				String qualifier = dcv.getMetadataField().getQualifier();
				String language = dcv.getLanguage();
				String authority = dcv.getAuthority();
				int confidence = dcv.getConfidence();
				...
				if (!metadataExposureService.isHidden(context, schema, element, qualifier)) {
					...
					if (authority != null || confidence != Choices.CF_UNSET) {
						attributes.put("authority", authority);
						attributes.put("confidence", Choices.getConfidenceText(confidence));

						AuthorityValue av = authorityValueService.findByUID(context, authority);
	 ...
	 */
	protected void renderAmdSubSection(String amdSecName, String mdType, DisseminationCrosswalk crosswalk,
			DSpaceObject dso) throws WingException, SAXException, CrosswalkException, IOException, SQLException {
		/////////////////////////////////
		...
		for (Bundle bundle : bundles) {
			...
			for (Bitstream bitstream : bundle.getBitstreams()) {
				...
				if (isContentBundle) {
					...
					if (bundle.getPrimaryBitstream().getID() == bitstream.getID()) {
	 ...
	 * user.
	 *
	 * Example: <structMap TYPE="LOGICAL" LABEL="DSpace">
	 * <div TYPE="DSpace Item" DMDID="space-separated list of ids">
	 * <fptr FILEID="primary bitstream"/> ... a div for each content bitstream.
	 * </div> </structMap>
	 */
	...
	protected static Bitstream findOriginalBitstream(Item item, Bitstream derived) throws SQLException {
		...
		for (int i = 0; i < bundles.size(); i++) {
			if ((bundles.get(i).getName() != null) && bundles.get(i).getName().equals("ORIGINAL")) {
				// Now find the corresponding bitstream
				List<Bitstream> bitstreams = bundles.get(i).getBitstreams();

				for (int bsnum = 0; bsnum < bitstreams.size(); bsnum++) {
					if (bitstreams.get(bsnum).getName().equals(originalFilename)) {
						return bitstreams.get(bsnum);
					}
				}
			}
		}
	...
	protected void renderFileWithAllowed(Item item, Bitstream bitstream, String fileID, String groupID, String admID)
			throws SAXException {
		AttributeMap attributes;
