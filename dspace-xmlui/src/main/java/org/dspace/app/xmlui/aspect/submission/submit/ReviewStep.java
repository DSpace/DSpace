 ...
 * <P>
 * This step is dynamic, since when using the Configurable Submission it is
 * unknown what steps are available and in what order.
 * <P>
 * This step builds a form with which consists of a separate section for each
 * step which implements the "addReviewSection()" method of
 * AbstractSubmissionStep class.
 *
...
public class ReviewStep extends AbstractSubmissionStep
{
    ...
    /**
     * Save these setup parameters, to use for loading up the previous step's
     * review information
     */
    @Override
            ...
            UIException, SQLException, IOException, AuthorizeException
    {
        ...
        String[] submissionPages = (String[]) submissionPagesSet
                .toArray(new String[submissionPagesSet.size()]);
        ...
        for (int i = 0; i < submissionPages.length; i++)
        {
            ...
            SubmissionStepConfig stepConfig = subConfig
                    .getStep(currentStepAndPage.getStep());

            // load the step's XML-UI Class
            AbstractStep stepUIClass = loadXMLUIClass(stepConfig
                    .getXMLUIClassName());
            ...
            catch (Exception e)
            {
                throw new UIException(
                        "Unable to initialize AbstractStep identified by "
            ...
            if (stepUIClass instanceof AbstractSubmissionStep)
            {
                ...
                List stepSection = ((AbstractSubmissionStep) stepUIClass)
                        .addReviewSection(review);

                // as long as this step has something to review
                if (stepSection != null)
                {
                    // add a Jump To button for this section
                    addJumpButton(stepSection, T_submit_jump,
                            currentStepAndPage);
                }
            }
            else
            {
                ...
                log.warn("The Step represented by "
                        + stepConfig.getXMLUIClassName()
    ...
    /**
     * Loads the specified XML-UI class which will generate the review
     * information for a given step
     *
     * @return AbstractStep which references the XML-UI Class
     */
            ...
            throws UIException
    {
        ...
        catch (ClassNotFoundException cnfe)
        {
            ...
            throw new UIException("Class Not Found: " + transformerClassName,
                    cnfe);
        }
        catch (Exception e)
        {
            ...
            throw new UIException(
                    "Unable to instantiate class "
                            ...
                            + ". "
                            + "Please make sure it extends org.dspace.app.xmlui.submission.AbstractSubmissionStep!",
                    e);
        }
    }
