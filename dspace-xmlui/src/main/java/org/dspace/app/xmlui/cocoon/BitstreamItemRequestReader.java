...
public class BitstreamItemRequestReader extends AbstractReader implements Recyclable
{
    private static Logger log = Logger.getLogger(BitstreamItemRequestReader.class);
    ...
    protected boolean isAnonymouslyReadable=false;
            ...
            IOException
    {
        ...
        try
        {
            ...
            String tokenDescarga = parameters.getParameter("tokenDescarga","");
            TableRow requestItem = DatabaseManager.findByUnique(context, "requestitem", "token_descarga", tokenDescarga);
            int numMaximoDescargas = ConfigurationManager.getIntProperty("request.item.num.usos",0);
            int horasExpiracionEnlace = ConfigurationManager.getIntProperty("request.item.hours.expired",0);
            
            if(requestItem==null){
            	throw new ResourceNotFoundException("El enlace de descarga no existe.");
            }
            int descargasActuales=0;
            descargasActuales=Math.max(0, requestItem.getIntColumn("num_usos")) ;
            Date fechaAceptacion=requestItem.getDateColumn("decision_date");
            Date fechaCaducidad=DateUtils.addHours(fechaAceptacion, horasExpiracionEnlace);
            		
            // Caducado
            if (fechaCaducidad.before(new Date()))
            {
                throw new ResourceNotFoundException("El enlace de descarga ha caducado.");
            }

            // Limite de descargas Alcanzado
            if (descargasActuales >=numMaximoDescargas)
            {
                throw new ResourceNotFoundException("Se ha alcanzado el numero maximo de descargas con ese enlace.");
            }
            
            Bitstream bitstream = null;
            if(requestItem!=null){
            	bitstream = Bitstream.find(context,requestItem.getIntColumn("bitstream_id"));
            }
            ...
            // Si lo encontramos añadimos una descarga a la bbdd
            requestItem.setColumn("num_usos", (descargasActuales+1));
