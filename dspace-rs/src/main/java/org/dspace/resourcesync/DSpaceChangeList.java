...
public class DSpaceChangeList extends DSpaceResourceDocument {
	...
	public DSpaceChangeList(Context context, Date from, Date to, UrlManager um) {
		super(context);
		this.includeRestricted = new DSpace().getConfigurationService().getBooleanProperty("resourcesync.changelist.include-restricted");
		this.from = from;
		this.to = to;
		this.um = um;
	}
...

	{
	 ...
	 protected URL addResources(ResourceSyncEvent rse, ResourceSyncDocument rl,String format)
	    ...
	    {
		 	URL bs = new URL();
		 	String resourceSyncDir = new DSpace().getConfigurationService().getProperty("resourcesync.base-url");
