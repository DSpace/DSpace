...
public class WorkflowManagerDefault implements WorkflowManager {
    ...
    public void replaceResourceContent(Context context, Item item)
        ...
        if (wft.isItemInWorkflow(context, item)) {
            throw new SwordError(UriRegistry.ERROR_METHOD_NOT_ALLOWED,
                                 "The item has already been injected into the review workflow, and can no longer be " +
                                     "modified");
        }
    }
    ...
    public void resolveState(Context context, Deposit deposit,
        ...
        // 0                0       0       1           NOTHING     the deposit finished, and the item is in the
        // archive;
        // 0                0       1       0           NOTHING     the deposit finished, and the item is in the
        // workflow.  Carry on as normal
        // 0                1       0       0           START WF    the deposit is finished, and the item is in the
        // workflow, so we start it
        // 1                0       0       1           NOTHING     the deposit is not finished, and the item is in
        // the archive;
        // 1                0       1       0           STOP WF     the deposit is not finished, and it is in the
        // workflow.  Pull it out into the workspace
        // 1                1       0       0           NOTHING     the deposit is not finished, and is in the
        // workspace; leave it there
