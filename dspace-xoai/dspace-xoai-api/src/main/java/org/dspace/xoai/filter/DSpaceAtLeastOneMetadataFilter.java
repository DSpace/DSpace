...
public class DSpaceAtLeastOneMetadataFilter extends DSpaceFilter {
    ...
    private DSpaceMetadataFilterOperator getOperator () {
        if (_operator == DSpaceMetadataFilterOperator.UNDEF) {
            _operator = DSpaceMetadataFilterOperator.valueOf(super.getParameter("operator").toUpperCase());
        }
        return _operator;
    }
    ...
    public DatabaseFilterResult getWhere(Context context) {
        if (this.getField() != null) {
            try {
                int id = MetadataFieldManager.getFieldID(context, this.getField());
    ...
    public boolean isShown(DSpaceItem item) {
        if (this.getField() == null) return true;
        List<String> values = item.getMetadata(this.getField());
        for (String praticValue : values) {
            for (String teoricValue : this.getValues()) {
                switch (this.getOperator()) {
                    case CONTAINS:
                        if (praticValue.contains(teoricValue)) return true;
                        break;
                    case ENDS_WITH:
                        if (praticValue.endsWith(teoricValue)) return true;
                        break;
                    case EQUAL:
                        if (praticValue.equals(teoricValue)) return true;
                        break;
                    case GREATER:
                        if (praticValue.compareTo(teoricValue) > 0) return true;
                        break;
    ...
    private DatabaseFilterResult getWhere(int mdid, List<String> values) {
        ...
        if (parts.size() > 0) {
            String query = "EXISTS (SELECT tmp.* FROM metadatavalue tmp WHERE tmp.item_id=i.item_id AND tmp.metadata_field_id=?"+
                    " AND (" + StringUtils.join(parts.iterator(), " OR ") + "))";
