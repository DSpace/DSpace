...
public class DSpaceItem extends AbstractItem {
	...
	private static Element.Field createValue(ObjectFactory factory, String name, String value) {
		Element.Field e = factory.createElementField();
		e.setValue(value);
		e.setName(name);
		return e;
		...
		if (metadata == null) {
			...
			for (DCValue val : vals) {
				...
				if (val.language != null && !val.language.equals("")) {
					Element language = getElement(valueElem.getElement(), val.language);
					if (language == null) {
			...
			try {
				...
				for (Bundle b : bs) {
					...
					bundle.getField().add(createValue(factory, "name", b.getName()));
					...
					for (Bitstream bit : bits) {
						...
						if (name != null) bitstream.getField().add(createValue(factory, "name", name));
						if (oname != null) bitstream.getField().add(createValue(factory, "originalName", name));
						bitstream.getField().add(createValue(factory, "format", bit.getFormat().getMIMEType()));
						bitstream.getField().add(createValue(factory, "size", ""+bit.getSize()));
						bitstream.getField().add(createValue(factory, "url", url));
						bitstream.getField().add(createValue(factory, "checksum", cks));
						bitstream.getField().add(createValue(factory, "checksumAlgorithm", cka));
						bitstream.getField().add(createValue(factory, "sid", bit.getSequenceID()+""));
					}
				}
				...
				repository.getField().add(createValue(factory, "name", ConfigurationManager.getProperty("dspace.name")));
				repository.getField().add(createValue(factory, "mail", ConfigurationManager.getProperty("mail.admin")));
				metadata.getElement().add(repository);
				
				// Licensing info
				Element license = create(factory, "license");
				Bundle[] licBundles = item.getBundles(Constants.LICENSE_BUNDLE_NAME);
				if (licBundles.length > 0) {
					...
					if (licBits.length > 0) {
						...
						try {
							...
							license.getField().add(createValue(factory, "bin", Base64Utils.encode(out.toString())));
				...
				other.getField().add(createValue(factory, "handle", item.getHandle()));
				other.getField().add(createValue(factory, "lastModifyDate", item.getLastModified().toString()));
			...
			if (parts[0].equals("*")) {
				for (Element e : elems) {
					if (e.getElement() != null)
						list.addAll(this.getMetadata(e.getElement(), Arrays.copyOfRange(parts, 1, parts.length)));
				}
			} else {
