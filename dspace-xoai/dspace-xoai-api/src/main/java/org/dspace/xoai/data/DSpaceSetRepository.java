...
public class DSpaceSetRepository extends AbstractSetRepository {
	...
	private int getCommunityCount () {
		...
		try {
			...
			if (iterator.hasNext()) return (int) iterator.next().getLongColumn("count");
	...
	private List<Set> community (int offset, int length) {
		...
		String query = "SELECT community_id, name, handle FROM community c, handle h WHERE h.resource_id=community_id AND h.resource_type_id=? ORDER BY community_id OFFSET ? LIMIT ?";
		try {
			TableRowIterator iterator = DatabaseManager.query(_context, query, Constants.COMMUNITY, offset, length);
			int i = 0;
	...
	private List<Set> collection (int offset, int length) {
		List<Set> array = new ArrayList<Set>();
		String query = "SELECT collection_id, name, handle FROM collection c, handle h WHERE h.resource_id=collection_id AND h.resource_type_id=? ORDER BY collection_id OFFSET ? LIMIT ?";
		try {
			TableRowIterator iterator = DatabaseManager.query(_context, query, Constants.COLLECTION, offset, length);
			int i = 0;
			while (iterator.hasNext() && i < length) {
				TableRow row = iterator.next();
