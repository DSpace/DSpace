   ...
   <xsl:template match="dri:referenceSet" mode="multiple_column_browse_community">
	 <!-- Recupero la cantidad de filas a mostrar en la pagina -->
      <xsl:variable name="itemsTotal">
		<xsl:value-of select="count(dri:reference/dri:referenceSet/dri:reference)"/>
     </xsl:variable>
     
      <!-- Calculo la cantidad de columnas y filas por columna -->
	 <xsl:variable name="cantItemsXCol">
          <xsl:choose>
	          <xsl:when test="$itemsTotal>2">
         ...
         <xsl:param name="cantItemsXCol"/>
          <div class="browse_column">
		     <xsl:call-template name="header_browse_columns_community">
		     	<xsl:with-param name="header"><xsl:value-of select="dri:reference/dri:referenceSet"/></xsl:with-param>
		     </xsl:call-template>
	       <ul>
             <xsl:apply-templates select="dri:reference/dri:referenceSet/dri:reference[floor((position()-1) div ($cantItemsXCol)) = $contador]"  mode="li_browse_columns"/>
		  </ul>
	     </div>
 ...
 <xsl:template match="mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim" mode="nameCollection">
			    <div class="artifact-description">
					...
					<div class="artifact-info">
						<span class="short-description"> <xsl:value-of select="dim:field[@element='description']"/></span>
					</div>
				</div>
