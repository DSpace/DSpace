    ...
    var collID = args.collection == null ? -1 : args.collection;
        ...
        ac.autocomplete({
            ...
            source: function(request, response) {
                ...
                DSpaceUpdateConfidence(document, args.confidenceIndicatorID,'ambiguous');
                ...
                $.get(reqUrl, function(xmldata) {
                    ...
                    if($(xmldata).find('Choice').length>0){
                		...
                		if (!isClosed){
                        	DSpaceUpdateConfidence(document, args.confidenceIndicatorID, 'notfound');
                        	if (isAuthorityControlled){
	                        	//Como no es cerrado debo cambiar el valor de la confianza, ya que lo ingresado es un value diferente para el mismo authority key
	                        	cambiarAuthorityConfidence(args.confidenceName, 'notaccepted');
                        	}
                        } else {
                        	if (isAuthorityControlled){
    	                    	//como no hay match para la entrada y es cerrado debo vaciar las authority
    	                		vaciarAuthority(authorityID, args.confidenceIndicatorID, args.confidenceName)
                        	}
	                    	//Como es cerrado pongo vacio el input 
	                    	alert('No hay concordancias para la busqueda')
	                    	$('#'+inputID).val("");
            ...
            search: function(event, ui) { 
                	...
                	if ($('#'+inputID).val().length==0){
                		...
                		if (isAuthorityControlled){
                			...
                			vaciarAuthority(authorityID, args.confidenceIndicatorID, args.confidenceName);
    	            	    if (!isClosed){
    	            	    	//si no es cerrado debo vaciar el authority label
            ...
            close: function(event, ui) {
            	if (!seleccion){
	            	if (isClosed){
	            		...
	            		//como no es cerrado, dejo todo como esta
	            		DSpaceUpdateConfidence(document, args.confidenceIndicatorID, 'notfound');
	                	if (isAuthorityControlled){
		            		//Como no es cerrado debo cambiar el valor de la confianza, ya que lo ingresado es un value diferente para el mismo authority key
		                	cambiarAuthorityConfidence(args.confidenceName, 'notaccepted');	                	
            ...
            select: function(event, ui) {
                ...
                //aca es lo mismo que preguntar si es authority controlled
                if (authInput != null) {
                    ...
                    cambiarAuthorityConfidence(args.confidenceName, 'accepted');
                }
                // make indicator blank if no authority value
                //aca siempre va a quedar accepted
...
//--------------------- Funcion para vaciar los input extras de authorities
