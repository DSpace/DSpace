...
<% if (availableFilters.size() > 0) { %>
		...
		<% if (appliedFilterQueries.size() > 0 ) { 
				...
				for (String[] filter : appliedFilters)
				{
				    boolean found = false;
				    %>
				    <input type="hidden" id="filter_field_<%=idx %>" name="filter_field_<%=idx %>" value="<%= Utils.addEntitiesfilter[0]) %>" />
...
if (qResults==null){
	...
	<p align="center" class="submitFormWarn">
		<fmt:message key="jsp.search.emptyResults.discovery" />
	</p>
	<%
}
else if( error )
...
<%
	...
	if ((brefine)&&(!sidebarError)) {
	    System.out.println("brefine= " +brefine);
%>
