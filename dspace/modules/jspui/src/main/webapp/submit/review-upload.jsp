...
<%
	...
	SubmissionInfo subInfo = SubmissionController.getSubmissionInfo(
			context, request);

	boolean advanced = ConfigurationManager.getBooleanProperty(
	...
	Object isUploadWithEmbargoB = request
			.getAttribute("submission.step.uploadwithembargo");
...
<div class="col-md-9">
	<div class="row">
		<span class="metadataFieldLabel col-md-2"><%=(subInfo.getSubmissionItem().hasMultipleFiles() ? LocaleSupport
					.getLocalizedMessage(pageContext,
							"jsp.submit.review.upload1") : LocaleSupport
					.getLocalizedMessage(pageContext,
							"jsp.submit.review.upload2"))%></span>
 	...
 	if (bitstreams.length > 0) {
 		for (int i = 0; i < bitstreams.length; i++) {
 			...
 			if (format != null
 					&& format.getMIMEType().equals("text/html")) {
 				downloadLink = "html/db-id/" + item.getID();
 			}
 %>  <%
 	if (isUploadWithEmbargo) {
 				List<ResourcePolicy> rpolicies = AuthorizeManager
 						.findPoliciesByDSOAndType(context,
 								bitstreams[i],
 ...
 %> <%
 	if (countPolicies > 0) {
 %> <i class="label label-info"><fmt:message
					key="jsp.submit.review.policies.founded">
					<fmt:param><%=countPolicies%></fmt:param>
				</fmt:message></i> <%
 	}
 %> <%
 	} else {
 %> <%
 	for (ResourcePolicy rpolicy : rpolicies) {
 							Date policyStartDate = rpolicy
 									.getStartDate();

 							if (policyStartDate != null) {
 								if (policyStartDate
 										.equals(EmbargoOption.RESTRICTED
 												.getAssociatedDate())) {
 %> <i class="label label-important pull-left"><fmt:message
					key="jsp.submit.review.policies.restricted"></fmt:message></i> <%
 	} else {
 %> <i class="label label-info pull-left"><fmt:message
					key="jsp.submit.review.policies.embargoed">
					<fmt:param><%=policyStartDate%></fmt:param>
				</fmt:message></i> <%
 	}
 							} else {
 %> <i class="label label-success pull-left"><fmt:message
					key="jsp.submit.review.policies.openaccess" /></i> <%
 	}
 						}
 %> <%
 	}
 ...
 %> <a
			href="<%=request.getContextPath()%>/<%=downloadLink%>/<%=UIUtil.encodeBitstreamName(bitstreams[i]
							.getName())%>"
			target="_blank"><%=bitstreams[i].getName()%></a> - <%=bitstreams[i].getFormatDescription()%>
			<%
...
<div class="col-md-3">
	...
	<input class="btn btn-default" type="submit"
		...
		value="<%=(subInfo.getSubmissionItem().hasMultipleFiles() ? LocaleSupport
						.getLocalizedMessage(pageContext,
								"jsp.submit.review.button.upload1")
						: LocaleSupport.getLocalizedMessage(pageContext,
								"jsp.submit.review.button.upload2"))%>" />
	<%
		} else {
	%>
