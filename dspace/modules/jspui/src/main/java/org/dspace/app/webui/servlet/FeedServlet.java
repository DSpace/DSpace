...
/**
 * Servlet for handling requests for a syndication feed. The Handle of the
 * collection or community is extracted from the URL, e.g:
 * <code>/feed/rss_1.0/1234/5678</code>. Currently supports only RSS feed
 * formats.
 * 
...
public class FeedServlet extends DSpaceServlet
{
    ...
    private final String clazz = "org.dspace.app.webui.servlet.FeedServlet";
    ...
    static
    {
        ...
        if (enabled)
        {
            String fmtsStr = ConfigurationManager
            ...
            cacheSize = ConfigurationManager
                    .getIntProperty("webui.feed.cache.size");
            if (cacheSize > 0)
            {
                feedCache = new HashMap<String, CacheFeed>();
                cacheAge = ConfigurationManager
                        .getIntProperty("webui.feed.cache.age");
            }
        }
    }
    ...
    {
        includeAll = ConfigurationManager.getBooleanProperty(
        ...
        labelMap.put(SyndicationFeed.MSG_UNTITLED,
                msgs.getString(clazz + ".notitle"));
        labelMap.put(SyndicationFeed.MSG_LOGO_TITLE,
                msgs.getString(clazz + ".logo.title"));
        labelMap.put(SyndicationFeed.MSG_FEED_DESCRIPTION,
                msgs.getString(clazz + ".general-feed.description"));
        ...
        for (String selector : SyndicationFeed.getDescriptionSelectors())
        {
            labelMap.put("metadata." + selector,
                    msgs.getString(SyndicationFeed.MSG_METADATA + selector));
        }
        ...
        if (handle != null && !handle.equals(SITE_FEED_KEY))
        {
            ...
            if (dso == null)
            {
                log.info(LogManager.getHeader(context, "invalid_handle",
                        "path=" + path));
        ...
        if (!enabled
                || (dso != null && (dso.getType() != Constants.COLLECTION && dso
        ...
        {
            log.info(LogManager.getHeader(context, "invalid_syndformat",
                    "path=" + path));
            JSPManager.showInvalidIDError(request, response, path, -1);
            return;
        }
        ...
        {
            labelMap.put(SyndicationFeed.MSG_FEED_TITLE, MessageFormat.format(
                    msgs.getString(clazz + ".feed.title"),
                    new Object[] {
                            msgs.getString(clazz + ".feed-type.collection"),
                            ((Collection) dso).getMetadata("name") }));
        }
        else if (dso != null && dso.getType() == Constants.COMMUNITY)
        {
            labelMap.put(SyndicationFeed.MSG_FEED_TITLE, MessageFormat.format(
                    ...
                    new Object[] {
                            ...
                            ((Community) dso).getMetadata("name") }));
        }

        // Lookup or generate the feed
        // Cache key is handle + locale
        String cacheKey = (handle == null ? "site" : handle) + "."
                + locale.toString();
        ...
        if (feedCache != null)
        {
            ...
            if (cFeed != null) // cache hit, but...
            {
                ...
                if (cFeed.timeStamp + (cacheAge * HOUR_MSECS) < System
                        .currentTimeMillis())
                {
                    cacheFeedCurrent = true;
                }
                // Not current, but have any items changed since feed was
            ...
            long timeStamp) throws SQLException
    {
        ...
        // this invocation should return a non-empty list if even 1 item has
        // changed
        try
        {
            return (Harvest.harvest(context, dso, startDate, endDate, 0, 1,
                    !includeAll, false, false, includeAll).size() > 0);
        }
        catch (ParseException pe)
        {
            // This should never get thrown as we have generated the dates
            // ourselves
            return false;
        }
    }
            ...
            throws IOException, SQLException
    {
        try
        {
            ...
            String idx = ConfigurationManager
                    .getProperty("recent.submissions.sort-option");
            if (idx == null)
            {
            ...
            {
                throw new IOException(
                        "There is no browse index with the name: " + idx);
            }
            ...
            else
            {
                ...
                // AuthorizeManager.getAuthorizedGroups(context, results[i],
                // Constants.READ);
                // boolean added = false;
                List<Item> items = new ArrayList<Item>();
                for (Item result : results)
                {
                    checkAccess: for (Group group : AuthorizeManager
                            .getAuthorizedGroups(context, result,
                                    Constants.READ))
                    {
                        if ((group.getID() == 0))
                        {
    ...
    /**
     * Add a feed to the cache - reducing the size of the cache by 1 to make
     * room if necessary. The removed entry has an access count equal to the
     * minumum in the cache.
     * 
     * @param feedKey
    ...
    private static void cache(String feedKey, CacheFeed newFeed)
    {
        ...
        if (feedCache.size() >= cacheSize)
        {
            ...
            String logMsg = "feedCache() - size: " + feedCache.size()
                    + " Hits - total: " + total + " avg: " + avg + " max: "
                    + maxFeed.hits + " min: " + minFeed.hits;
            log.info(logMsg);
