...
import org.dspace.app.webui.simulation.StatisticsSimulator;
...
public class DisplayStatisticsServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        boolean isSimulated = ConfigurationManager.getBooleanProperty("dspace.stats.simulation.active");
	...
	try
        ...
        request.setAttribute("statsVisits", isSimulated ? StatisticsSimulator.simulateVisitStats() : statsVisits);
        request.setAttribute("statsMonthlyVisits", isSimulated ? StatisticsSimulator.simulateMonthlyVisits() : statsMonthlyVisits);
        request.setAttribute("statsFileDownloads", isSimulated ? StatisticsSimulator.simulateFileDownloads() : statsFileDownloads);
        request.setAttribute("statsCountryVisits", isSimulated ? StatisticsSimulator.simulateCountryVisits() : statsCountryVisits);
        request.setAttribute("statsCityVisits", isSimulated ? StatisticsSimulator.simulateCityVisits() : statsCityVisits);
        ...
        if(isItem)
        {
			...
			DCValue[] metadata = item.getMetadata("dc.title");
        	if(metadata != null && metadata.length > 0)
        	{
        		request.setAttribute("statsTitle", metadata[0].value);
        	}
        }
        else
        {
        	request.setAttribute("statsTitle", dso.getName());
        }
        
        request.setAttribute("pageHandle", dso.getHandle());
