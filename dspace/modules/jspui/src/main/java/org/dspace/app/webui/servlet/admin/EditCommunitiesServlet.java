...
public class EditCommunitiesServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        case START_CREATE_COMMUNITY:
            // no authorize attribute will be given to the jsp so a "clean"
            // creation form
            // will be always supplied, advanced setting on policies and admin
            // group creation
    ...
    private void storeAuthorizeAttributeCommunityEdit(Context context,
            HttpServletRequest request, Community community)
            throws SQLException
    {
        try
        {
    ...
    static void storeAuthorizeAttributeCollectionEdit(Context context,
            HttpServletRequest request, Collection collection)
            throws SQLException
    {
            ...
            SQLException, AuthorizeException
    {
        ...
        {
            response.sendRedirect(response.encodeRedirectURL(request
                    .getContextPath() + "/handle/" + collection.getHandle()));
        }
        else if (community != null)
        {
            response.sendRedirect(response.encodeRedirectURL(request
                    .getContextPath() + "/handle/" + community.getHandle()));
        }
        else
        {
            ...
            AuthorizeException
    {
        ...
        // harvest type to 0; wipe harvest settings
        ...
        else if (button.equals("submit_create_template"))
        {
            ...
            context.complete();
            response.sendRedirect(response.encodeRedirectURL(request
                    .getContextPath() + "/tools/edit-item?item_id=" + i.getID()));
            ...
            SQLException, AuthorizeException
    {
        try
        {
            ...
            if (collection == null)
            {
                ...
                request.setAttribute("community", community);
                storeAuthorizeAttributeCommunityEdit(context, request,
                        community);
                dso = community;
            ...
            else
            {
                ...
                request.setAttribute("collection", collection);
                request.setAttribute("community", community);
                storeAuthorizeAttributeCollectionEdit(context, request,
                        collection);
                dso = collection;
