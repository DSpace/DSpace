...
public class Item extends DSpaceObject
{
     ...
     * @throws SQLException
     */
    public static ItemIterator findAllNoRevisionesNiJuicios(Context context) throws SQLException
    {
        String myQuery = "select item.* from item left JOIN revision_token ON (item.item_id = to_number(revision_token.revision_id,'99999999999')) where in_archive='1' and revision_token_id is null ";
...
//               Remove from indices
    ...
    public static ItemIterator findNoRevisionesNiJuiciosByAuthorityValue(Context context, String schema, String element, String qualifier, String value)
        throws SQLException, AuthorizeException, IOException
    {
        MetadataSchema mds = MetadataSchema.find(context, schema);
        if (mds == null)
        ...
        TableRowIterator rows = DatabaseManager.queryTable(context, "item",
            "SELECT item.* FROM metadatavalue,item left JOIN revision_token ON (item.item_id = to_number(revision_token.revision_id,'99999999999')) where revision_token_id is null and item.in_archive='1' "+
            "AND item.item_id = metadatavalue.resource_id AND metadata_field_id = ? AND authority = ? AND resource_type_id = ?",
            mdf.getFieldID(), value, Constants.ITEM);
        return new ItemIterator(context, rows);
    }
