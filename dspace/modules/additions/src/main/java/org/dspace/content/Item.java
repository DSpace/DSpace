...
public class Item extends DSpaceObject
{
    ...
    /**
     * Get all "final" items in the archive, both archived ("in archive" flag)
     * or withdrawn items are included. The order of the list is indeterminate.
     * 
     * @param context
     *            DSpace context object
     * @return an iterator over the items in the archive.
     * @throws SQLException
     */
    public static ItemIterator findAllUnfiltered(Context context)
            throws SQLException
    {
    ...
    public void updateLastModified()
    {
        try
        {
            ...
            DatabaseManager.updateQuery(ourContext,
                    "UPDATE item SET last_modified = ? WHERE item_id= ? ",
                    lastModified, getID());
            // Also fire a modified event since the item HAS been modified
            ourContext.addEvent(new Event(Event.MODIFY, Constants.ITEM,
                    getID(), null));
        }
        catch (SQLException e)
        {
            log.error(LogManager.getHeader(ourContext,
                    "Error while updating last modified timestamp", "Item: "
                            + getID()));
        }
    }
     ...
     * value for <code>qualifier</code> or <code>lang</code> only matches Dublin
     * Core fields where that qualifier or languages is actually
     * <code>null</code>. Passing in <code>Item.ANY</code> retrieves all
     * metadata fields with any value for the qualifier or language, including
     * <code>null</code>
     * <P>
     * Examples:
     * <P>
     ...
     *            element. <code>null</code> doesn't really make sense as all DC
     ...
     *            <code>Item.ANY</code> means values with any country code or no
     *            country code are returned.
     ...
     * in a <code>null</code> value for <code>qualifier</code> or
     * <code>lang</code> only matches metadata fields where that qualifier or
     * languages is actually <code>null</code>. Passing in <code>Item.ANY</code>
     ...
     * @param element
     *            the element name. <code>Item.ANY</code> matches any element.
     *            <code>null</code> doesn't really make sense as all metadata
     *            must have an element.
     * @param qualifier
     *            the qualifier. <code>null</code> means unqualified, and
     *            <code>Item.ANY</code> means any qualifier (including
     ...
     * Use <code>clearDC</code> to remove values. The ordering of values passed
     * in is maintained.
     * 
     * @param element
     ...
     * is maintained.
     * <p>
     * If metadata authority control is available, try to get authority values.
     * The authority confidence depends on whether authority is
            ...
            String lang, String[] values)
    {
        ...
        String fieldKey = MetadataAuthorityManager.makeFieldKey(schema,
                element, qualifier);
        if (mam.isAuthorityControlled(fieldKey))
        {
            ...
            {
                Choices c = ChoiceAuthorityManager.getManager().getBestMatch(
                        fieldKey, values[i], getOwningCollectionID(), null);
                authorities[i] = c.values.length > 0 ? c.values[0].authority
                        : null;
                confidences[i] = c.confidence;
            }
            addMetadata(schema, element, qualifier, lang, values, authorities,
                    confidences);
        }
        else
        {
    ...
    @Override
    public void addMetadata(String schema, String element, String qualifier,
            String lang, String[] values, String authorities[],
            int confidences[])
    {
        List<Metadatum> dublinCore = getMetadata();
        MetadataAuthorityManager mam = MetadataAuthorityManager.getManager();
        boolean authorityControlled = mam.isAuthorityControlled(schema,
                element, qualifier);
        boolean authorityRequired = mam.isAuthorityRequired(schema, element,
                qualifier);
        ...
        for (int i = 0; i < values.length; i++)
        {
            ...
            // - if authority key is present, use given confidence or NOVALUE if
            // not given
            // - otherwise, preserve confidence if meaningful value was given
            // since it may document a failed authority lookup
            // - CF_UNSET signifies no authority nor meaningful confidence.
            // - it's possible to have empty authority & CF_ACCEPTED if e.g.
            // user deletes authority key
            if (authorityControlled)
            {
                if (authorities != null && authorities[i] != null
                        && authorities[i].length() > 0)
                {
                    dcv.authority = authorities[i];
                    dcv.confidence = confidences == null ? Choices.CF_NOVALUE
                            : confidences[i];
                }
                else
                {
                    dcv.authority = null;
                    dcv.confidence = confidences == null ? Choices.CF_UNSET
                            : confidences[i];
                }
                // authority sanity check: if authority is required, was it
                // supplied?
                // XXX FIXME? can't throw a "real" exception here without
                // changing all the callers to expect it, so use a runtime
                // exception
                if (authorityRequired
                        && (dcv.authority == null || dcv.authority.length() == 0))
                {
     ...
     *            the metadata element name
     * @param qualifier
     *            the metadata qualifier, or <code>null</code> for unqualified
     * @param lang
     *            the ISO639 language code, optionally followed by an underscore
     *            and the ISO3166 country code. <code>null</code> means the
     *            value has no language (for example, a date).
            ...
            String lang, String value, String authority, int confidence)
    {
        ...
        addMetadata(schema, element, qualifier, lang, valArray, authArray,
                confArray);
    }
     ...
     * language is actually <code>null</code>.<code>Item.ANY</code> will match
     * any element, qualifier or language, including <code>null</code>. Thus,
    ...
    /**
     * Clear metadata values. As with <code>getDC</code> above, passing in
     * <code>null</code> only matches fields where the qualifier or language is
     * actually <code>null</code>.<code>Item.ANY</code> will match any element,
     * qualifier or language, including <code>null</code>. Thus,
     * <code>item.clearDC(Item.ANY, Item.ANY, Item.ANY)</code> will remove all
     * Dublin Core metadata associated with an item.
     * 
     * @param schema
     ...
     * <code>Metadatum</code> object passed in. Any or all of the element,
    ...
    public boolean isIn(Collection collection) throws SQLException
    {
        TableRow tr = DatabaseManager.querySingle(ourContext,
                "SELECT COUNT(*) AS count" + " FROM collection2item"
    ...
    public Bundle[] getBundles() throws SQLException
    {
        if (bundles == null)
        {
            ...
            TableRowIterator tri = DatabaseManager.queryTable(ourContext,
                    "bundle", "SELECT bundle.* FROM bundle, item2bundle WHERE "
            ...
            try
            {
                while (tri.hasNext())
                {
                    ...
                    // First check the cache
                    Bundle fromCache = (Bundle) ourContext.fromCache(
     ...
     * @throws AuthorizeException
     */
    public void update(boolean bLastModified) throws SQLException,
            AuthorizeException
    {
        // Check authorisation
        ...
        if (dublinCoreChanged)
        {
            ...
            for (int dcIdx = 0; dcIdx < getMetadata().size(); dcIdx++)
            {
                ...
                // Store the calculated place number, reset the stored flag, and
                // cache the metadatafield
                ...
                if (dcFields[dcIdx] == null)
                {
                    ...
                    throw new SQLException("bad_dublin_core " + "schema="
                            + dcv.schema + ", " + dcv.element + " "
                            + dcv.qualifier);
                }
            }

            // Now the precalculations are done, iterate through the existing
            // metadata
            // looking for matches
            ...
            if (tri != null)
            {
                try
                {
                    while (tri.hasNext())
                    {
                        ...
                        // Assume that we will remove this row, unless we get a
                        // match
                        boolean removeRow = true;

                        // Go through the in-memory metadata, unless we've
                        // already decided to keep this row
                        for (int dcIdx = 0; dcIdx < getMetadata().size()
                                && removeRow; dcIdx++)
                        {
                            // Only process if this metadata has not already
                            // been matched to something in the DB
                            if (!storedDC[dcIdx])
                            {
                                ...
                                if (matched
                                        && dcFields[dcIdx].getFieldID() != tr
                                                .getIntColumn("metadata_field_id"))
                                {
                                    matched = false;
                                }
                                ...
                                {
                                    String lang = tr
                                            .getStringColumn("text_lang");
                                    if (dcv.language == null && lang == null)
                                    {
                                        matched = true;
                                    }
                                    else if (dcv.language != null
                                            && dcv.language.equals(lang))
                                    {
                                        matched = true;
                                    }
                                    else
                                ...
                                {
                                    String auth = tr
                                            .getStringColumn("authority");
                                    int conf = tr.getIntColumn("confidence");
                                    if (!((dcv.authority == null && auth == null) || (dcv.authority != null
                                            && auth != null && dcv.authority
                                                .equals(auth))
                                            && dcv.confidence == conf))
                                    {
                                        matched = false;
                                ...
                                // If the db record is identical to the in
                                // memory values
                                if (matched)
                                {
                                    // Flag that the metadata is already in the
                                    // DB
                                    storedDC[dcIdx] = true;

                                    // Flag that we are not going to remove the
                                    // row
                                    removeRow = false;
                                }
                            }
                        ...
                        // If after processing all the metadata values, we
                        // didn't find a match
        ...
        if (dublinCoreChanged || modified)
        {
            ...
            if (dublinCoreChanged)
            {
                ourContext.addEvent(new Event(Event.MODIFY_METADATA,
                        Constants.ITEM, getID(), getDetails()));
                clearDetails();
                dublinCoreChanged = false;
            }
    ...
    @Override
    private MetadataField getMetadataField(Metadatum dcv) throws SQLException,
            AuthorizeException
    {
        ...
        if (allMetadataFields != null)
        {
            ...
            for (MetadataField field : allMetadataFields)
            {
                if (field.getSchemaID() == schemaID
                        ...
                        && StringUtils.equals(field.getQualifier(),
                                dcv.qualifier))
                {
                    return field;
                }
    ...
    public void withdraw() throws SQLException, AuthorizeException, IOException
    {
        ...
        prov.append("Item withdrawn by ").append(e.getFullName()).append(" (")
                .append(e.getEmail()).append(") on ").append(timestamp)
        ...
        // Write log
        log.info(LogManager.getHeader(ourContext, "withdraw_item",
            ...
            IOException
    {
        ...
        // Write log
        log.info(LogManager.getHeader(ourContext, "reinstate_item",
                "user=" + e.getEmail() + ",item_id=" + getID()));
    }
    ...
    void delete() throws SQLException, AuthorizeException, IOException
    {
        ...
        // the
        // metadata but when getting to the point of removing the bundles we get
        // an exception
         ...
         * because otherwise, tables are left in an inconsistent state and the
         * DB transaction will fail. Any fix would involve too much work on
         * Browse code that is likely to be replaced soon anyway. --lcs, Aug
         * 2006
         ** 
         ** NB Do not check to see if the item is archived - withdrawn /
         * non-archived items may still be tracked in some browse tables for
         * administrative purposes, and these need to be removed.
         **/
    ...
    public void decache() throws SQLException
    {
        ...
        if (bundles != null)
        {
            ...
            for (int i = 0; i < bunds.length; i++)
            {
                ...
                for (int j = 0; j < bitstreams.length; j++)
                {
                    ourContext.removeCached(bitstreams[j],
                            bitstreams[j].getID());
                }
            }
        }
     ...
     * @throws AuthorizeException
     */
    public void replaceAllItemPolicies(List<ResourcePolicy> newpolicies)
            ...
            throws java.sql.SQLException, AuthorizeException
    {
        ...
        log.debug(LogManager.getHeader(ourContext,
                "item_inheritCollectionDefaultPolicies", "item_id=" + getID()));
    }

    public void adjustBundleBitstreamPolicies(Collection c)
            throws SQLException, AuthorizeException
    {

        List<ResourcePolicy> defaultCollectionPolicies = AuthorizeManager
                .getPoliciesActionFilter(ourContext, c,
        ...
        for (int i = 0; i < bunds.length; i++)
        {
            ...
            AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext,
                    mybundle, ResourcePolicy.TYPE_SUBMISSION);
            AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext,
                    mybundle, ResourcePolicy.TYPE_WORKFLOW);

            List<ResourcePolicy> policiesBundleToAdd = filterPoliciesToAdd(
                    defaultCollectionPolicies, mybundle);
            AuthorizeManager.addPolicies(ourContext, policiesBundleToAdd,
                    mybundle);

            for (Bitstream bitstream : mybundle.getBitstreams())
            {
                // if come from InstallItem: remove all submission/workflow
                // policies
                AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext,
                        bitstream, ResourcePolicy.TYPE_SUBMISSION);
                AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext,
                        bitstream, ResourcePolicy.TYPE_WORKFLOW);

                List<ResourcePolicy> policiesBitstreamToAdd = filterPoliciesToAdd(
                        defaultCollectionPolicies, bitstream);
                AuthorizeManager.addPolicies(ourContext,
                        policiesBitstreamToAdd, bitstream);
            }
        }
    }

    public void adjustItemPolicies(Collection c) throws SQLException,
            AuthorizeException
    {
        ...
        // if come from InstallItem: remove all submission/workflow policies
        AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext, this,
                ResourcePolicy.TYPE_SUBMISSION);
        AuthorizeManager.removeAllPoliciesByDSOAndType(ourContext, this,
                ResourcePolicy.TYPE_WORKFLOW);

        // add default policies only if not already in place
        List<ResourcePolicy> policiesToAdd = filterPoliciesToAdd(
                defaultCollectionPolicies, this);
        AuthorizeManager.addPolicies(ourContext, policiesToAdd, this);
    }

    private List<ResourcePolicy> filterPoliciesToAdd(
            List<ResourcePolicy> defaultCollectionPolicies, DSpaceObject dso)
            throws SQLException
    {
        ...
        for (ResourcePolicy rp : defaultCollectionPolicies)
        {
            ...
            if (!AuthorizeManager.isAnIdenticalPolicyAlreadyInPlace(ourContext,
                    dso, rp))
            {
     ...
     * @throws IOException
     */
    public void move(Collection from, Collection to) throws SQLException,
     ...
     * @throws IOException
     */
    public void move(Collection from, Collection to,
            boolean inheritDefaultPolicies) throws SQLException,
            AuthorizeException, IOException
    {
        // Check authorisation on the item before that the move occur
        // otherwise we will need edit permission on the "target collection" to
        // archive our goal
        // only do write authorization if user is not an editor
        if (!canEdit())
        {
        ...
        if (isOwningCollection(from))
        {
            ...
            log.info(LogManager.getHeader(ourContext, "move_item", "item_id="
                    + getID() + ", from " + "collection_id=" + from.getID()
        ...
        else
        {
            ...
            // we'll tell the event system that it has, so that any consumers
            // that
            // care about the structure of the repository can take account of
            // the move

            // Note that updating the owning collection above will have the same
            // effect,
    ...
    {
        Bundle[] bundles = (Bundle[]) ArrayUtils.addAll(getBundles("ORIGINAL"),
    ...
    public Collection[] getCollectionsNotLinked() throws SQLException
    {
        ...
        Collection[] notLinkedCollections = new Collection[allCollections.length
    ...
    {
        Metadatum t[] = getMetadata("dc", "title", null, Item.ANY);
     ...
     * @param value
     *            field value or Item.ANY to match any value
            ...
            throws SQLException, AuthorizeException, IOException
    {
        ...
        if (mds == null)
        {
            throw new IllegalArgumentException("No such metadata schema: "
                    + schema);
        }
        MetadataField mdf = MetadataField.findByElement(context,
                mds.getSchemaID(), element, qualifier);
        if (mdf == null)
        {
            throw new IllegalArgumentException(
                    "No such metadata field: schema=" + schema + ", element="
                            + element + ", qualifier=" + qualifier);
        }
    ...
    public DSpaceObject getAdminObject(int action) throws SQLException
    {
        ...
        else
        {
            ...
            if (qResult != null)
            {
                collection = Collection.find(ourContext,
                        qResult.getIntColumn("collection_id"));
                Community[] communities = collection.getCommunities();
                if (communities != null && communities.length > 0)
                {
                    community = communities[0];
        ...
        {
        case Constants.ADD:
            // ADD a cc license is less general than add a bitstream but we
            // can't/won't
            // add complex logic here to know if the ADD action on the item is
            // required by a cc or
            // a generic bitstream so simply we ignore it.. UI need to enforce
            // the requirements.
    ...
    public DSpaceObject getParentObject() throws SQLException
    {
        ...
        else
        {
            ...
            if (qResult != null)
            {
                return Collection.find(ourContext,
                        qResult.getIntColumn("collection_id"));
            }
            return null;
        }
     ...
     *            metadata field qualifier
     * @param value
     *            the value of authority key to look for
     * @return an iterator over the items matching that authority value
     * @throws SQLException
     *             , AuthorizeException, IOException
     */
            ...
            throws SQLException, AuthorizeException, IOException
    {
        ...
        TableRowIterator rows = DatabaseManager
                .queryTable(
                        context,
                        "item",
    ...
    @Override
    private List<Metadatum> getMetadata()
    {
        try
        {
    ...
    public List<Metadatum> getMetadataPublic()
    {
        return getMetadata();
    }
    ...
    class MetadataCache
    {
        ...
        List<Metadatum> get(Context c, int itemId, Logger log)
                throws SQLException
        {
            if (metadata == null)
            {
                ...
                if (tri != null)
                {
                    try
                    {
                        while (tri.hasNext())
                        {
                            ...
                            // Get the associated metadata field and schema
                            // information
                            int fieldID = resultRow
                                    .getIntColumn("metadata_field_id");
                            ...
                            {
                                MetadataSchema schema = MetadataSchema.find(c,
                                        field.getSchemaID());
                                if (schema == null)
                                {
                                    log.error("Loading item - cannot find metadata schema "
                                            + field.getSchemaID()
                                            + ", field "
                                            + fieldID);
                                }
                                else
                                {
                                    ...
                                    dcv.value = resultRow
                                            .getStringColumn("text_value");
                                    dcv.language = resultRow
                                            .getStringColumn("text_lang");
                                    // dcv.namespace = schema.getNamespace();
                                    dcv.schema = schema.getName();
                                    dcv.authority = resultRow
                                            .getStringColumn("authority");
                                    dcv.confidence = resultRow
                                            .getIntColumn("confidence");
