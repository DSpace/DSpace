...
/* damanzano: This import is used to enable citation */ 
 ...
 * the database.
 * 
 * <damanzano> This class is modified in order to implement citation functionality
 *
...
public class Item extends DSpaceObject
{
    ...
    Item(Context context, TableRow row) throws SQLException
    {
        ...
        // FIXME - I think it is not necesary to update every time an item is created.
        /**
         * damanzano: Agregado para generación de citas
         */
        if(this.isArchived()){
            updateCitation();
        }
    ...
    public void update() throws SQLException, AuthorizeException
    {
        ...
        if (dublinCoreChanged)
        {
            ...
            for (int dcIdx = 0; dcIdx < getMetadata().size(); dcIdx++)
            {
                ...
                /**
                 * damanzano: The original behavior is changed to enable citation.
                 */
        ...
        if (dublinCoreChanged || modified)
        {
            ...
            {
                /**
                 * Damanzano: Agredado para gestión de citas
                 */
                if(this.isArchived()){
                    updateCitation();
                }
...
//               Remove from indices
    ...
    private void updateCitation() {        
            ...
            log.debug(new Date().toString()+":Llamado a updateCitation");
            // citation is configed?
            if(CitationManager.isConfiged()){
                log.debug("Citation manager esta configurado ");
            
            if (dublinCore != null) {
                log.debug("dublincore no es null ");
                // Make a DCValue object
                DCValue dcv = new DCValue();
                ...
                log.debug("Generated citation string is: "+dcvalue);
                if ((dcvalue != null) && dcvalue.length() > 0) {
                    dcv.value = dcvalue;
                    try {
                        dublinCore.get(ourContext, getID(), log).add(dcv);
                    } catch (SQLException e) {
                        log.error("Loading item - cannot load metadata");
                    }
                }
                ...
                log.debug("dublicore es null ");
            }
            }else{
                log.debug("Citation manager no esta configurado ");
            }            
    }
}
