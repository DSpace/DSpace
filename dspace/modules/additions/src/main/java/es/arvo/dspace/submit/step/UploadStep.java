...
public class UploadStep extends AbstractProcessingStep
{
   ...
   // protected boolean fileRequired = ConfigurationManager.getBooleanProperty("webui.submit.upload.required", true);
    ...
    public int doProcessing(Context context, HttpServletRequest request,
            ...
            AuthorizeException
    {
	boolean fileRequired=getFileRequired(context,request);
        ...
        if (buttonPressed.equalsIgnoreCase("submit_remove_selected"))
        {
            ...
            if (request.getParameter("remove") != null)
            {
                ...
                for (int i = 0; i < removeIDs.length; i++)
                {
                    ...
                    int status = processRemoveFile(context,request, item, id, subInfo);
...
//    		    }
     ...
     *         no errors occurred!)
     */
    protected int processRemoveFile(Context context,HttpServletRequest request, Item item, int bitstreamID, SubmissionInfo subInfo)
            throws IOException, SQLException, AuthorizeException
    {
        ...
        String tokenEvaluacion=request.getParameter("tokenEvaluacion");
        RevisionToken revisionToken=RevisionToken.verificaTokenWorkspaceID(context, tokenEvaluacion, workspaceID);
        
//        String tokenJuicio=request.getParameter("tokenJuicio");
//        JuicioToken juicioToken=JuicioToken.verificaTokenWorkspaceID(context, tokenJuicio, workspaceID);
        
        EPerson previousUser=context.getCurrentUser();
        if(revisionToken!=null /*|| juicioToken!=null*/){
