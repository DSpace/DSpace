...
public class EditDepartmentsForm extends AbstractDSpaceTransformer
{
    ...
    public void addBody(Body body) throws WingException, SQLException
    {
        ...
        String currentName = decodeFromURL(parameters.getParameter("groupName",
                null));
        ...
        if (group != null)
        {
            ...
            {
                int communityID = FlowGroupUtils
                        .getCommunityId(group.getName());
                if (communityID > -1)
                {
        ...
        String memberEPeopleIDsString = parameters.getParameter(
                "memberEPeopleIDs", null);
        ...
        int highlightEPersonID = parameters.getParameterAsInteger(
                "highlightEPersonID", -1);
        ...
        // DIVISION: group-edit
        Division main = body.addInteractiveDivision("group-edit", contextPath
        ...
        {
            // If this group is associated with a collection or community then
            // it is special,
            // thus they shouldn't be able to update it.
            groupText.setDisabled();
            groupText.setHelp(T_label_instructions);
        }
        else if (errors.contains("group_name")
                || errors.contains("group_name_duplicate"))
        {
            groupText.addError("");
        }
        ...
        searchBoxes.addButton("submit_search_epeople").setValue(
                T_submit_search_people);
        searchBoxes.addButton("submit_search_groups").setValue(
        ...
        if (group != null)
        {
            changes = addMemberList(main, group, memberGroupIDs,
                    memberEPeopleIDs, highlightEPersonID, highlightGroupID);
        }
     ...
     */
    private void addEPeopleSearch(Division div, String query, int page,
            Group group, List<Integer> memberEPeopleIDs) throws SQLException,
            WingException
    {
        int resultCount = EPerson.searchResultCount(context, query);
        EPerson[] epeople = EPerson.search(context, query, page
        ...
        Table table = results.addTable("group-edit-search-eperson",
                epeople.length + 1, 1);
        ...
        for (EPerson person : epeople)
        {
            ...
            String url = contextPath
                    + "/admin/epeople?administrative-continue=" + knot.getId()
                    + "&submit_edit_eperson&epersonID=" + epersonID;
            ...
            if (memberEPeopleIDs.contains(person.getID()))
            {
                ...
                {
                    personData.addCell().addHighlight("warn")
            ...
            {
                personData.addCell()
                        .addButton("submit_add_eperson_" + epersonID)
     ...
     */
    private void addGroupSearch(Division div, Group sourceGroup, String query,
            int page, Group parent, List<Integer> memberGroupIDs)
            throws WingException, SQLException
    {
        ...
        Table table = results.addTable("roup-edit-search-group",
                groups.length + 1, 1);
        Row header = table.addRow(Row.ROLE_HEADER);
        ...
        for (Group group : groups)
        {
            ...
            String url = contextPath + "/admin/groups?administrative-continue="
                    + knot.getId() + "&submit_edit_group&groupID=" + groupID;
            ...
            if (FlowGroupUtils.getCollectionId(group.getName()) > -1)
            {
                ...
                if (collection != null)
                {
                    ...
                    highlight.addXref(
                            contextPath + "/handle/" + collection.getHandle(),
            ...
            {
                row.addCell().addButton("submit_add_group_" + groupID)
                        .setValue(T_submit_add);
            }
    ...
    /**
     * Method to extensively check whether the first group has the second group
     * as a distant parent. This is used to avoid creating cycles like A->B,
     * B->C, C->D, D->A which leads all the groups involved to essentially
     * include themselves.
     */
    ...
    /**
     * Add a table with all the current group's members to the specified
     * division.
     *
     * @throws SQLException
     */
    private boolean addMemberList(Division div, Group parent,
            List<Integer> memberGroupIDs, List<Integer> memberEPeopleIDs,
            int highlightEPersonID, int highlightGroupID) throws WingException,
            SQLException
    {
        ...
        Table table = members.addTable("group-edit-members-table",
                memberGroupIDs.size() + memberEPeopleIDs.size() + 1, 4);
        ...
        @SuppressWarnings("unchecked")
        // the cast is correct
        List<Integer> allMemberEPeopleIDs = new ArrayList<Integer>(
                memberEPeopleIDs);
        ...
        for (Integer epersonID : allMemberEPeopleIDs)
        {
            ...
            addMemberRow(table, eperson, highlight, pendingAddition,
                    pendingRemoval);
     ...
     *
     * @param table
     *            The table to add the row too.
     * @param group
     *            The group being displayed in this row.
     * @param highlight
     *            Should the row be highlighted.
     * @param pendingAddition
     *            Is this group pending additition
            ...
            throws WingException, SQLException
    {
        String name = group.getName();
        String url = contextPath + "/admin/groups?administrative-continue="
                + knot.getId() + "&submit_edit_group&groupID=" + group.getID();
        ...
        {
            nameCell.addHighlight("bold").addContent(
                    T_members_group_name.parameterize(name));
        }
     ...
     * @param eperson
     *            The eperson being displayed
     * @param highlight
     *            Should this eperson be highlighted?
     * @param pendingAddition
     *            Is this eperson pending addition?
     * @param pendingRemoval
     *            Is this eperson pending removal?
     */
    private void addMemberRow(Table table, EPerson eperson, boolean highlight,
            boolean pendingAddition, boolean pendingRemoval)
            throws WingException, SQLException
    {
        String fullName = eperson.getFullName();
        String email = eperson.getEmail();
        String url = contextPath + "/admin/epeople?administrative-continue="
                + knot.getId() + "&submit_edit_eperson&epersonID="
                + eperson.getID();

        Row personData = table.addRow(null, null, highlight ? "highlight"
                : null);
        ...
        {
            personData.addCell()
                    .addButton("submit_remove_eperson_" + eperson.getID())
                    .setValue(T_submit_remove);
        }
    }
}
