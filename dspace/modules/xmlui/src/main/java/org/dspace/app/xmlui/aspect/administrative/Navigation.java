...
public class Navigation extends AbstractDSpaceTransformer implements
  ...
  public void setup(SourceResolver resolver, Map objectModel, String src,
    ...
    if (context.getCurrentUser() != null) {
      try {
        availableExports = ItemExport.getExportsAvailable(context
            .getCurrentUser());
      } catch (Exception e) {
  ...
  public void addOptions(Options options) throws SAXException, WingException,
    ...
    if (availableExports != null && availableExports.size() > 0) {
      account.addItem()
          .addXref(contextPath + "/admin/export", T_account_export);
    }
    ...
    if (dso instanceof Item) {
      ...
      if (item.canEdit()) {
        ...
        context.addItem().addXref(
            contextPath + "/admin/item?itemID=" + item.getID(),
            T_context_edit_item);
        if (AuthorizeManager.isAdmin(this.context, dso)) {
          context.addItem().addXref(
              contextPath + "/admin/export?itemID=" + item.getID(),
              T_context_export_item);
          context.addItem().addXref(
      ...
      if (collection.canEditBoolean(true)) {
        context.setHead(T_context_head);
        context.addItemXref(contextPath + "/admin/collection?collectionID="
            + collection.getID(), T_context_edit_collection);
        context.addItemXref(contextPath + "/admin/mapper?collectionID="
            + collection.getID(), T_context_item_mapper);
        if (AuthorizeManager.isAdmin(this.context, dso)) {
          context.addItem().addXref(
              contextPath + "/admin/export?collectionID=" + collection.getID(),
      ...
      if (community.canEditBoolean()) {
        context.setHead(T_context_head);
        context.addItemXref(contextPath + "/admin/community?communityID="
            + community.getID(), T_context_edit_community);
        if (AuthorizeManager.isAdmin(this.context, dso)) {
          context.addItem().addXref(
              contextPath + "/admin/export?communityID=" + community.getID(),
      ...
      if (AuthorizeManager.authorizeActionBoolean(this.context, community,
          Constants.ADD)) {
        context.setHead(T_context_head);
        context.addItemXref(contextPath
            + "/admin/collection?createNew&communityID=" + community.getID(),
            T_context_create_collection);
        context.addItemXref(contextPath
            + "/admin/community?createNew&communityID=" + community.getID(),
            T_context_create_subcommunity);
      }
    }

    if (isSystemAdmin
        && ("community-list".equals(this.sitemapURI) || ""
            .equals(this.sitemapURI))) {
    ...
    if (isSystemAdmin) {
      ...
      drum.addItemXref(contextPath + "/ETD-departments", T_DRUM_ETD_departments);
      ...
      epeople.addItemXref(contextPath + "/admin/epeople",
          T_administrative_people);
      epeople.addItemXref(contextPath + "/admin/groups",
          T_administrative_groups);
      epeople.addItemXref(contextPath + "/admin/authorize",
          T_administrative_authorizations);

      registries.setHead(T_administrative_registries);
      registries.addItemXref(contextPath + "/admin/metadata-registry",
          T_administrative_metadata);
      registries.addItemXref(contextPath + "/admin/format-registry",
          T_administrative_format);

      admin.addItemXref(contextPath + "/admin/item", T_administrative_items);
      admin.addItemXref(contextPath + "/admin/withdrawn",
          T_administrative_withdrawn);
      admin.addItemXref(contextPath + "/admin/private",
          T_administrative_private);
      admin.addItemXref(contextPath + "/admin/panel",
          T_administrative_control_panel);
      ...
      admin.addItemXref(contextPath + "/admin/curate",
          T_administrative_curation);
    }
  }
  ...
  public int addContextualOptions(List context) throws SQLException,
    ...
    if (dso instanceof Item) {
      ...
      // can they admin this collection?
      if (AuthorizeManager.authorizeActionBoolean(this.context, collection,
          Constants.ADMIN)) {
        context.addItemXref(contextPath + "/admin/collection?collectionID="
            + collection.getID(), T_context_edit_collection);
        context.addItemXref(contextPath + "/admin/mapper?collectionID="
    ...
    if (("community-list".equals(this.sitemapURI) || "".equals(this.sitemapURI))
        && AuthorizeManager.isAdmin(this.context)) {
      context.addItemXref(contextPath + "/admin/community?createNew",
          T_context_create_community);
      options++;
    }
