...
public class EditCommunityMetadataForm extends AbstractDSpaceTransformer
{
            ...
            AuthorizeException
    {
        ...
        // Get CommunityGroup array
        CommunityGroup[] groups = CommunityGroup.findAll(context);

        String baseURL = contextPath
                + "/admin/community?administrative-continue=" + knot.getId();

        String short_description_error = FlowContainerUtils
                .checkXMLFragment(thisCommunity
                        .getMetadata("short_description"));
        String introductory_text_error = FlowContainerUtils
                .checkXMLFragment(thisCommunity
                        .getMetadata("introductory_text"));
        String copyright_text_error = FlowContainerUtils
                .checkXMLFragment(thisCommunity.getMetadata("copyright_text"));
        String side_bar_text_error = FlowContainerUtils
                .checkXMLFragment(thisCommunity.getMetadata("side_bar_text"));

        // DIVISION: main
        Division main = body.addInteractiveDivision("community-metadata-edit",
                contextPath + "/admin/community", Division.METHOD_MULTIPART,
                "primary administrative community");
        main.setHead(T_main_head.parameterize(thisCommunity.getName()));

        List options = main.addList("options", List.TYPE_SIMPLE, "horizontal");
        options.addItem().addHighlight("bold")
                .addXref(baseURL + "&submit_metadata", T_options_metadata);
        ...
        {
            group.addOption(groups[i].getID(), groups[i].getShortName());
        }
        group.setOptionSelected(thisCommunity.getIntMetadata("group_id"));

        // short description
        metadataList.addLabel(T_label_short_description);
        Text short_description = metadataList.addItem().addText(
                "short_description");
        short_description.setValue(thisCommunity
                .getMetadata("short_description"));
        ...
        TextArea introductory_text = metadataList.addItem().addTextArea(
                "introductory_text");
        introductory_text.setValue(thisCommunity
                .getMetadata("introductory_text"));
        ...
        TextArea copyright_text = metadataList.addItem().addTextArea(
                "copyright_text");
        ...
        TextArea side_bar_text = metadataList.addItem().addTextArea(
                "side_bar_text");
        ...
        if (thisCommunity.getLogo() != null)
        {
            ...
            item.addFigure(contextPath + "/bitstream/id/"
                    + thisCommunity.getLogo().getID() + "/bob.jpg", null, null);
