 ...
 * following the DSpace METS profile, err well mostly. At least if you use the
 * proper configuration it will be fully compliant with the profile, however
 * this adapter will allow you to configure it to be incorrect.
 *
 * When we are configured to be non-compliant with the profile, the MET's
 * profile is changed to reflect the deviation. The DSpace profile states that
 * metadata should be given in MODS format. However, you can configure this
 * adapter to use any metadata crosswalk. When that case is detected we change
 * the profile to say that we are deviating from the standard profile and it
 * lists what metadata has been added.
 *
 * There are four parts to an item's METS document: descriptive metadata, file
 * section, structural map, and extra sections.
 *
 * Request item-support Original Concept, JSPUI version: Universidade do Minho
 * at www.uminho.pt Sponsorship of XMLUI version: Instituto Oceanográfico de
 * España at www.ieo.es
 *
...
public class ItemAdapter extends AbstractAdapter
{
     ...
     */
    protected String getAmdSecID(String admSecName, String mdType,
            DSpaceObject dso)
    {
     ...
     * section for each crosswalk configured. Furthermore, a special check has
     * been added that will add MODS descriptive metadata if it is available in
     * DSpace.
     *
     * Example: <dmdSec> <mdWrap MDTYPE="MODS"> <xmlData> ... content from the
     * crosswalk ... </xmlDate> </mdWrap> </dmdSec
     */
    @Override
    protected void renderDescriptiveSection() throws WingException,
            SAXException, CrosswalkException, IOException, SQLException
    {
        AttributeMap attributes;
        String groupID = getGenericID("group_dmd_");
        dmdSecIDS = new StringBuffer();

        // Add DIM descriptive metadata if it was requested or if no metadata
        // types
        // were specified. Furthermore, since this is the default type we also
        // use a
            ...
            for (Metadatum dcv : dcvs)
            {
                        ...
                        dcv.element, dcv.qualifier))
                {
                            ...
                            || dcv.confidence != Choices.CF_UNSET)
                    {
                        ...
                        attributes.put("confidence",
                                Choices.getConfidenceText(dcv.confidence));
                    }
        ...
        for (String dmdType : dmdTypes)
        {
            ...
            try
            {
                ...
                SAXFilter filter = new SAXFilter(contentHandler,
                        lexicalHandler, namespaces);
                // Allow the basics for XML
                filter.allowElements().allowIgnorableWhitespace()
                        .allowCharacters().allowCDATA().allowPrefixMappings();
        ...
        Boolean include = ConfigurationManager
                .getBooleanProperty("xmlui.bitstream.mods");
        if (include && dmdTypes.contains("MODS"))
        {
            ...
            for (Bundle bundle : bundles)
            {
                ...
                reader.setProperty(
                        "http://xml.org/sax/properties/lexical-handler", filter);
                try
                {
     ...
     *
     * Example: <amdSec> <mdWrap MDTYPE="OTHER" OTHERMDTYPE="METSRights">
     * <xmlData> ... content from the crosswalk ... </xmlDate> </mdWrap>
     * </amdSec>
     */
    @Override
    protected void renderAdministrativeSection() throws WingException,
            SAXException, CrosswalkException, IOException, SQLException
    {
        AttributeMap attributes;
        String groupID;

        // Only create an <amdSec>, if we have amdTypes (or sub-sections)
        // specified...
        // (this keeps our METS file small, by default, and hides all our admin
        // metadata)
        if (amdTypes.size() > 0)
        {
            // //////////////////////////////
        ...
        for (String amdSecName : amdTypes.keySet())
        {
            ...
            for (String mdType : mdTypes)
            {
                ...
                // First, check if this crosswalk can handle disseminating
                // Item-level Administrative metadata
                if (crosswalk.canDisseminate(item))
                {
                    // Since this crosswalk works with items, first render a
                    // section for entire item
                    renderAmdSubSection(amdSecName, mdType, crosswalk, item);
                }

                // Next, we'll try and render Bitstream-level administrative
                // metadata
                // (Although, we're only rendering this metadata for the bundles
                // specified)
                ...
                for (Bundle bundle : bundles)
                {
                    ...
                    // Create a sub-section of <amdSec> for each bitstream in
                    // bundle
                    for (Bitstream bitstream : bitstreams)
                    {
                        // Only render the section if crosswalk works with
                        // bitstreams
                        if (crosswalk.canDisseminate(bitstream))
                        {
                            renderAmdSubSection(amdSecName, mdType, crosswalk,
                                    bitstream);
                        }
    ...
    /**
     * Render a sub-section of the administrative metadata section. Valid
     * sub-sections include: techMD, rightsMD, sourceMD, digiprovMD
     *
     * Example: <techMD> <mdWrap MDTYPE="PREMIS"> <xmlData> [PREMIS content ...
     * ] </xmlData> </mdWrap> </techMD>
     *
     * @param amdSecName
     *            Name of administrative metadata section
     * @param mdType
     *            Type of metadata section (e.g. PREMIS)
     * @param crosswalk
     *            The DisseminationCrosswalk to use to generate this section
     * @param dso
     *            The current DSpace object to use the crosswalk on
     */
    protected void renderAmdSubSection(String amdSecName, String mdType,
            DisseminationCrosswalk crosswalk, DSpaceObject dso)
            throws WingException, SAXException, CrosswalkException,
            IOException, SQLException
    {
        ...
        try
        {
            ...
            SAXFilter filter = new SAXFilter(contentHandler, lexicalHandler,
                    namespaces);
            // Allow the basics for XML
            filter.allowElements().allowIgnorableWhitespace().allowCharacters()
                    .allowCDATA().allowPrefixMappings();
    ...
    /**
     * Render the METS file section. This will contain a list of all bitstreams
     * in the item. Each bundle, even those that are not typically displayed
     * will be listed.
     *
     * Example: <fileSec> <fileGrp USE="CONTENT"> <file ... > <fLocate ... >
     * </file> </fileGrp> <fileGrp USE="TEXT"> <file ... > <fLocate ... >
    ...
    protected void renderFileSection() throws SQLException, SAXException
    {
        ...
        // Suppress license?
        Boolean showLicense = ConfigurationManager
        ...
        for (Bundle bundle : bundles)
        {
            ...
            for (Bitstream bitstream : bundle.getBitstreams())
            {
                ...
                String groupID = getGroupFileID((originalBitstream == null) ? bitstream
                        : originalBitstream);

                // Check if there were administrative metadata sections
    ...
    /**
     * Render the item's structural map. This includes a list of content
     * bitstreams, those are bitstreams that are typically viewable by the end
     * user.
     *
     * Example: <structMap TYPE="LOGICAL" LABEL="DSpace"> <div
     * TYPE="DSpace Item" DMDID="space-separated list of ids"> <fptr
     * FILEID="primary bitstream"/> ... a div for each content bitstream. </div>
     * </structMap>
     */
    @Override
     ...
     * then all of that document's sections are included in this document's METS
     * document.
     */
    @Override
    protected void renderExtraSections() throws SAXException, SQLException,
            IOException
    {
        Boolean include = ConfigurationManager
                .getBooleanProperty("xmlui.bitstream.mets");
        if (!include)
        {
            return;
        }
        ...
        for (Bundle bundle : bundles)
        {
            ...
            try
            {
                SAXFilter filter = new SAXFilter(contentHandler,
                        lexicalHandler, namespaces);
                // Allow the basics for XML
                filter.allowIgnorableWhitespace().allowCharacters()
                        .allowCDATA().allowPrefixMappings();
                // Special option, only allow elements below the second level to
                // pass through. This
                // will trim out the METS declaration and only leave the actual
                // METS parts to be
                // included.
    ...
    /**
     * Checks which Bundles of current item a user has requested. If none
     * specifically requested, then all Bundles are returned.
     *
    ...
    protected static Bitstream findOriginalBitstream(Item item,
            Bitstream derived) throws SQLException
    {
        ...
        // minus the extension (ie everything from and including the last "."
        // character)
        String originalFilename = derived.getName().substring(0,
                derived.getName().lastIndexOf("."));
    ...
    protected void renderFileWithAllowed(Item item, Bitstream bitstream,
            String fileID, String groupID, String admID) throws SAXException
    {
        AttributeMap attributes;
        ...
        // be null if a handle has not yet been assigned. In this case reference
        // the
        // item its internal id. In the last case where the bitstream is not
        // associated
        // with an item (such as a community logo) then reference the
        // bitstreamID directly.
        ...
        try
        {
            if (AuthorizeManager.authorizeActionBoolean(context, bitstream,
                    Constants.READ))
            {
                isAllowed = "y";
            }
