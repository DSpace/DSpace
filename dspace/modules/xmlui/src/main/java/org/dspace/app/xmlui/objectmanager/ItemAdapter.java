...
public class ItemAdapter extends AbstractAdapter {
   ...
   */
  @Override
  protected void renderDescriptiveSection() throws WingException, SAXException,
    ...
    if (dmdTypes.size() == 0 || dmdTypes.contains("DIM")) {
      ...
      for (DCValue dcv : dcvs) {
        if (!MetadataExposure.isHidden(context, dcv.schema, dcv.element,
            dcv.qualifier)) {
          // ///////////////////////////////
   ...
   * <xmlData> ... content from the crosswalk ... </xmlDate> </mdWrap> </amdSec>
   */
  @Override
  protected void renderAdministrativeSection() throws WingException,
    ...
    for (String amdSecName : amdTypes.keySet()) {
      ...
      for (String mdType : mdTypes) {
        ...
        // First, check if this crosswalk can handle disseminating Item-level
        // Administrative metadata
        if (crosswalk.canDisseminate(item)) {
          // Since this crosswalk works with items, first render a section for
          // entire item
  ...
  protected void renderAmdSubSection(String amdSecName, String mdType,
      DisseminationCrosswalk crosswalk, DSpaceObject dso) throws WingException,
      SAXException, CrosswalkException, IOException, SQLException {
    // ///////////////////////////////
  ...
  /**
   * Render the METS file section. This will contain a list of all bitstreams in
   * the item. Each bundle, even those that are not typically displayed will be
   * listed.
   *
   * Example: <fileSec> <fileGrp USE="CONTENT"> <file ... > <fLocate ... >
   * </file> </fileGrp> <fileGrp USE="TEXT"> <file ... > <fLocate ... > </file>
   * </fileGrp> </fileSec>
   */
  @Override
  protected void renderFileSection() throws SQLException, SAXException {
    ...
    for (Bundle bundle : bundles) {
      ...
      for (Bitstream bitstream : bundle.getBitstreams()) {
        ...
        // Check if there were administrative metadata sections corresponding to
        // this file
        String admIDs = null;
        if (fileAmdSecIDs.containsKey(fileID)) {
   ...
   * user.
   *
   * Example: <structMap TYPE="LOGICAL" LABEL="DSpace"> <div TYPE="DSpace Item"
   * DMDID="space-separated list of ids"> <fptr FILEID="primary bitstream"/> ...
   * a div for each content bitstream. </div> </structMap>
   */
  @Override
    ...
    for (Bundle bundle : bundles) {
      ...
      try {
        SAXFilter filter = new SAXFilter(contentHandler, lexicalHandler,
            namespaces);
        // Allow the basics for XML
        filter.allowIgnorableWhitespace().allowCharacters().allowCDATA()
            .allowPrefixMappings();
        // Special option, only allow elements below the second level to pass
        // through. This
        // will trim out the METS declaration and only leave the actual METS
        // parts to be
        // included.
   ...
   * @param admID
   *          The IDs of the administrative metadata sections which pertain to
   *          this file
   */
