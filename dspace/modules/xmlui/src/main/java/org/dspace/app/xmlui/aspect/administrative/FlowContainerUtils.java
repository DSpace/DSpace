...
public class FlowContainerUtils
{
     ...
     *            The collection id.
     * @param deleteLogo
     *            Determines if the logo should be deleted along with the
     *            metadata editing action.
     * @param request
     *            the Cocoon request object
     * @return A process result's object.
     */
    public static FlowResult processEditCollection(Context context,
            int collectionID, boolean deleteLogo, Request request)
            throws SQLException, IOException, AuthorizeException
    {
        ...
        String provenanceDescription = request
        ...
        if (provenanceDescription != null
                && provenanceDescription.length() == 0)
        {
    ...
    public static FlowResult processSetupCollectionHarvesting(Context context,
            int collectionID, Request request) throws SQLException,
            IOException, AuthorizeException
    {
        FlowResult result = new FlowResult();
        HarvestedCollection hc = HarvestedCollection
                .find(context, collectionID);

        String contentSource = request.getParameter("source");

        // First, if this is not a harvested collection (anymore), set the
        // harvest type to 0; possibly also wipe harvest settings
        ...
        else
        {
            ...
            // if the supplied options all check out, set the harvesting
            // parameters on the collection
            if (subResult.getErrors().isEmpty())
            {
                String oaiProvider = request.getParameter("oai_provider");
                boolean oaiAllSets = "all".equals(request
                        .getParameter("oai-set-setting"));
                String oaiSetId;
                if (oaiAllSets)
                {
                    oaiSetId = "all";
                ...
                hc.setHarvestParams(Integer.parseInt(harvestType), oaiProvider,
                        oaiSetId, metadataKey);
        ...
        // Message("default","Harvesting options successfully modified."));
    ...
    /**
     * Use the collection's harvest settings to immediately perform a harvest
     * cycle.
     * 
     * @param context
     *            The current DSpace context.
    ...
    public static FlowResult processRunCollectionHarvest(Context context,
            int collectionID, Request request) throws SQLException,
            IOException, AuthorizeException, CrosswalkException,
                    ...
                    .hasStatus(HarvestScheduler.HARVESTER_STATUS_STOPPED))
            {
                synchronized (HarvestScheduler.lock)
                {
                    HarvestScheduler.setInterrupt(
                            HarvestScheduler.HARVESTER_INTERRUPT_INSERT_THREAD,
                            collectionID);
    ...
    public static FlowResult testOAISettings(Context context, Request request)
    {
        ...
        {
            List<String> testErrors = OAIHarvester.verifyOAIharvester(
                    oaiProvider, oaiSetId, metadataKey, (harvestTypeInt > 1));
            result.setErrors(testErrors);
        }
        ...
        {
            result.setOutcome(true);
            // On a successful test we still want to stay in the loop, not
            // continue out of it
            // result.setContinue(true);
        ...
        else
        {
            ...
            result.setContinue(false);
            // don't really need a message when the errors are highlighted
            // already
            // result.setMessage(new
            // Message("default","Harvesting is not properly configured."));
        }
    ...
    /**
     * Look up the id of a group authorized for one of the given roles. If no
     * group is currently authorized to perform this role then a new group will
     * be created and assigned the role.
     * 
     * @param context
     *            The current DSpace context.
     * @param collectionID
     ...
     */
    public static int getCollectionRole(Context context, int collectionID,
            String roleName) throws SQLException, AuthorizeException,
            ...
            WorkflowConfigurationException, ParserConfigurationException
    {
        ...
        else
        {
            ...
            {// Resolve our id to a role
                roleGroup = getXMLWorkflowRole(context, collectionID, roleName,
                        collection, roleGroup);
            }
            else
            {
                roleGroup = getOriginalWorkflowRole(roleName, collection,
                        roleGroup);
            }
    ...
    private static Group getOriginalWorkflowRole(String roleName,
            Collection collection, Group roleGroup) throws SQLException,
            AuthorizeException
    {
    ...
    private static Group getXMLWorkflowRole(Context context, int collectionID,
            String roleName, Collection collection, Group roleGroup)
            throws IOException, WorkflowConfigurationException, SQLException,
            AuthorizeException
    {
        Role role = WorkflowUtils.getCollectionAndRepositoryRoles(collection)
                .get(roleName);
        if (role.getScope() == Role.Scope.COLLECTION
                || role.getScope() == Role.Scope.REPOSITORY)
        {
            ...
            if (roleGroup == null)
            {
                ...
                if (role.getScope() == Role.Scope.COLLECTION)
                {
                    roleGroup.setName("COLLECTION_" + collection.getID()
                ...
                if (role.getScope() == Role.Scope.COLLECTION)
                {
                    WorkflowUtils.createCollectionWorkflowRole(context,
                            collectionID, roleName, roleGroup);
                }
            }
        }
        return roleGroup;
    ...
    public static FlowResult processDeleteCollectionRole(Context context,
            int collectionID, String roleName, int groupID)
            ...
            WorkflowConfigurationException
    {
        ...
        // Second, remove all authorizations for this role by searching for all
        // policies that this
        // group has on the collection and remove them otherwise the delete will
        // fail because
            ...
            throws SQLException, AuthorizeException
    {
        ...
        Group[] itemGroups = AuthorizeManager.getAuthorizedGroups(context,
                collection, Constants.DEFAULT_ITEM_READ);
        Group[] bitstreamGroups = AuthorizeManager.getAuthorizedGroups(context,
                collection, Constants.DEFAULT_BITSTREAM_READ);

        int itemGroupID = -1;

        // If there are more than one groups assigned either of these privileges
        // then this role based method will not work.
        // The user will need to go to the authorization section to manually
        // straighten this out.
        ...
        else
        {
            ...
            // If the same group is not assigned both of these privileges then
            // this role based method will not work. The user
            // will need to go to the authorization section to manually
            // straighten this out.
    ...
    /**
     * Change default privileges from the anonymous group to a new group that
     * will be created and appropriate privileges assigned. The id of this new
    ...
    public static int createCollectionDefaultReadGroup(Context context,
            int collectionID) throws SQLException, AuthorizeException,
            UIException
    {
        ...
        AuthorizeManager.addPolicy(context, collection,
                Constants.DEFAULT_ITEM_READ, role);
        AuthorizeManager.addPolicy(context, collection,
                Constants.DEFAULT_BITSTREAM_READ, role);
     ...
     * 
     * If getCollectionDefaultRead() returns -1 or the anonymous group then
     * nothing is done.
     * 
     * @param context
     *            The current DSpace context.
     ...
     * @return A process result's object.
     */
    public static FlowResult changeCollectionDefaultReadToAnonymous(
            Context context, int collectionID) throws SQLException,
            AuthorizeException, UIException
    {
        ...
        AuthorizeManager.addPolicy(context, collection,
                Constants.DEFAULT_ITEM_READ, anonymous);
        AuthorizeManager.addPolicy(context, collection,
                Constants.DEFAULT_BITSTREAM_READ, anonymous);
    ...
    public static FlowResult processCreateCollection(Context context,
            int communityID, Request request) throws SQLException,
            AuthorizeException, IOException
    {
        ...
        newCollection.setMetadata("provenance_description",
     ...
     *            The current DSpace context.
     * @param communityID
     *            The id of the parent community (-1 for a top-level community).
     * @return A process result's object.
     */
    public static FlowResult processCreateCommunity(Context context,
            int communityID, Request request) throws AuthorizeException,
            IOException, SQLException
    {
        ...
        result.setOutcome(true);
        result.setMessage(new Message("default",
                "The community was successfully created."));
    ...
    public static FlowResult processEditCommunity(Context context,
            int communityID, boolean deleteLogo, Request request)
    ...
    public static FlowResult processDeleteCommunity(Context context,
            int communityID) throws SQLException, AuthorizeException,
            IOException
    {
        FlowResult result = new FlowResult();
     ...
     *         if the role was not found.
     */
    public static int getCommunityRole(Context context, int communityID,
            String roleName) throws SQLException, AuthorizeException,
            IOException
    {
        Community community = Community.find(context, communityID);
    ...
    public static FlowResult processDeleteCommunityRole(Context context,
            int communityID, String roleName, int groupID) throws SQLException,
    ...
    /**
     * Delete a collection's template item (which is not a member of the
     * collection).
     * 
     * @param context
     * @param collectionID
    ...
    public static FlowResult processCurateCollection(Context context,
            int dsoID, Request request) throws AuthorizeException, IOException,
            SQLException, Exception
    {
        ...
        try
        {
            ...
            if (collection != null)
            {
                // Call curate(context,ID) to ensure a Task Performer (Eperson)
                // is set in Curator
     ...
     */
    public static FlowResult processQueueCollection(Context context, int dsoID,
            Request request) throws AuthorizeException, IOException,
            SQLException, Exception
    {
        String task = request.getParameter("curate_task");
        Curator curator = FlowCurationUtils.getCurator(task);
        String objId = String.valueOf(dsoID);
        String taskQueueName = ConfigurationManager.getProperty("curate",
                "ui.queuename");
        ...
        return FlowCurationUtils.getQueueFlowResult(task, status, objId,
                taskQueueName);
    }
     ...
     * 
     */
    public static FlowResult processCurateCommunity(Context context, int dsoID,
            Request request) throws AuthorizeException, IOException,
            SQLException, Exception
    {
        String task = request.getParameter("curate_task");
     ...
     * queues curation tasks
     */
    public static FlowResult processQueueCommunity(Context context, int dsoID,
            Request request) throws AuthorizeException, IOException,
            SQLException, Exception
    {
        String task = request.getParameter("curate_task");
    ...
    /**
     * Check whether this metadata value is a proper XML fragment. If the value
     * is not then an error message will be returned that might (sometimes not)
     * tell the user how to correct the problem.
     * 
     * @param value
     *            The metadata value
     * @return An error string of the problem or null if there is no problem
    ...
    public static String checkXMLFragment(String value)
    {
        ...
        String xml = "<?xml version='1.0' encoding='UTF-8'?><fragment>" + value
                + "</fragment>";
        ...
        {
            inputStream = new ByteArrayInputStream(xml.getBytes()); // not
                                                                    // supposed
                                                                    // to
                                                                    // happen,
                                                                    // but never
    ...
    /**
     * Sanitize any XML that was inputed by the user, this will clean up any
     * unescaped characters so that they can be stored as proper XML. These are
     * errors that in general we want to take care of on behalf of the user.
     * 
     * @param value
    ...
    /**
     * Check if the given character sequence is located in the given string at
     * the specified index. If it is then return true, otherwise false.
     * 
     * @param string
     ...
     * @param characters
     *            The character sequence to look for.
