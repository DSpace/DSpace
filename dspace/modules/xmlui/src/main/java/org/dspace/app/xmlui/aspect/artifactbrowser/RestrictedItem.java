 ...
 */
public class RestrictedItem extends AbstractDSpaceTransformer // implements
// CacheableProcessingComponent
{
            ...
            ResourceNotFoundException
    {
        ...
        else if (dso instanceof Community)
        {
            ...
            unauthorized.addPara(T_para_community.parameterize(community
                    .getMetadata("name")));
        }
        else if (dso instanceof Collection)
        {
            ...
            unauthorized.addPara(T_para_collection.parameterize(collection
                    .getMetadata("name")));
        }
        else if (dso instanceof Item)
        {
            // The dso may be an item but it could still be an item's bitstream.
            // So let's check for the parameter.
            if (request.getParameter("bitstreamId") != null)
            {
                ...
                try
                {
                    Bitstream bit = Bitstream.find(context,
                            new Integer(request.getParameter("bitstreamId")));
                    if (bit != null)
                    {
            ...
            else
            {
                ...
                unauthorized.addPara("item_status", status.getKey())
                        .addContent(status);
        ...
        {
            // This case should not occur, but if it does just fall back to the
            // resource message.
            unauthorized = body.addDivision("unauthorized-resource", "primary");
            unauthorized.setHead(T_head_resource);
            unauthorized.addPara(T_para_resource);
                ...
                && isEmbargo == false)
        {
            ...
            // Interrupt request if the user is not authenticated, so they may
            // come back to
            ...
            AuthenticationUtil.interruptRequest(objectModel, header, message,
                    characters);
        }

        // Finally, set proper response. Return "404 Not Found" for all
        // restricted/withdrawn items
