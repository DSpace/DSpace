...
public class FlowDepartmentUtils
{
    ...
    /** Language Strings */
    private static final Message T_edit_group_success_notice = new Message(
            "default",
            "xmlui.administrative.FlowGroupUtils.edit_group_success_notice");

    private static final Message T_delete_group_success_notice = new Message(
            "default",
            "xmlui.administrative.FlowGroupUtils.delete_group_success_notice");
     ...
     * @return A result
     */
    public static FlowResult processSaveGroup(Context context, int groupID,
            String newName, String[] newEPeopleIDsArray,
            String[] newGroupIDsArray) throws SQLException, AuthorizeException,
            UIException
    {
        ...
        if (groupID == -1)
        {
            ...
            if (newName == null || newName.length() == 0)
            {
                ...
                result.setMessage(new Message("default",
                        "The group name may not be blank."));
            ...
            else
            {
                ...
                result.setOutcome(false);
                result.setMessage(new Message("default",
                        "The group name is allready in use"));
        ...
        else
        {
            ...
            if (newName != null && newName.length() > 0
                    && !name.equals(newName))
            {
                // The group name is to be updated, check if the newName is
                // allready in use.
                Group potentialDuplicate = Group.findByName(context, newName);
    ...
    /**
     * Remove the specified groups. It is assumed that the user has allready
    ...
    public static FlowResult processDeleteGroups(Context context,
            String[] groupIDs) throws SQLException, AuthorizeException,
            IOException
    {
        FlowResult result = new FlowResult();
        result.setContinue(true);
     ...
     * inbetween the prefix and the suffix.
     *
