        ...
        Selector
{
    ...
    private static Logger log = Logger.getLogger(AuthenticatedOrTokenSelector.class);
            ...
            Parameters parameters)
    {
        try
        {
            ...
            // Token de revision
            String token=(String) ((org.apache.cocoon.environment.wrapper.RequestWrapper)objectModel.get("request")).get("tokenEvaluacion");
            if(token==null){
            	token=RevisionToken.getTokenParameter(((org.apache.cocoon.environment.wrapper.RequestWrapper)objectModel.get("request")).getRequestURI());
            }
            RevisionToken revisionToken=null;
            if(StringUtils.isNotEmpty(token)){
        	revisionToken = RevisionToken.findUnique(context,token);
            }
            
//            // Token de juicio
//            String tokenJuicio=(String) ((org.apache.cocoon.environment.wrapper.RequestWrapper)objectModel.get("request")).get("tokenJuicio");
//            if(tokenJuicio==null){
//            	tokenJuicio=JuicioToken.getTokenParameter(((org.apache.cocoon.environment.wrapper.RequestWrapper)objectModel.get("request")).getRequestURI());
//            }
//            JuicioToken juicioToken=null;
//            if(StringUtils.isNotEmpty(tokenJuicio)){
//            	juicioToken = JuicioToken.findUnique(context,tokenJuicio);
//            }

            
            // Si es a la coleccion adecuada y el token existe return true
            // mock
            // ojo que puede estar a medias: http://localhost:8080/e-ieo/submit?workspaceID=7
