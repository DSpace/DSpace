...
<!--						</xsl:if>-->
    ...
    <xsl:template match="dri:field[@id='aspect.submission.submit.OverviewStep.field.submit_genbank']">
	    <xsl:variable name="url" select="//dri:field[@id='aspect.submission.submit.OverviewStep.field.genbank_url']/dri:value"/>
		...
		<xsl:attribute name="onclick">javascript:return openCenterPopup('<xsl:value-of  select="$url"/>', 'GenBank', '1000', '800');</xsl:attribute>
	    </xsl:element>
	</xsl:template>
