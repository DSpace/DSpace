...
var t=this.chart.theme,_9,_a,_b,f,_d,_e,_f;
ht=this._hScaler.scaler.getTransformerFromModel(this._hScaler),vt=this._vScaler.scaler.getTransformerFromModel(this._vScaler),baseline=Math.max(0,this._vScaler.bounds.lower),baselineHeight=vt(baseline),events=this.events();
f=dc.calculateBarSize(this._hScaler.bounds.scale,this.opt,this.series.length);
_d=f.gap;
_e=_f=f.size;
this.resetEvents();
for(var i=0;i<this.series.length;++i){
var run=this.series[i],_12=_f*i;
...
var v=run.data[j],vv=vt(v),_16=vv-baselineHeight,h=Math.abs(_16);
if(_e>=1&&h>=1){
var _18=s.createRect({x:_5.l+ht(j+0.5)+_d+_12,y:_4.height-_5.b-(v>baseline?vv:baselineHeight),width:_e,height:h}).setFill(_b).setStroke(_a);
run.dyn.fill=_18.getFill();
run.dyn.stroke=_18.getStroke();
if(events){
var o={element:"column",index:j,run:run,plot:this,hAxis:this.hAxis||null,vAxis:this.vAxis||null,shape:_18,x:j+0.5,y:v};
