...
var query = clean_count_query(jQuery('#lterCount').attr('class'), 'l4');

jQuery.get("/solr/search/select/?indent=on&rows=1&fq=location:l4&" + query,
	function(xml){
		var count = jQuery(xml).find('result').attr('numFound');
		jQuery('#lterCount').append(document.createTextNode('(' + count + ')'));
		...
		if (count == 0) {
			jQuery('#lterResultsLink').attr("href", "#");
		}
	});