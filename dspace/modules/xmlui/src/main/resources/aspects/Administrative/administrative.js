...
/**
 * Send the given page and DO NOT wait for the flow to be continued. Execution
 * will proceed as normal. This method will perform two useful actions: set the
 * flow parameter & add result information.
 *
...
/*
 * This function cycles through the submit parameters until it finds one that
 * starts with "prefix". It then returns everything after the prefix for the
 * first match. If none are found, null is returned. For example, if one of the
 * submit variables is named "submit_add_2123", calling this function with a
...
/*
 * Return whether the currently authenticated EPerson is authorized to perform
 * the given action over/on the the given object.
 */
function isAuthorized(objectType, objectID, action) {
	...
	return AuthorizeManager.authorizeActionBoolean(getDSContext(), object,
			action);
}

/**
 * Assert that the currently authenticated eperson is able to perform the given
 * action over the given object. If they are not then an error page is returned
...
/**
 * Return whether the currently authenticated eperson can edit the identified
 * item.
 */
...
/**
 * Assert that the currently authenticated eperson can edit this collection. If
...
/**
 * Return whether the currently authenticated eperson can administrate this
 * collection
 */
function canAdminCollection(collectionID) {
	...
	return AuthorizeManager.authorizeActionBoolean(getDSContext(), collection,
			Constants.ADMIN);
}

/**
 * Assert that the currently authenticated eperson can administrate this
 * collection. If they cannot then this method will never return.
 */
...
/**
 * Assert that the currently authenticated eperson can edit this community. If
 * they cannot then this method will never return.
 */
...
function canAdminCommunity(communityID) {
	...
	return AuthorizeManager.authorizeActionBoolean(getDSContext(), community,
			Constants.ADMIN);
}

/**
 * Assert that the currently authenticated eperson can administrate this
 * community. If they cannot then this method will never return.
 */
...
/**
 * Assert that the currently authenticated eperson can edit the given group. If
 * they cannot then this method will never return.
 */
...
/**
 * Assert that the currently authenticated eperson is an administrator. If they
 * are not then an error page is returned and this function will NEVER return.
 */
...
function startCreateCollection() {
	...
	cocoon
			.redirectTo(cocoon.request.getContextPath() + "/community-list",
					true);
	getDSContext().complete();
	cocoon.exit();
}
...
/**
 * Manage epeople, allow users to create new, edit existing, or remove epeople.
 * The user may also search or browse for epeople.
 *
...
function doManageEPeople() {
	...
	do {
		...
		sendPageAndWait("admin/epeople/main", {
			"page" : page,
			"query" : query,
		...
		if (cocoon.request.get("submit_search")) {
			...
			highlightID = epersonID;
				...
				&& cocoon.request.get("select_eperson")) {
			// Delete a set of epeople
			assertAdministrator();

			var epeopleIDs = cocoon.request
					.getParameterValues("select_eperson");
...
/**
 * Add a new eperson, the user is presented with a form to create a new eperson.
 * They will repeat this form until the user has supplied a unique email
...
function doAddEPerson() {
	...
	do {
		...
		if (cocoon.request.get("submit_save")) {
			// Save the new eperson, assuming they have met all the
			// requirements.
			assertAdministrator();

			result = FlowEPersonUtils.processAddEPerson(getDSContext(),
					cocoon.request, getObjectModel());
...
/**
 * Edit an existing eperson, all standard metadata elements are presented for
 * editing. The same validation is required as is in adding a new eperson:
 * unique email, first name, and last name
 */
function doEditEPerson(epersonID) {
	// We can't assert any privileges at this point, the user could be a
	// collection
	// admin or a supper admin. Instead we protect each operation.
	var result;
	do {
		sendPageAndWait("admin/epeople/edit", {
			"epersonID" : epersonID
		}, result);
		result == null;
		...
		if (cocoon.request.get("submit_save")) {
			...
			assertAdministrator();
			result = FlowEPersonUtils.processEditEPerson(getDSContext(),
					cocoon.request, getObjectModel(), epersonID);
		} else if (cocoon.request.get("submit_cancel")) {
			// Cancel out and return to wherever the user came from.
			return null;
		} else if (cocoon.request.get("submit_edit_group")
				&& cocoon.request.get("groupID")) {
			...
			// Reset the user's password by sending them the forgot password
			// email.
			assertAdministrator();
			result = FlowEPersonUtils.processResetPassword(getDSContext(),
					epersonID);
			...
			// Login as this user.
			assertAdministrator();
			result = FlowEPersonUtils.processLoginAs(getDSContext(),
					getObjectModel(), epersonID);

			if (result != null && result.getOutcome().equals("success")) {
				// the user is loged in as another user, we can't let them
				// continue on
...
/**
 * Confirm and delete the list of given epeople. The user will be presented with
 * a list of selected epeople, and if they click the confirm delete button then
 * the epeople will be deleted.
 */
function doDeleteEPeople(epeopleIDs) {
	assertAdministrator();

	sendPageAndWait("admin/epeople/delete", {
		"epeopleIDs" : epeopleIDs.join(',')
	});
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		var result = FlowEPersonUtils.processDeleteEPeople(getDSContext(),
				epeopleIDs);
		return result;
	}
	return null;
...
/**
 * Manage groups, allow users to create new, edit existing, or remove groups.
 * The user may also search or browse for groups.
 *
 * The is typically used as an entry point flow.
 */
function doManageGroups() {
	...
	do {
		...
		sendPageAndWait("admin/group/main", {
			"query" : query,
			"page" : page,
			"highlightID" : highlightID
		}, result);
		...
		if (cocoon.request.get("submit_search")) {
			...
			highlightID = groupID;
				...
				&& cocoon.request.get("select_group")) {
...
/**
 * This flow allows for the full editing of a group, changing the group's name
 * or removing members. Users may also search for epeople / groups to add as
 * members to this group.
 */
function doEditGroup(groupID) {
	var groupName = FlowGroupUtils.getName(getDSContext(), groupID);
	var memberEPeopleIDs = FlowGroupUtils.getEPeopleMembers(getDSContext(),
			groupID);
	var memberGroupIDs = FlowGroupUtils
			.getGroupMembers(getDSContext(), groupID);
	...
	var result = null;
	do {
		sendPageAndWait("admin/group/edit", {
			"groupID" : groupID,
			"groupName" : groupName,
			"memberGroupIDs" : memberGroupIDs.join(','),
			"memberEPeopleIDs" : memberEPeopleIDs.join(','),
			"highlightEPersonID" : highlightEPersonID,
		...
		if (cocoon.request.get("submit_cancel")) {
			...
			result = FlowGroupUtils.processSaveGroup(getDSContext(), groupID,
					groupName, memberEPeopleIDs, memberGroupIDs);

			// In case a group was created, update our id.
			if (result != null && result.getParameter("groupID"))
				groupID = result.getParameter("groupID");
		} else if (cocoon.request.get("submit_search_epeople")
				&& cocoon.request.get("query")) {
			// Perform a new search for epeople.
			query = cocoon.request.get("query");
			page = 0;
			type = "eperson";
		} else if (cocoon.request.get("submit_search_groups")
				&& cocoon.request.get("query")) {
			...
			page = 0;
			type = "";
		} else if (cocoon.request.get("submit_edit_eperson")
				&& cocoon.request.get("epersonID")) {
		...
		while (names.hasMoreElements()) {
			...
			if ((match = name.match(/submit_add_eperson_(\d+)/)) != null) {
				...
				memberEPeopleIDs = FlowGroupUtils.addMember(memberEPeopleIDs,
						epersonID);
			...
			if ((match = name.match(/submit_add_group_(\d+)/)) != null) {
				...
				memberGroupIDs = FlowGroupUtils.addMember(memberGroupIDs,
						_groupID);
			...
			if ((match = name.match(/submit_remove_eperson_(\d+)/)) != null) {
				// remove an eperson
				var epersonID = match[1];
				memberEPeopleIDs = FlowGroupUtils.removeMember(
						memberEPeopleIDs, epersonID);
				highlightEPersonID = epersonID;
			}
			if ((match = name.match(/submit_remove_group_(\d+)/)) != null) {
				// remove a group
				var _groupID = match[1];
				memberGroupIDs = FlowGroupUtils.removeMember(memberGroupIDs,
						_groupID);
				highlightGroupID = _groupID;
			}
		}
...
/**
 * Confirm that the given groupIDs should be deleted, if confirmed they will be
 * deleted.
 */
function doDeleteGroups(groupIDs) {
	assertAdministrator();

	sendPageAndWait("admin/group/delete", {
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		assertAdministrator();
		var result = FlowGroupUtils.processDeleteGroups(getDSContext(),
				groupIDs);
		return result;
	}
	return null;
 ...
 * one or more schemas. This flow will list all available schemas for edit,
 * allow the user to add or delete schemas.
 *
...
function doManageMetadataRegistry() {
	...
	do {
		...
		if (cocoon.request.get("submit_edit") && cocoon.request.get("schemaID")) {
			...
			result = FlowRegistryUtils.processAddMetadataSchema(getDSContext(),
					namespace, name);
		} else if (cocoon.request.get("submit_delete")
				&& cocoon.request.get("select_schema")) {
...
/**
 * Edit a particular schema, this will list all fields in the schema. When
 * clicking on a field it will be loaded into the top of the page where it can
 * be edited. When the top form is not loaded it can be used to add new fields.
 *
 * The last field operated on is kept as a highlighted field to make it easier
 * to find things in the interface.
 */
function doEditMetadataSchema(schemaID) {
	...
	var result = null;
	do {
		sendPageAndWait("admin/metadata-registry/edit-schema", {
			"schemaID" : schemaID,
			"updateID" : updateID,
			"highlightID" : highlightID
		}, result);
		...
		if (cocoon.request.get("submit_return")) {
			...
			result = FlowRegistryUtils.processAddMetadataField(getDSContext(),
					schemaID, element, qualifier, note);
			...
			result = FlowRegistryUtils.processEditMetadataField(getDSContext(),
					schemaID, updateID, element, qualifier, note);
			...
			highlightID = -1;
				...
				&& cocoon.request.get("select_field")) {
...
function doDeleteMetadataFields(fieldIDs) {
	...
	sendPageAndWait("admin/metadata-registry/delete-fields", {
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		var result = FlowRegistryUtils.processDeleteMetadataField(
				getDSContext(), fieldIDs);
		return result;
	}
	return null;
}
...
function doMoveMetadataFields(fieldIDs) {
	...
	sendPageAndWait("admin/metadata-registry/move-fields", {
		"fieldIDs" : fieldIDs.join(',')
	});
	assertAdministrator();
	...
	if (cocoon.request.get("submit_move") && cocoon.request.get("to_schema")) {
		...
		var result = FlowRegistryUtils.processMoveMetadataField(getDSContext(),
				schemaID, fieldIDs);
		return result;
	}
	return null;
...
function doDeleteMetadataSchemas(schemaIDs) {
	...
	sendPageAndWait("admin/metadata-registry/delete-schemas", {
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		var result = FlowRegistryUtils.processDeleteMetadataSchemas(
				getDSContext(), schemaIDs)
		return result;
	}
	return null;
}
...
/**
 * Manage the set of bitstream formats that are available in the system. A list
 * of all available formats is given each one can be editeded or deleteed and
...
function doManageFormatRegistry() {
	...
	var result = null;
	do {
		sendPageAndWait("admin/format-registry/main", {
			"updateID" : updateID,
			"highlightID" : highlightID
		}, result);
		assertAdministrator();
		...
		if (cocoon.request.get("submit_return")) {
			...
			if (result != null && result.getParameter("formatID"))
				...
				&& cocoon.request.get("select_format")) {
...
/**
 * Edit a single bitstream format specified by formatID. If the formatID is -1
 * then a new bitstream will be added instead.
 */
function doEditBitstreamFormat(formatID) {
	...
	var result = null;
	do {
		sendPageAndWait("admin/format-registry/edit", {
			"formatID" : formatID
		}, result);
		assertAdministrator();
		result = null;
		...
		if (cocoon.request.get("submit_cancel")) {
			...
			result = FlowRegistryUtils.processEditBitstreamFormat(
					getDSContext(), formatID, cocoon.request);
		}
...
function doDeleteBitstreamFormats(formatIDs) {
	...
	sendPageAndWait("admin/format-registry/delete", {
		"formatIDs" : formatIDs.join(',')
	});
	assertAdministrator();

	if (cocoon.request.get("submit_confirm")) {
		// Actually delete the formats
		var result = FlowRegistryUtils.processDeleteBitstreamFormats(
				getDSContext(), formatIDs)
		return result;
	}
	return null;
}
...
/**
 * Manage items, this is a flow entry point allowing the user to search for
 * items by their internal id or handle.
 */
function doManageItems() {
	...
	var result;
	do {
		sendPageAndWait("admin/item/find", {
		...
		if (cocoon.request.get("submit_find")
				&& cocoon.request.get("identifier")) {
			// Search for the identifier
			identifier = cocoon.request.get("identifier");
			result = FlowItemUtils.resolveItemIdentifier(getDSContext(),
					identifier);
...
/**
 * Edit a single item. This method allows the user to switch between the three
 * sections of editing an item: status, bitstreams, and metadata.
 */
...
function doViewItem(itemID) {
	do {
		sendPageAndWait("admin/item/view_item", {
			"itemID" : itemID
		}, null);
		assertEditItem(itemID);
...
function doEditItemStatus(itemID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/status", {
			"itemID" : itemID
		}, result);
		assertEditItem(itemID);
		result = null;
		...
		if (cocoon.request.get("submit_return")
			...
			// authorization check performed by the doAuthorize methods in
...
/**
 * Allow the user to manage the item's bitstreams, add, delete, or change a
 * bitstream's metadata.
 */
function doEditItemBitstreams(itemID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/bitstreams", {
			"itemID" : itemID
		}, result);
		assertEditItem(itemID);
		result = null;

		var submitButton = Util
				.getSubmitButton(cocoon.request, "submit_return");
		if (cocoon.request.get("submit_return")
			...
			result = doAddBitstream(itemID);
				...
				&& cocoon.request.get("bitstreamID")) {
		...
		} else if (submitButton.equals("submit_update_order")
				|| submitButton.startsWith("submit_order_")) {
			result = FlowItemUtils.processReorderBitstream(getDSContext(),
					itemID, cocoon.request);
		}
	} while (true)
}
...
function doEditItemMetadata(itemID, templateCollectionID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/metadata", {
			"itemID" : itemID,
		...
		if (cocoon.request.get("submit_return")
			...
			// Update the item
			result = FlowItemUtils.processEditItem(getDSContext(), itemID,
					cocoon.request);
		}
	} while (true)
}
...
/**
 * Curate an Item Can only be performed by someone who is able to edit that
 * Item.
 */
function doCurateItem(itemID, task) {
	...
	do {
		if (cocoon.request.get("select_curate_group")) {
			var select_curate_group = cocoon.request
					.getParameter("select_curate_group");
			sendPageAndWait("admin/item/curateItem", {
				"itemID" : itemID,
				"select_curate_group" : select_curate_group
			}, result);
		} else {
			sendPageAndWait("admin/item/curateItem", {
				"itemID" : itemID
		...
		if (!cocoon.request.get("submit_curate_task")
				&& !cocoon.request.get("submit_queue_task")
				&& !cocoon.request.get("select_curate_group")) {
			return null;
		} else if (cocoon.request.get("submit_curate_task")) {
			result = FlowItemUtils.processCurateItem(getDSContext(), itemID,
					cocoon.request);
		} else if (cocoon.request.get("submit_queue_task")) {
			result = FlowItemUtils.processQueueItem(getDSContext(), itemID,
					cocoon.request);
		}
	} while (true);
}
...
function doDeleteItem(itemID) {
	...
	sendPageAndWait("admin/item/delete", {
		"itemID" : itemID
	});
...
function doWithdrawItem(itemID) {
	...
	sendPageAndWait("admin/item/withdraw", {
		"itemID" : itemID
	});
...
function doReinstateItem(itemID) {
	...
	sendPageAndWait("admin/item/reinstate", {
		"itemID" : itemID
	});
...
function doPrivateItem(itemID) {
	assertEditItem(itemID);
	sendPageAndWait("admin/item/private", {
		"itemID" : itemID
	});
...
function doPublicItem(itemID) {
	assertEditItem(itemID);
	sendPageAndWait("admin/item/public", {
		"itemID" : itemID
	});
...
function doMoveItem(itemID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/move", {
			"itemID" : itemID
		});
		result = null;
		...
		if (cocoon.request.get("submit_cancel")) {
			...
			result = FlowItemUtils.processMoveItem(getDSContext(), itemID,
					collectionID, inherit);
		}
	} while (result == null || !result.getContinue());
...
function doAddBitstream(itemID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/add-bitstream", {
			"itemID" : itemID
		}, result);
		assertAuthorized(Constants.ITEM, itemID, Constants.ADD);
		result = null;
		...
		if (cocoon.request.get("submit_cancel")) {
			...
			// Upload the file
			result = FlowItemUtils.processAddBitstream(getDSContext(), itemID,
					cocoon.request);
		}
	} while (result == null || !result.getContinue())
...
function doEditBitstream(itemID, bitstreamID) {
	...
	var result;
	do {
		sendPageAndWait("admin/item/edit-bitstream", {
			"itemID" : itemID,
		...
		if (cocoon.request.get("submit_cancel")) {
			...
			result = FlowItemUtils.processEditBitstream(getDSContext(), itemID,
					bitstreamID, bitstreamName, primary, description, formatID,
					userFormat, cocoon.request);
		}
	} while (result == null || !result.getContinue())
...
function doDeleteBitstreams(itemID, bitstreamIDs) {
	...
	sendPageAndWait("admin/item/delete-bitstreams", {
		"itemID" : itemID,
		"bitstreamIDs" : bitstreamIDs.join(',')
	});
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		var result = FlowItemUtils.processDeleteBitstreams(getDSContext(),
				itemID, bitstreamIDs);
		return result;
	}
	return null;
...
function doMapItems(collectionID) {
	...
	do {
		sendPageAndWait("admin/mapper/main", {
...
function doMetadataImportUpload() {
	var result = FlowMetadataImportUtils.processUploadCSV(getDSContext(),
			cocoon.request);
...
function doMetadataImportConfirm() {
	var result = FlowMetadataImportUtils.processMetadataImport(getDSContext(),
			cocoon.request);
	assertAdministrator();
...
function doMapItemSearch(collectionID) {
	...
	sendPageAndWait("admin/mapper/search", {
		"collectionID" : collectionID,
		"query" : query
	}, result);
	assertAuthorized(Constants.COLLECTION, collectionID, Constants.ADD);
	...
	if (cocoon.request.get("submit_cancel")) {
		...
		result = FlowMapperUtils.processMapItems(getDSContext(), collectionID,
				itemIDs);
		return result;
	}
}
...
function doMapItemBrowse(collectionID) {
	...
	do {
		sendPageAndWait("admin/mapper/browse", {
			"collectionID" : collectionID
		});
		...
		if (cocoon.request.get("submit_return")) {
			...
			return null;
		} else if (cocoon.request.get("submit_unmap")
				&& cocoon.request.get("itemID")) {
			// Unmap these items.
			assertAuthorized(Constants.COLLECTION, collectionID,
					Constants.REMOVE);
			var itemIDs = cocoon.request.getParameterValues("itemID");
			result = FlowMapperUtils.processUnmapItems(getDSContext(),
...
/*
 * Current entry point into the authorization flow. Presents the user with form
 * to look up items, perform wildcard authorizations, or select a
 * collection/community from the list to edit.
 */
function doManageAuthorizations() {
	...
	do {
		sendPageAndWait("admin/authorize/main", {
			"query" : query
		}, result);
		...
		if (cocoon.request.get("submit_edit")
			...
			// resolve the identifier to a type, look up its associated object,
			// and act accordingly
			result = FlowAuthorizationUtils.resolveItemIdentifier(
					getDSContext(), identifier);
			if (result.getParameter("type") == Constants.ITEM
					&& result.getParameter("itemID")) {
				var itemID = result.getParameter("itemID");
				result = doAuthorizeItem(itemID);
			} else if (result.getParameter("type") == Constants.COLLECTION
					&& result.getParameter("collectionID")) {
				var collectionID = result.getParameter("collectionID");
				result = doAuthorizeCollection(collectionID);
			} else if (result.getParameter("type") == Constants.COMMUNITY
					&& result.getParameter("communityID")) {
...
/*
 * Wrapper functions for editing authorization policies for collections and
 * communities
 */
...
/*
 * Editing authorization policies for collections and communities, collectively
...
function doAuthorizeContainer(containerType, containerID) {
	...
	do {
		sendPageAndWait("admin/authorize/container", {
...
function doAuthorizeItem(itemID) {
	...
	do {
		sendPageAndWait("admin/authorize/item", {
			"itemID" : itemID,
			"highlightID" : highlightID
		}, result);
		// authorization check moved to FlowAuthorizationUtils
		...
		if (cocoon.request.get("submit_return")) {
			...
			if (result != null && result.getParameter("policyID"))
				...
				&& cocoon.request.get("policy_id")
				&& cocoon.request.get("object_id")
				&& cocoon.request.get("object_type")) {
			// Edit an existing policy
			var policyID = cocoon.request.get("policy_id");
...
/*
 * Advanced or "wildcard" authorizations; presents the user with a list of
 * communities and list of groups, together with options for an authorization
 * action and a recepient of that action. The user can select multiple
 * collections and multiple groups to create repository-wide authorizations.
 */
function doAdvancedAuthorization() {
	...
	do {
		...
		sendPageAndWait("admin/authorize/advanced", {
		...
		if (cocoon.request.get("submit_return")) {
			...
			result = FlowAuthorizationUtils.processAdvancedPolicyAdd(
					getDSContext(), groupIDs, actionID, resourceID,
					collectionIDs, name, description, startDate, endDate);
		} else if (cocoon.request.get("submit_remove_all")) {
			result = FlowAuthorizationUtils.processAdvancedPolicyDelete(
					getDSContext(), resourceID, collectionIDs);
		}
	} while (result == null || !result.getContinue());
}

/**
 * Edit a policy, giving the user a choice of actions and the ability to either
 * browse to a desired group via drop-down field, or search for one via search
 * box.
 */
function doEditPolicy(objectType, objectID, policyID) {
	...
	do {
		/*
		 * The page receives parameters for the type and ID of the DSpace object
		 * that the policy is assciated with, the policy ID, the group search
		 * query (if a search was performed), the ID of the currently associated
		 * group, the current action and the currently viewed page (if a search
		 * returned more than one page of results)
		 */
		sendPageAndWait("admin/authorize/edit", {
		...
		if (cocoon.request.get("submit_search_groups")) {
			...
			result = FlowAuthorizationUtils.processEditPolicy(getDSContext(),
					objectType, objectID, policyID, groupID, actionID, name,
					description, startDate, endDate);
		} else if (cocoon.request.get("submit_cancel")) {
			return null;
		}
		// TODO; make sure to handle the odd case of the target group getting
		// deleted from inside the authorization flow
		else if (cocoon.request.get("submit_edit_group")
				&& cocoon.request.get("group_id")) {
...
/**
 * Confirm that the given policies should be deleted; if confirmed they will be
 * deleted.
 */
function doDeletePolicies(policyIDs) {
	// authorization check moved to FlowAuthorizationUtils
	// assertAdministrator();
	sendPageAndWait("admin/authorize/delete", {
	...
	if (cocoon.request.get("submit_confirm")) {
		...
		var result = FlowAuthorizationUtils.processDeletePolicies(
				getDSContext(), policyIDs);
		return result;
	}
	return null;
...
function doEditCollection(collectionID, newCollectionP) {
	...
	do {
		...
		if (cocoon.request.get("submit_return")
				|| cocoon.request.get("submit_save")) {
			// go back to wherever we came from.
			return null;
		} else if (cocoon.request.get("submit_metadata")) {
...
/**
 * Edit metadata of a collection; presenting the user with a form of standard
 * collection metadata, an option add/remove a logo, and set the item template.
 * From here the user can also move on to edit roles and authorizations screen
 */
function doEditCollectionMetadata(collectionID) {
	...
	do {
		sendPageAndWait("admin/collection/editMetadata", {
			"collectionID" : collectionID
		}, result);
		assertEditCollection(collectionID);
		result = null;
		...
		if (cocoon.request.get("submit_return")
				|| cocoon.request.get("submit_metadata")
				|| cocoon.request.get("submit_roles")
				|| cocoon.request.get("submit_harvesting")
				|| cocoon.request.get("submit_curate")) {
			// return to the editCollection function which will determine where
			// to go next.
			return null;
		} else if (cocoon.request.get("submit_save")) {
			// Save updates
			result = FlowContainerUtils.processEditCollection(getDSContext(),
					collectionID, false, cocoon.request);
			if (result.getContinue())
				return null;
		} else if (cocoon.request.get("submit_delete")) {
			// delete collection
			...
			result = FlowContainerUtils.processEditCollection(getDSContext(),
					collectionID, true, cocoon.request);
		} else if (cocoon.request.get("submit_create_template")
				|| cocoon.request.get("submit_edit_template")) {
			// Create or edit the item's template
			var itemID = FlowContainerUtils.getTemplateItemID(getDSContext(),
					collectionID);
			...
			result = FlowContainerUtils.processDeleteTemplateItem(
					getDSContext(), collectionID);
		}
...
/**
 * Edit the set roles for a collection: admin, workflows, submitters, and
 * default read. Returns to the EditCollection page and selected tab if this is
 * a simple navigation, not a submit.
 */
function setXMLWorkflowRoles(workflow, collectionID, result) {
	...
	for (var i = 0; i < roles.length; i++) {
		...
		if (cocoon.request.get("submit_edit_wf_role_" + roles[i])) {
			assertEditCollection(collectionID);
			var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
					collectionID, roles[i]);
			result = doEditGroup(groupID);
		}
...
function setOriginalWorkflowRoles(collectionID, result) {
	// WORKFLOW STEPS 1-3
	if (cocoon.request.get("submit_edit_wf_step1")
			|| cocoon.request.get("submit_create_wf_step1")) {
		assertEditCollection(collectionID);
		var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
				collectionID, "WF_STEP1");
		result = doEditGroup(groupID);
	} else if (cocoon.request.get("submit_delete_wf_step1")) {
		result = doDeleteCollectionRole(collectionID, "WF_STEP1");
	}

	else if (cocoon.request.get("submit_edit_wf_step2")
			|| cocoon.request.get("submit_create_wf_step2")) {
		assertEditCollection(collectionID);
		var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
				collectionID, "WF_STEP2");
		result = doEditGroup(groupID);
	} else if (cocoon.request.get("submit_delete_wf_step2")) {
		result = doDeleteCollectionRole(collectionID, "WF_STEP2");
	}

	else if (cocoon.request.get("submit_edit_wf_step3")
			|| cocoon.request.get("submit_create_wf_step3")) {
		assertEditCollection(collectionID);
		var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
				collectionID, "WF_STEP3");
		result = doEditGroup(groupID);
...
function doAssignCollectionRoles(collectionID) {
	...
	do {
		sendPageAndWait("admin/collection/assignRoles", {
			"collectionID" : collectionID
		}, result);
		assertEditCollection(collectionID);
		result = null;
		...
		// ADMIN
		else if (cocoon.request.get("submit_edit_admin")
				|| cocoon.request.get("submit_create_admin")) {
			assertEditCollection(collectionID);
			var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
					collectionID, "ADMIN");
			result = doEditGroup(groupID);
		...
		// SUBMIT
		else if (cocoon.request.get("submit_edit_submit")
				|| cocoon.request.get("submit_create_submit")) {
			assertEditCollection(collectionID);
			var groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
					collectionID, "SUBMIT");
			result = doEditGroup(groupID);
		...
		// DEFAULT_READ
			...
			var groupID = FlowContainerUtils.createCollectionDefaultReadGroup(
					getDSContext(), collectionID);
			result = doEditGroup(groupID);
		} else if (cocoon.request.get("submit_edit_default_read")) {
			assertEditCollection(collectionID);
			var groupID = FlowContainerUtils.getCollectionDefaultRead(
					getDSContext(), collectionID);
			result = doEditGroup(groupID);
			...
			if (ConfigurationManager.getProperty("workflow",
				...
				if (workflow == null) {
					var collection = Collection.find(getDSContext(),
							collectionID);
...
/**
 * Curate a Collection Can only be performed by someone who is able to edit that
 * collection.
 */
function doCurateCollection(collectionID, task) {
	...
	do {
		if (cocoon.request.get("select_curate_group")) {
			var select_curate_group = cocoon.request
					.getParameter("select_curate_group");
			sendPageAndWait("admin/collection/curateCollection", {
				"collectionID" : collectionID,
				"select_curate_group" : select_curate_group
			}, result);
		} else {
		...
		if (cocoon.request.get("submit_curate_task")) {
			result = FlowContainerUtils.processCurateCollection(getDSContext(),
					collectionID, cocoon.request);
		} else if (cocoon.request.get("submit_queue_task")) {
			result = FlowContainerUtils.processQueueCollection(getDSContext(),
					collectionID, cocoon.request);
		}
	} while (true);
}
...
/**
 * Set up various harvesting options. From here the user can also move on to
 * edit roles and edit metadata screen.
 */
function doSetupCollectionHarvesting(collectionID) {
	...
	do {
		sendPageAndWait("admin/collection/setupHarvesting", {
			"collectionID" : collectionID,
			"oaiProviderValue" : oaiProviderValue,
			"oaiSetAll" : oaiSetAll,
			"oaiSetIdValue" : oaiSetIdValue,
			"metadataFormatValue" : metadataFormatValue,
			"harvestLevelValue" : harvestLevelValue
		}, result);
		result = null;
		...
		if (cocoon.request.get("submit_return")
			...
			// Save updates
			result = FlowContainerUtils.processSetupCollectionHarvesting(
					getDSContext(), collectionID, cocoon.request);
		} else if (cocoon.request.get("submit_test")) {
			// Ping the OAI server and verify that the address/set/metadata
			// combo is present there
			// Can get this either in a single GetRecords OAI request or via two
			// separate ones: ListSets and ListMetadataFormats
			result = FlowContainerUtils.testOAISettings(getDSContext(),
					cocoon.request);
		}
...
/**
 * Edit existing harvesting options. From here the user can also move on to edit
 * roles and edit metadata screen.
 */
function doEditCollectionHarvesting(collectionID) {
	...
	var result = null;
	do {
		// If this collection's havresting is not set up properly, redirect to
		// the setup screen
		if (HarvestedCollection.find(getDSContext(), collectionID) == null) {
			sendPageAndWait("admin/collection/toggleHarvesting", {
				"collectionID" : collectionID
			}, result);
		} else if (!HarvestedCollection.isHarvestable(getDSContext(),
				collectionID)) {
			doSetupCollectionHarvesting(collectionID);
		} else {
			sendPageAndWait("admin/collection/editHarvesting", {
				"collectionID" : collectionID
			}, result);
		}
		...
		if (cocoon.request.get("submit_return")
			...
			result = FlowContainerUtils.processRunCollectionHarvest(
					getDSContext(), collectionID, cocoon.request);
		} else if (cocoon.request.get("submit_reimport")) {
			// Test the settings and run the import immediately
			result = FlowContainerUtils.processReimportCollection(
					getDSContext(), collectionID, cocoon.request);
...
/**
 * Delete a specified collection role. Under the current implementation, the
 * only roles this applies to directly are the workflow steps. Admin and
 * submitter authorizations cannot be deleted once formed, and the default read
 * group is changed to Anonymous instead.
 */
function doDeleteCollectionRole(collectionID, role) {
	...
	if (role == "DEFAULT_READ") {
		groupID = FlowContainerUtils.getCollectionDefaultRead(getDSContext(),
				collectionID);
	} else {
		groupID = FlowContainerUtils.getCollectionRole(getDSContext(),
				collectionID, role);
	}

	sendPageAndWait("admin/collection/deleteRole", {
		"collectionID" : collectionID,
		"role" : role,
		"groupID" : groupID
	});
	...
	if (cocoon.request.get("submit_confirm") && role == "DEFAULT_READ") {
		...
		var result = FlowContainerUtils.changeCollectionDefaultReadToAnonymous(
				getDSContext(), collectionID);
		return result;
	} else if (cocoon.request.get("submit_confirm")) {
		// All other roles use the standard methods
		var result = FlowContainerUtils.processDeleteCollectionRole(
				getDSContext(), collectionID, role, groupID);
		return result;
	}
	return null;
}
...
function doDeleteCollection(collectionID) {
	...
	sendPageAndWait("admin/collection/delete", {
		"collectionID" : collectionID
	});
	assertAuthorized(Constants.COLLECTION, collectionID, Constants.DELETE);

	if (cocoon.request.get("submit_confirm")) {
		var result = FlowContainerUtils.processDeleteCollection(getDSContext(),
				collectionID);
...
function doCreateCollection(communityID) {
	...
	do {
		sendPageAndWait("admin/collection/createCollection", {
		...
		if (cocoon.request.get("submit_save")) {
			...
			result = FlowContainerUtils.processCreateCollection(getDSContext(),
					communityID, cocoon.request);
...
// Creating a new community, given the ID of its parent community or an ID of -1
// to designate top-level
function doCreateCommunity(parentCommunityID) {
	var result;
	var newCommunityID;
	// If we are not passed a communityID from the flow, we assume that is
	// passed in from the sitemap
	if (parentCommunityID == null
			&& cocoon.request.getParameter("communityID") != null) {
	...
	do {
		sendPageAndWait("admin/community/createCommunity", {
		...
		if (cocoon.request.get("submit_save")) {
			...
			result = FlowContainerUtils.processCreateCommunity(getDSContext(),
					parentCommunityID, cocoon.request);
	...
	do {
		if (cocoon.request.get("submit_return")) {
			...
			doEditItemBitstreams(itemID);
				...
				|| cocoon.request.get("submit_delete")
				|| cocoon.request.get("submit_delete_logo")) {
...
/**
 * Edit metadata of a community; presenting the user with a form of standard
 * community metadata, an option add/remove a logo and a link to the
...
function doEditCommunityMetadata(communityID) {
	...
	do {
		sendPageAndWait("admin/community/editMetadata", {
			"communityID" : communityID
		}, result);
		assertEditCommunity(communityID);
		result = null;
		...
		if (cocoon.request.get("submit_save")) {
			result = FlowContainerUtils.processEditCommunity(getDSContext(),
					communityID, false, cocoon.request);
			if (result.getContinue())
				return null;
		} else if (cocoon.request.get("submit_delete")) {
			...
			result = FlowContainerUtils.processEditCommunity(getDSContext(),
					communityID, true, cocoon.request);
		}
	} while (true);
}
...
function doDeleteCommunity(communityID) {
	...
	sendPageAndWait("admin/community/delete", {
		"communityID" : communityID
	});
	assertAuthorized(Constants.COMMUNITY, communityID, Constants.DELETE);

	if (cocoon.request.get("submit_confirm")) {
		var result = FlowContainerUtils.processDeleteCommunity(getDSContext(),
				communityID);
...
function doAssignCommunityRoles(communityID) {
	...
	do {
		sendPageAndWait("admin/community/assignRoles", {
			"communityID" : communityID
		}, result);
		assertEditCommunity(communityID);
		result = null;
		...
		if (cocoon.request.get("submit_return")
				...
				|| cocoon.request.get("submit_roles")
				|| cocoon.request.get("submit_curate")) {
			// return to the editCommunity function which will determine where
			// to go next.
			return null;
		} else if (cocoon.request.get("submit_authorizations")) {
			// authorization check moved to FlowAuthorizationUtils
		...
		// ADMIN
				...
				|| cocoon.request.get("submit_create_admin")) {
			var groupID = FlowContainerUtils.getCommunityRole(getDSContext(),
					communityID, "ADMIN");
			result = doEditGroup(groupID);
		} else if (cocoon.request.get("submit_delete_admin")) {
...
/**
 * Curate a Community Can only be performed by someone who is able to edit that
 * Community.
 */
function doCurateCommunity(communityID, task) {
	...
	do {
		if (cocoon.request.get("select_curate_group")) {
			var select_curate_group = cocoon.request
					.getParameter("select_curate_group");
			sendPageAndWait("admin/community/curateCommunity", {
				"communityID" : communityID,
				"select_curate_group" : select_curate_group
			}, result);
		} else {
		...
		if (cocoon.request.get("submit_return")
			...
			result = FlowContainerUtils.processCurateCommunity(getDSContext(),
					communityID, cocoon.request);
		} else if (cocoon.request.get("submit_queue_task")) {
			result = FlowContainerUtils.processQueueCommunity(getDSContext(),
					communityID, cocoon.request);
		}
...
/**
 * Delete a specified community role. Under the current implementation, admin
 * authorizations cannot be deleted once formed, and the default read group is
 * changed to Anonymous instead.
 */
function doDeleteCommunityRole(communityID, role) {
	...
	sendPageAndWait("admin/community/deleteRole", {
		"communityID" : communityID,
		"role" : role,
		"groupID" : groupID
	});
	...
	if (cocoon.request.get("submit_confirm")) {
		// All other roles use the standard methods
		var result = FlowContainerUtils.processDeleteCommunityRole(
				getDSContext(), communityID, role, groupID);
		return result;
	}
	return null;
}
...
/**
 * Curate a DSpace Object, from site-wide Administrator tools (Can only be
 * performed by a DSpace Administrator)
 */
function doCurate() {
	var result;
	...
	do {
		if (cocoon.request.get("select_curate_group")) {
			...
			sendPageAndWait("admin/curate/main", {
				"identifier" : identifier,
				"curate_task" : curateTask,
				"select_curate_group" : select_curate_group
			}, result);
		} else {
			sendPageAndWait("admin/curate/main", {
		...
		if (cocoon.request.get("submit_curate_task")) {
			result = FlowCurationUtils.processCurateObject(getDSContext(),
					cocoon.request);
		} else if (cocoon.request.get("submit_queue_task")) {
			result = FlowCurationUtils.processQueueObject(getDSContext(),
					cocoon.request);
		}

		// if 'identifier' parameter was set in result, pass it back to
		// sendPageAndWait call (so it is prepopulated in Admin UI)
		...
		// if 'curate_task' parameter was set in result, pass it back to
		// sendPageAndWait call (so it is prepopulated in Admin UI)
