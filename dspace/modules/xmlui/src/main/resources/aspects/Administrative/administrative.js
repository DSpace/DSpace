...
function doEditGroup(groupID)
{
    ...
    do {
        ...
        if (cocoon.request.get("submit_cancel"))
        {
       	...
       	{
       		result = FlowGroupUtils.processSaveGroup(getDSContext(),escape(groupID),groupName,memberEPeopleIDs,memberGroupIDs);
...
function doEditItem(itemID)
{
	...
	do {
        ...
        else if (cocoon.request.get("submit_embargo"))
        {
            doEditItemEmbargo(itemID);
        }
...
/**
 * Go to the edit embargo page for this tem
 */
function doEditItemEmbargo(itemID)
{
    assertEditItem(itemID);
	...
	do {
		...
		if (cocoon.request.get("submit_return") || cocoon.request.get("submit_status") || cocoon.request.get("submit_bitstreams") || cocoon.request.get("submit_metadata") || cocoon.request.get("submit_embargo") || cocoon.request.get("view_item"))
		{
			// go back to where ever we came from.
			return null;
		}
...
function doEditItemBitstreams(itemID)
{
	...
	do {
		...
		if (cocoon.request.get("submit_return") || cocoon.request.get("submit_status") || cocoon.request.get("submit_bitstreams")
            || cocoon.request.get("submit_metadata") || cocoon.request.get("view_item") || cocoon.request.get("submit_curate")
              || cocoon.request.get("submit_embargo"))
		{
			// go back to where ever we came from.
			return null;
		}
...
function doEditItemMetadata(itemID, templateCollectionID)
{
	...
	do {
		...
		if (cocoon.request.get("submit_return") || cocoon.request.get("submit_status") ||
                    ...
                    cocoon.request.get("view_item") || cocoon.request.get("submit_embargo") || cocoon.request.get("submit_curate"))
		{
			// go back to where ever we came from.
			return null;
		}
