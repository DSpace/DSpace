...
importClass(Packages.org.dspace.content.RevisionToken);
...
function doSubmission() 
{
   ...
   var tokenEvaluacion = cocoon.request.get("tokenEvaluacion");
           ...
           if (handle != null)
           {
               ...
               if (dso != null && dso.getType() == Constants.COLLECTION)
               {
                   ...
                   if (AuthorizeManager.authorizeActionBoolean(getDSContext(), dso, Constants.ADD))
                   {
                       ...
                       //ARVO: si hay token de evaluacion:si esta completado redirigir, sino actualizar el registro con el workspaceID
                       if(tokenEvaluacion!=null){
                    	   if(RevisionToken.estaFinalizado(getDSContext(),tokenEvaluacion)){
                    		   cocoon.redirectTo(contextPath+"/revisionCompletada?workspaceID="+revisionToken.getWorkspaceId()+"&tokenEvaluacion="+token, true);
                    		   getDSContext().complete();
                    	       cocoon.exit();
                    	   }else{
                    		   RevisionToken.updateWorkspaceID(getDSContext(),tokenEvaluacion,workspaceID);
                    	   }
                       }
   ...
   else
   {
       ...
       //ARVO: Si andamos con los tokens de openaire no importa que los epersons no coincidan.
       var revisionToken=null;
       if(tokenEvaluacion!=null){
    	   revisionToken=RevisionToken.verificaTokenWorkspaceID(getDSContext(),tokenEvaluacion,workspaceID);
       }
       ...
       if(revisionToken==null){
    	   ...
    	   submitterID = workspace.getSubmitter().getID();
           currentID = getDSContext().getCurrentUser().getID();
       }
...
function doDiscrimateRevision() {
	var contextPath = cocoon.request.getContextPath();
	var token = cocoon.parameters["token"];
	var revisionToken=RevisionToken.findUnique(getDSContext(),token);
	
	if(revisionToken==null){
		 cocoon.exit();
	}else if(revisionToken.getWorkspaceId()!=null){
		cocoon.redirectTo(contextPath+"/submit?workspaceID="+revisionToken.getWorkspaceId()+"&tokenEvaluacion="+token, true);
		getDSContext().complete();
        cocoon.exit();
	}else{
		cocoon.redirectTo(contextPath+"/handle/"+ConfigurationManager.getProperty("openaire.coleccion.evaluaciones")+"/submit?tokenEvaluacion="+token, true);
		getDSContext().complete();
        cocoon.exit();
	}
	
}
function doDiscrimateJuicio() {
	var contextPath = cocoon.request.getContextPath();
	var token = cocoon.parameters["token"];
	var revisionToken=RevisionToken.findUnique(getDSContext(),token);
	...
	if(revisionToken==null){
		...
		cocoon.redirectTo(contextPath+"/handle/"+ConfigurationManager.getProperty("openaire.coleccion.juicios")+"/submit?tokenEvaluacion="+token, true);
		getDSContext().complete();
        cocoon.exit();
	}
