...
public class OAIHarvester {
	...
	{    	
		...
		try 
		{
			...
			while (listRecords != null) 
			{
                ...
                // Find all our metadata formats so we can request the metadata in these formats
                // And save them as xml's in the METADATA bundle
                ListMetadataFormats formats = new ListMetadataFormats(oaiSource);
                Document formatResponse = db.build(formats.getDocument());
                List metadataFormats = formatResponse.getRootElement().getChild("ListMetadataFormats", OAI_NS).getChildren("metadataFormat", OAI_NS);
                List<String> importFormats = new ArrayList<String>();
                //Loop over our metadata formats & get all
                for (Object o : metadataFormats) {
                    Element formatElement = (Element) o;
                    String metadataFormat = formatElement.getChild("metadataPrefix", OAI_NS).getValue();
                    if (metadataFormat != null)
                        importFormats.add(metadataFormat);
                }
				...
				if (records != null && records.size()>0) 
				{
					...
					for (Element record : records) {
						...
						processRecord(oaiSource, record,OREPrefix, importFormats);
						ourContext.commit();
					}
				}
                ...
                ourContext.restoreAuthSystemState(); //added
				ourContext.commit();
			}
		}
     ...
     */
    private void processRecord(String oaiSource, Element record, String OREPrefix, List<String> storeFormats) throws SQLException, AuthorizeException, IOException, CrosswalkException, HarvestingException, ParserConfigurationException, SAXException, TransformerException 
    {
    	...
    	{	
			...
			String dateString = header.getChildText("datestamp", OAI_NS);

    		if (dateString.length() == 10) {
    			dateString += "T00:00:00"; // supply missing hh:mm:ss in current tz
    		}
    		else if (dateString.length() == 0) {
    			dateString = "1900-00-00T00:00:00"; // supply something if lacking
    		}

    		log.info("Harvesting datestamp: " + dateString);

    		Date OAIDatestamp = Utils.parseISO8601Date(dateString);
        ...
        //First of all make sure we have a bundle called METADATA
        Bundle metadataBundle;
        if(0 < item.getBundles(Constants.METADATA_BUNDLE_NAME).length){
            //We have a bundle ?
            metadataBundle = item.getBundles(Constants.METADATA_BUNDLE_NAME)[0];
            Bitstream[] bitstreams = metadataBundle.getBitstreams();
            // Remove any bitstreams it may contain
            for (Bitstream bitstream : bitstreams)
                metadataBundle.removeBitstream(bitstream);
        }else{
            metadataBundle = item.createBundle(Constants.METADATA_BUNDLE_NAME);
        }
        for (String storeFormat : storeFormats) {
            try{
                GetRecord recordget = new GetRecord(oaiSource, itemOaiID, storeFormat);

                //Create an inputstream out of our recordGet
                DOMSource source = new DOMSource(recordget.getDocument());
                StringWriter xmlAsWriter = new StringWriter();
                StreamResult result = new StreamResult(xmlAsWriter);
                TransformerFactory.newInstance().newTransformer().transform(source, result);
                // write changes
                ByteArrayInputStream bitInputStream = new ByteArrayInputStream(xmlAsWriter.toString().getBytes("UTF-8"));

                //Now that we got an inputstream we can add it to the bundle
                Bitstream bs = metadataBundle.createBitstream(bitInputStream);
                bs.setName(storeFormat + ".xml");
                bs.setSource(recordget.getRequestURL());
                BitstreamFormat format = FormatIdentifier.guessFormat(ourContext, bs);
                bs.setFormat(format);
                bs.update();

                metadataBundle.addBitstream(bs);
                metadataBundle.update();
            }catch (SAXParseException e){
                log.error(LogManager.getHeader(ourContext, "Error while harvesting", "Info: \n\tSRC: " + oaiSource + "\n\tIdentifier: " + itemOaiID + "\n\tFormat: " + storeFormat + " In DSpace coll: " + targetCollection.getHandle() + " DSpace Item id: " + item.getID()), e);
            }
        }
    ...
    private void scrubMetadata(Item item) throws SQLException, HarvestingException, AuthorizeException, IOException 
    {
    	...
    	for (DCValue value : values) 
    	{
            ...
            // A very bad hack to work around TreeBASE's md (until it's fixed)
    		if (value.schema.equals("dc") && value.element.equals("subject")) {
    			if (value.value.contains(";") || value.value.contains(",")) {
    				log.warn("TreeBASE subject to be split: " + value.value);
    			
    				StringTokenizer tokenizer = new StringTokenizer(value.value, ",;");
    				item.clearMetadata("dc", "subject", Item.ANY, Item.ANY);
    			
    				while (tokenizer.hasMoreTokens()) {
    					String token = tokenizer.nextToken().trim();
    				
    					if (!token.equals("") && !token.equals("in press")) {
    						item.addMetadata("dc", "subject", null, null, token);
    					}
    				}
    			}
    			// Those that have "in press" don't seem to have other subjects
    			// but doesn't dspace offer a way to delete one metadata value??
				else if (value.value.trim().equals("") || value.value.equals("in press")) {
					item.clearMetadata("dc", "subject", Item.ANY, Item.ANY);
					item.clearMetadata("dc", "subject_lc", Item.ANY, Item.ANY);
				}
    		}
    		
    		// Might as well muck around now that we're in here...
    		// We were going to do this in a spreadsheet, but I can do it in code
    		if (value.schema.equals("dc") && value.element.equals("publisher")) {
    			item.addMetadata("prism", "publicationName", null, null, value.value);
    		}
