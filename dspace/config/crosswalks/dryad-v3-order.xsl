...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="dryad:DryadPublication">
		<xsl:copy>
			<xsl:attribute name="xsi:schemaLocation"
				>http://purl.org/dryad/terms/<xsl:text> </xsl:text>http://datadryad.org/profile/v3/dryad.xsd</xsl:attribute>
			<xsl:apply-templates select="dcterms:type" mode="copy"/>
			<xsl:apply-templates select="dryad:status" mode="copy"/>
			<xsl:apply-templates select="bibo:status" mode="copy"/>
			<xsl:apply-templates select="dcterms:creator" mode="copy"/>
			<xsl:apply-templates select="dcterms:issued" mode="copy"/>
			<xsl:apply-templates select="dcterms:title" mode="copy"/>
			<xsl:apply-templates select="bibo:Journal" mode="copy"/>
			<xsl:apply-templates select="bibo:issn" mode="copy"/>
			<xsl:apply-templates select="bibo:eissn" mode="copy"/>
			<xsl:apply-templates select="bibo:volume" mode="copy"/>
			<xsl:apply-templates select="bibo:issue" mode="copy"/>
			<xsl:apply-templates select="bibo:pageStart" mode="copy"/>
			<xsl:apply-templates select="bibo:pageEnd" mode="copy"/>
			<xsl:apply-templates select="bibo:pages" mode="copy"/>
			<xsl:apply-templates select="dcterms:identifier" mode="copy"/>
			<xsl:apply-templates select="bibo:doi" mode="copy"/>
			<xsl:apply-templates select="bibo:pmid" mode="copy"/>
			<xsl:apply-templates select="dcterms:abstract" mode="copy"/>
			<xsl:apply-templates select="dcterms:isReferencedBy" mode="copy"/>
		</xsl:copy>
	</xsl:template>
