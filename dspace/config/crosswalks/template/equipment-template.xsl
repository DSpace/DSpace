			...
			<fo:page-sequence master-reference="simpleA4">
				<fo:flow flow-name="xsl-region-body">
					...
					<xsl:call-template name="print-value">
				    	...
				    	<xsl:with-param name="value" select="Identifier[@type = 'Institution assigned unique equipment identifier']" />
			    	</xsl:call-template>
			    	
					<xsl:call-template name="print-value">
				    	<xsl:with-param name="label" select="'Owner (Organization)'" />
				    	<xsl:with-param name="value" select="Owner/OrgUnit/Name" />
			    	</xsl:call-template>
			    	
					<xsl:call-template name="print-value">
				    	<xsl:with-param name="label" select="'Owner (Person)'" />
				    	<xsl:with-param name="value" select="Owner/Person/@displayName" />
			    	</xsl:call-template>
