...
<xsl:stylesheet
    ...
    <xsl:template match="dri:div[@id='aspect.administrative.importer.external.scidir.LiveImportResult.div.record-left']">
        <div>
            <xsl:call-template name="copy-attributes"/>
            <xsl:attribute name="rend">
                <xsl:text>col-xs-1</xsl:text>
            </xsl:attribute>
    ...
    <xsl:template match="dri:div[@id='aspect.administrative.importer.external.scidir.LiveImportResult.div.record-right']">
        <div>
            <xsl:call-template name="copy-attributes"/>
            <xsl:attribute name="rend">
                <xsl:text>col-xs-11</xsl:text>
            </xsl:attribute>
