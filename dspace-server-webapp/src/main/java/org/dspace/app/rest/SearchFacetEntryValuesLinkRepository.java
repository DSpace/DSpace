...
public class SearchFacetEntryValuesLinkRepository extends AbstractDSpaceRestRepository implements LinkRestRepository {
    ...
    public Page<SearchFacetValueRest> getValues(@Nullable HttpServletRequest request, String facetName,
        ...
        if (optionalPageable == null) {
            optionalPageable = information.getPageable();
        }

        IndexableObject scopeObject = scopeResolver.resolveScope(context, information.getScope());
        DiscoveryConfiguration discoveryConfiguration = searchConfigurationService
            .getDiscoveryConfigurationByNameOrIndexableObject(context, information.getConfiguration(), scopeObject);

        DiscoverQuery discoverQuery = queryBuilder.buildFacetQuery(context, scopeObject, discoveryConfiguration,
            information.getPrefix(), information.getQuery(), information.getSearchFilters(), information.getDsoTypes(),
            null, facetName);
        ...
        Page<SearchFacetValueRest> page = converter.toRestPage(facetValues, optionalPageable, projection);
        page.getContent().forEach((searchFacetValueRest) -> {
            searchFacetValueRest.setFacetInformation(information);
        });
        return page;
    }
}
