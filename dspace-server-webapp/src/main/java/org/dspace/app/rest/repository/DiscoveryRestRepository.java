...
public class DiscoveryRestRepository extends AbstractDSpaceRestRepository {
    ...
    public SearchConfigurationRest getSearchConfiguration(final String dsoScope, final String configuration) {
        ...
        if (isInvalidScope(dsoScope, scopeObject)) {
            return discoverConfigurationConverter.convert(new DiscoveryConfiguration(), utils.obtainProjection());
        }
    ...
    public SearchResultsRest getSearchObjects(final String query, final List<String> dsoTypes, final String dsoScope,
        ...
        if (isInvalidScope(dsoScope, scopeObject)) {
            return discoverResultConverter
                    .convert(context, query, dsoTypes, configuration, dsoScope, searchFilters, page,
                            new DiscoverResult(), new DiscoveryConfiguration(), projection);
        }
    ...
    public FacetConfigurationRest getFacetsConfiguration(final String dsoScope, final String configuration) {
        ...
        if (isInvalidScope(dsoScope, scopeObject)) {
            return discoverFacetConfigurationConverter.convert(configuration, dsoScope, new DiscoveryConfiguration());
        }
    ...
    public FacetResultsRest getFacetObjects(String facetName, String prefix, String query, List<String> dsoTypes,
                                            String dsoScope, final String configuration,
        ...
        if (isInvalidScope(dsoScope, scopeObject)) {
            return discoverFacetResultsConverter.convert(context, facetName, prefix, query,
                    dsoTypes, dsoScope, searchFilters, new DiscoverResult(), discoveryConfiguration, page,
    ...
    public SearchResultsRest getAllFacets(String query, List<String> dsoTypes, String dsoScope, String configuration,
        ...
        if (isInvalidScope(dsoScope, scopeObject)) {
            return discoverFacetsConverter.convert(context, query, dsoTypes,
                    configuration, dsoScope, searchFilters, page, new DiscoveryConfiguration(), new DiscoverResult(),
                    utils.obtainProjection());
        }
    ...
    private boolean isInvalidScope(String dsoScope, IndexableObject scopeObject) {
        return dsoScope != null && !dsoScope.isEmpty() && scopeObject == null;
    }
}
