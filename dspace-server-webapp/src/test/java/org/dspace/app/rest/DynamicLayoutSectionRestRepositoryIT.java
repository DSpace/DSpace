...
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withBrowseComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withFacetComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withIdAndBrowseComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withIdAndCountersComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withIdAndFacetComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withIdAndSearchComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withIdAndTopComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withSearchComponent;
import static org.dspace.app.rest.matcher.DynamicLayoutSectionMatcher.withTopComponent;
...
import org.dspace.layout.service.impl.DynamicLayoutSectionServiceImpl;
import org.hamcrest.Matchers;
import org.junit.Test;
import org.springframework.beans.factory.annotation.Autowired;

/**
 * Integration tests for {@link DynamicLayoutSectionRestRepository}.
 *
 * @author Luca Giamminonni (luca.giamminonni at 4science.it)
 *
 */
public class DynamicLayoutSectionRestRepositoryIT extends AbstractControllerIntegrationTest {

    @Autowired
    DynamicLayoutSectionServiceImpl dynamicLayoutSectionService;
    ...
    public void testSearchVisibleTopBarSections() throws Exception {
        ...
        List<DynamicLayoutSection> originalSections = new LinkedList<>();
        originalSections.addAll(dynamicLayoutSectionService.getComponents());

        List<List<DynamicLayoutSectionComponent>> components = new ArrayList<List<DynamicLayoutSectionComponent>>();

        components.add(new ArrayList<DynamicLayoutSectionComponent>());
        components.get(0).add(new org.dspace.layout.DynamicLayoutSearchComponent());

        List<DynamicLayoutSection> sectionsForMock = new LinkedList<>();
        sectionsForMock.add(new DynamicLayoutSection("CasualIdForTestingPurposes1", true, components));
        sectionsForMock.add(new DynamicLayoutSection("CasualIdForTestingPurposes2", false, components));
        sectionsForMock.add(new DynamicLayoutSection("CasualIdForTestingPurposes3", true, components));


        //MOCKING the sections
        dynamicLayoutSectionService.getComponents().clear();
        dynamicLayoutSectionService.getComponents().addAll(sectionsForMock);
        ...
        try {
            ...
            dynamicLayoutSectionService.getComponents().clear();
            dynamicLayoutSectionService.getComponents().addAll(originalSections);
            // end restoring
        }
    ...
    public void testFindAllWithExistingOfNestedSections() throws Exception {
        ...
        List<List<DynamicLayoutSectionComponent>> components = new ArrayList<>();

        components.add(new ArrayList<>());
        components.get(0).add(new org.dspace.layout.DynamicLayoutSearchComponent());

        List<DynamicLayoutSection> sectionsForMock = new LinkedList<>();
        DynamicLayoutSection sectionOne = new DynamicLayoutSection("CasualIdForTestingPurposes1", true, components);
        DynamicLayoutSection sectionTwo = new DynamicLayoutSection("CasualIdForTestingPurposes2", true, null);
        sectionTwo.setNestedSections(List.of(sectionOne));
        DynamicLayoutSection sectionThree = new DynamicLayoutSection("CasualIdForTestingPurposes3", true, null);
    ...
    @Test
    public void testCreateDynamicLayoutSectionWithNestedSectionsAndComponents() throws Exception {
        ...
        DynamicLayoutSection sectionOne =
            new DynamicLayoutSection("CasualIdForTestingPurposes1", true, components);

        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            sectionOne.setNestedSections(
                List.of(new DynamicLayoutSection("CasualIdForTestingPurposes2", true, components))
            );
        });
