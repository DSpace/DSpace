...
public class TaskRestRepositoriesIT extends AbstractControllerIntegrationTest {
    ...
    @Test
    @Ignore(value = "See w2p-68726 logs => submit_edit_metadata in edit step ends up in AcceptEditRejectAction#execute" +
        " where there is no code for handling a SUBMIT_EDIT_METADATA parameter (so it results in a TYPE_CANCEL " +
        "which leads to the 422 in XmlWorkflowServiceImpl#doState)")
    /**
    ...
    @Test
    @Ignore(value = "Ends up in FinalEditAction#processMainPage where all options besides submit_approve end up in " +
        "TYPE_SUBMISSION_PAGE (so it does not result in a 422 Unprocesseable Entity)")
    /**
    ...
    @Test
    @Ignore( value = "Ends up in FinalEditAction#processMainPage where all options besides submit_approve end up " +
        "in TYPE_SUBMISSION_PAGE, so even non-existent/non-valid options do not end up as a TYPE_CANCEL " +
        "so they do not return the expected 244 Unprocessable Entity")
    /**
